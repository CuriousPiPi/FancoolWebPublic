...
def fetch_raw_perf_rows(
    ...
    sql = (
        "SELECT p.model_id, p.condition_id, p.rpm, p.airflow_cfm AS airflow, p.noise_db, p.pressure_mmh2o "
    ...
    for r in rows or []:
        ...
        try:
            ...
            pr = float(pressure_val) if pressure_val is not None else None
        except Exception:
            continue
        ...
        bucket["pressure_mmh2o"].append(pr)
