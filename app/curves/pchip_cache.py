...
_INTERP_CONTRACT = "conservative_pchip_sharpturn_tail_virtual_v4"
...
def raw_quads_hash(rpm: List[float], airflow: List[float], noise: List[float], pressure: List[float]) -> str:
    """对四轴点（含风压）的顺序无关散列，None 以 'null' 表示，统一到 6 位小数。"""
    def norm(v):
        if v is None: return "null"
        try:
            f = float(v)
    ...
    for i in range(n):
        quads.append((
            norm(rpm[i] if i < len(rpm or []) else None),
            norm(airflow[i]),
            norm(noise[i] if i < len(noise or []) else None),
            norm(pressure[i] if i < len(pressure or []) else None),
        ))
    quads.sort()
    buf = ";".join("|".join(q) for q in quads)
    return hashlib.sha1(buf.encode("utf-8")).hexdigest()
...
def build_pchip_model_with_opts(xs_in: List[float], ys_in: List[float], axis: str) -> Optional[Dict[str, Any]]:
    """统一轴向 PCHIP 构建：统一用于 rpm/noise 两类 airflow 相关曲线。"""
    pairs = []
...
def env_key_for_perf() -> str:
    ...
    Changes to interpolation contract/tuning constants, monotone enforcement,
    node-lock flags, or CODE_VERSION produce a different key, causing cached
    models built under old settings to be treated as stale.
    """
...
# =========================
# Sone 段（perf cache 的可选扩展）
# =========================
#
# sone 曲线由 admin 侧音频标定显式写入 perf cache（写时补全），它 **不参与**
# perf cache 的 TTL / data_hash / env_key 自动重建判定：缺少 sone 的 perf cache
# 仍然是合法缓存，读取路径不会因此触发任何重跑。

SONE_PCHIP_KEYS = ("rpm_to_sone", "sone_to_rpm", "sone_to_airflow")


def _normalize_sone_payload(sone: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """Return a normalized sone payload, or None when it carries no usable curve."""
    if not isinstance(sone, dict):
        return None
    out: Dict[str, Any] = {}
    for k in SONE_PCHIP_KEYS:
        v = sone.get(k)
        if isinstance(v, dict) and v:
            out[k] = v
    if not out.get("rpm_to_sone"):
        return None
    for k in ("rpm_nodes", "sone_nodes"):
        v = sone.get(k)
...
def _apply_sone_to_payload(payload: Dict[str, Any], sone: Optional[Dict[str, Any]]) -> None:
    """Merge a normalized sone payload into an in-memory perf cache payload."""
    norm = _normalize_sone_payload(sone)
    if not norm:
        return
    pchip = payload.setdefault("pchip", {})
    for k in SONE_PCHIP_KEYS:
        if k in norm:
            pchip[k] = norm[k]
    if norm.get("rpm_nodes"):
        payload["rpm_nodes_for_sone"] = norm["rpm_nodes"]
    if norm.get("sone_nodes"):
        payload["sone_nodes"] = norm["sone_nodes"]
    if norm.get("version"):
        payload["sone_version"] = norm["version"]
    if norm.get("source"):
        payload["sone_source"] = norm["source"]


def extract_sone_from_perf_cache(cached: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """Extract the sone section of a loaded perf cache payload (or None)."""
    if not isinstance(cached, dict):
        return None
    pchip = cached.get("pchip") or {}
    if not isinstance(pchip, dict):
        return None
    out: Dict[str, Any] = {k: pchip.get(k) for k in SONE_PCHIP_KEYS if isinstance(pchip.get(k), dict)}
    if not out.get("rpm_to_sone"):
        return None
    if isinstance(cached.get("rpm_nodes_for_sone"), list):
        out["rpm_nodes"] = cached.get("rpm_nodes_for_sone")
    if isinstance(cached.get("sone_nodes"), list):
        out["sone_nodes"] = cached.get("sone_nodes")
    if cached.get("sone_version"):
        out["version"] = cached.get("sone_version")
    if cached.get("sone_source"):
        out["source"] = cached.get("sone_source")
    return out


def merge_sone_into_perf_cache(model_id: int, condition_id: int, sone: Dict[str, Any]) -> bool:
    """Merge a sone payload into an existing perf cache file, in place.

    This is an explicit **write-time** completion helper used by the admin
    calibration flows; it never rebuilds or revalidates the perf cache and it
    never triggers a short-recording re-run.  Returns False when no perf cache
    exists yet (the caller decides whether to build one first) or when the
    payload carries no usable sone curve.
    """
    norm = _normalize_sone_payload(sone)
    if not norm:
        return False
    ...
    if cached is None:
        return False
    _apply_sone_to_payload(cached, norm)
...
def save_unified_perf_model(model_id: int, condition_id: int, models: dict, *, data_hash: str, env_key: str,
                            supports_audio: bool = False, sone: Optional[Dict[str, Any]] = None,
    ...
    if raw is not None:
        payload["raw"] = raw
    _apply_sone_to_payload(payload, sone)
...
def get_or_build_unified_perf_model(model_id: int, condition_id: int,
                                    ...
                                    pressure: Optional[List] = None) -> Optional[Dict[str, Any]]:
    """
    四合一模型唯一入口：
      - 依据四轴原始点计算 data_hash（含 pressure_mmh2o）
      - 组成 env_key（含插值契约/局部保守调参/单调/节点锁定/代码版本）
      - 先查内存 LRU；再查磁盘；任一命中且 meta 匹配则直接返回
      - 否则重建四条曲线并落盘 + 进入 LRU
    """
    data_hash = raw_quads_hash(rpm or [], airflow or [], noise or [], pressure or [])
    ...
    # sone 段不参与重建判定，但重建时需要保留已有的 sone 曲线，
    # 避免基础性能曲线刷新把 admin 写入的 sone 丢掉（sone 不进入自动自愈）。
    prev_sone = extract_sone_from_perf_cache(cached)
    ...
    save_unified_perf_model(model_id, condition_id, pack, data_hash=data_hash, env_key=env_key,
                            supports_audio=supports_audio, sone=prev_sone, raw=raw_block)
    out = {
        "type": "perf_pchip_v1",
        "model_id": int(model_id),
        "condition_id": int(condition_id),
    ...
    _apply_sone_to_payload(out, prev_sone)
    if _INMEM and _note_hit(ikey) >= _ADMIT_HITS:
        _INMEM.put(ikey, out)
    return out
