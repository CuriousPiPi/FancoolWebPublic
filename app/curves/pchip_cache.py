...
_INTERP_CONTRACT = "conservative_pchip_sharpturn_v3"
...
_SHARP_STEP_SLOPE_RATIO_TRIGGER = 3.0
...
def _pchip_slopes_fritsch_carlson(xs: List[float], ys: List[float], axis: str) -> List[float]:
    ...
    for i in range(1, n - 1):
        d_prev = delta[i - 1]
        d_next = delta[i]
        if d_prev == 0.0 or d_next == 0.0 or abs(d_prev) <= eps or abs(d_next) <= eps or d_prev * d_next <= 0:
            m[i] = 0.0
        else:
            w1 = 2.0 * h[i] + h[i - 1]
            w2 = h[i] + 2.0 * h[i - 1]
    ...
    for i in range(n - 1):
        ...
        seg_mag = abs(di)
        is_sharp = (
            (i > 0 and _is_sharp_slope_jump(di, delta[i - 1], eps)) or
            (i < n - 2 and _is_sharp_slope_jump(di, delta[i + 1], eps))
        )
