...
from app import whistle_value_service
...
def refresh() -> Dict[int, dict]:
    ...
    whistle_values: Dict[int, float] = {}
    whistle_baseline_condition_id = None
    if data:
        try:
            whistle_values, whistle_baseline_condition_id = whistle_value_service.derive_whistle_values_for_models(
                _fetch_all,
                data.keys(),
                logger=_logger,
            )
            ...
            if _logger:
                _logger.exception('model_meta_cache whistle value load failed')
            whistle_values, whistle_baseline_condition_id = {}, None

    for model_id, item in data.items():
        whistle_value = whistle_values.get(model_id)
        if whistle_value is None or whistle_baseline_condition_id is None:
            item.pop('whistle_value', None)
            item.pop('whistle_baseline_condition_id', None)
            continue
        item['whistle_value'] = whistle_value
        item['whistle_baseline_condition_id'] = int(whistle_baseline_condition_id)
