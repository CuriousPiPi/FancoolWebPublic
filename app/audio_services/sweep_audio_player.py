...
SELECTION_TOL_RPM = 15.0      # 固定 RPM 容差（绝对值）/ Fixed RPM tolerance (absolute)
...
LOOP_CROSSFADE_MS = 120.0     # 循环交叉淡化时长（毫秒）/ Loop crossfade duration in milliseconds
VALIDATION_SAMPLE_SIZE = 10   # 验证时检查的帧数 / Number of frames to check during validation
CORR_OFFSET_NONNEGATIVE_ONLY = False # loop 对齐策略开关 / Loop alignment switches

MIN_RELIABILITY = 0.2         # reliability 硬阈值 / Hard reliability threshold
...
def select_best_subclip_by_min_endpoint_rpm_diff(
    ...
    """
    对单个连续段（run）选择一个子片段 i..j（按 run 内位置），
    使得 abs(rpm[j] - rpm[i]) 最小，且长度 (j-i+1) >= n。
    Returns:
        (best_i_pos, best_j_pos, best_diff)
        注意 i/j 是 run 内位置（0..len(run)-1），不是 sweep_frame_index 的全局下标
    """
...
def create_wav_bytes(audio_data: np.ndarray, fs: int) -> bytes:
    ...
    """
    # 转换为 16-bit PCM / Convert to 16-bit PCM
