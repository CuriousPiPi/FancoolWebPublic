...
(function () {
  ...
  var PURCHASE_BADGE_META = {
    jd: { text: 'JD', cls: 'is-jd' },
    taobao: { text: '淘', cls: 'is-taobao' },
    pinduoduo: { text: '拼', cls: 'is-pdd' },
    pdd: { text: '拼', cls: 'is-pdd' },
    other: { text: '购', cls: 'is-other' }
  };
  ...
  function _getDocLang() {
    try {
      return String((document.documentElement && document.documentElement.lang) || '').toLowerCase();
    } catch (_) {
      return '';
    }
  }

  function _resolvePurchaseText(link) {
    var lang = _getDocLang();
    var preferEn = lang.indexOf('en') === 0;
    var zh = link && link.display_text_zh ? String(link.display_text_zh).trim() : '';
    var en = link && link.display_text_en ? String(link.display_text_en).trim() : '';
    return preferEn ? (en || zh || '购买链接') : (zh || en || '购买链接');
  }

  function _resolvePurchaseBadgeMeta(platformCode) {
    var code = (platformCode || '').toLowerCase();
    return PURCHASE_BADGE_META[code] || PURCHASE_BADGE_META.other;
  }

  function _buildPurchaseSection(item, purchaseSource) {
    ...
    purchaseTitle.textContent = '购买链接';
    purchase.appendChild(purchaseTitle);

    var links = Array.isArray(item && item.purchase_links) ? item.purchase_links : [];
    if (!links.length) {
      var empty = document.createElement('div');
      empty.className = 'fc-model-detail__purchase-empty';
      empty.textContent = '暂无购买链接';
      purchase.appendChild(empty);
      return purchase;
    }

    var list = document.createElement('div');
    list.className = 'fc-model-detail__purchase-list';
    var rendered = 0;

    links.forEach(function (link) {
      var linkId = Number(link && link.id);
      if (!Number.isFinite(linkId) || linkId <= 0) return;

      var anchor = document.createElement('a');
      anchor.className = 'fc-model-detail__purchase-link';
      var href = '/go/purchase/' + linkId;
      if (purchaseSource) href += '?from=' + encodeURIComponent(purchaseSource);
      anchor.href = href;
      anchor.target = '_blank';
      anchor.rel = 'nofollow sponsored noopener noreferrer';

      var badgeMeta = _resolvePurchaseBadgeMeta(link.platform_code);
      var badge = document.createElement('span');
      badge.className = 'fc-model-detail__purchase-badge ' + badgeMeta.cls;
      badge.textContent = badgeMeta.text;

      var text = document.createElement('span');
      text.className = 'fc-model-detail__purchase-link-text';
      text.textContent = _resolvePurchaseText(link);
    ...
    if (!rendered) {
      ...
      fallback.className = 'fc-model-detail__purchase-empty';
      fallback.textContent = '暂无购买链接';
      purchase.appendChild(fallback);
      return purchase;
    }
  ...
  function _resolveField(key, item) {
    switch (key) {
      ...
      case 'stop_pwm': {
        var stopPwm = item.stop_pwm;
        return {
          value: (stopPwm != null && stopPwm !== '') ? ('≤ ' + stopPwm + '%') : '无',
          isElement: false,
        };
      }
      case 'color_flags':
   ...
   * @param {string} [options.purchaseSource] - source identifier appended as ?from= to purchase link hrefs
  ...
  function build(item, options) {
    ...
    var purchaseSource = opts.purchaseSource || null;
    ...
    // ---- Purchase ----
    if (showPurchase) {
      root.appendChild(_buildPurchaseSection(item, purchaseSource));
    }
