...
function buildCustomConditionDropdown(sel, items){
  ...
  function initConditionSearch(){
    ...
    form.addEventListener('submit', async (e)=>{
      ...
      // Include active score profile so server sorts by the correct profile
      payload.score_profile = (typeof window.ScoreProfile !== 'undefined')
        ? window.ScoreProfile.getProfile()
        : 'med';

      // Store for profile-switch re-fetch
  ...
  // Radar: 6-axis layout — CW from top, CIDs map to hexagon vertices
  ...
  // Last search payload (includes score_profile) — used to re-fetch on profile change
   ...
   *  When score_profiles is available, uses the currently selected profile's condition_scores.
   */
  function radarCacheToItems(radarData) {
    // Try using profile-specific condition scores if available
    if (typeof window.ScoreProfile !== 'undefined' && radarData && radarData.score_profiles) {
      ...
      if (profData && profData.condition_scores) {
        return Object.entries(profData.condition_scores).map(([cid, score]) => ({
          condition_id: Number(cid),
          score_total: score,
        }));
      }
    }
    // Fallback: use med profile's raw conditions
  ...
  /** Get the composite score for a radar cache entry under the current profile. */
  function radarCacheCompositeScore(radarData) {
    if (!radarData) return null;
    if (typeof window.ScoreProfile !== 'undefined' && radarData.score_profiles) {
      const profile = window.ScoreProfile.getProfile();
      const profData = radarData.score_profiles[profile];
      if (profData && profData.composite_score !== undefined) return profData.composite_score;
    }
  ...
  function renderConditionList(items, compositeScore){
    ...
    // Use shared SVG builder (window.buildMiniRadarSVG) if available; fall back to inline.
    ...
    // Fallback (should not be reached when loaded normally)
...
function initModelCascade(){
  ...
  // Custom model dropdown: renders a score badge after the model name when a
  // composite score is available.  Colors come from the shared ScoreBadgeHelper.
  ...
  modelSelect.addEventListener('change', async ()=>{
    ...
    form.addEventListener('submit', (e)=>{
      ...
      if (typeof window.addModelToRadar === 'function') {
        window.addModelToRadar(Number(mid), brand, label);
  ...
  function initAll(){
    ...
    // Re-render condition list and re-fetch search results when score profile changes
    window.addEventListener('localstate:score_profile_changed', async (e) => {
      // Re-render add-by-model condition list from cached radar data (local switch)
      const modelSelect = document.getElementById('modelSelect');
      const mid = modelSelect && modelSelect.value;
      if (mid) {
        const cached = _radarCache[String(mid)];
        if (cached && cached.conditions) {
          const items = radarCacheToItems(cached);
          if (items.length) renderConditionList(items, radarCacheCompositeScore(cached));
        }
      }

      // Re-fetch last search with the new profile so server-side sort order is correct
      if (_lastSearchBasePayload) {
        const newProfile = (e.detail && e.detail.profile) ||
          (typeof window.ScoreProfile !== 'undefined' ? window.ScoreProfile.getProfile() : 'med');
        const newPayload = Object.assign({}, _lastSearchBasePayload, { score_profile: newProfile });
        _lastSearchBasePayload = newPayload;
        await _executeSearch(newPayload, { showToasts: false, switchTab: false });
      }
    });
  }
