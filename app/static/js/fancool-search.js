...
function buildCustomConditionDropdown(sel, items){
  ...
  function initConditionSearch(){
    ...
    let sortSelUi = null;
    if (sortSel) sortSelUi = buildCustomSelectFromNative(sortSel, { placeholder: '排序依据' });
...
function initModelCascade(){
  ...
  function _renderModelOption(opt) {
    ...
    const styleAttr = window.ScoreBadgeHelper
      ...
      : '';
    const badge = `<span class="rpv2-score-badge" style="${styleAttr}">${Math.round(score)}</span>`;
  ...
  modelSelect.addEventListener('change', async ()=>{
    ...
    if (cached && cached.conditions) {
      ...
      if (items.length) {
        renderConditionList(items, cached.composite_score);
        return;
      }
    }
    ...
    try {
      ...
      const compositeScore = (_radarCache[String(mid)] && _radarCache[String(mid)].composite_score) ?? null;
