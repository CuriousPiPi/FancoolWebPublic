...
(function initLadderModalModule() {
  ...
  var SCORE_BAR_MAX_W = 220;  // maximum score bar width
  ...
  var TEXT_W          = 200;  // model/brand text zone width
  var PRICE_BAR_X     = TEXT_X + TEXT_W + 6;            // 466 – price bar left edge
  var PRICE_BAR_MAX_W = 80;   // maximum price bar width
  ...
  var RGB_TAG_H       = 14;
  var RGB_TAG_GAP     = 6;
  var RGB_TAG_RADIUS  = 7;
  var WHISTLE_TAG_TEXT = '啸叫';
  var WHISTLE_TAG_PAD_X = 6;
  var WHISTLE_TAG_H = 14;
  var WHISTLE_TAG_GAP = 6;
  var WHISTLE_TAG_RADIUS = 7;
  var WHISTLE_LOW_THRESHOLD = 2;
  var WHISTLE_MID_THRESHOLD = 4;
  var C_WHISTLE_LOW = '#86efac';
  var C_WHISTLE_MID = '#fdba74';
  var C_WHISTLE_HIGH = '#fca5a5';
  ...
  function getWhistleValue(item) {
    var rawValue = item && item.whistle_value;
    if (rawValue === null || rawValue === undefined || rawValue === '') return null;
    var value = Number(rawValue);
    return Number.isFinite(value) && value >= 0 ? value : null;
  }

  function shouldShowWhistleTag(item, ladderType) {
    return ladderType === 'intake_exhaust' && getWhistleValue(item) !== null;
  }

  function setWhistleTagFont(ctx) {
    ctx.font = '600 10px sans-serif';
  }

  function measureWhistleTagWidth(ctx) {
    var prevFont = ctx.font;
    setWhistleTagFont(ctx);
    var width = ctx.measureText(WHISTLE_TAG_TEXT).width + WHISTLE_TAG_PAD_X * 2;
    ctx.font = prevFont;
    return width;
  }

  function getWhistleTagColor(value) {
    if (value < WHISTLE_LOW_THRESHOLD) return C_WHISTLE_LOW;
    if (value < WHISTLE_MID_THRESHOLD) return C_WHISTLE_MID;
    return C_WHISTLE_HIGH;
  }

  function drawWhistleTag(ctx, x, centerY, value) {
    var width = measureWhistleTagWidth(ctx);
    var top = centerY - WHISTLE_TAG_H / 2;
    ctx.fillStyle = getWhistleTagColor(value);
    roundRect(ctx, x, top, width, WHISTLE_TAG_H, WHISTLE_TAG_RADIUS);
    ctx.fill();
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(WHISTLE_TAG_TEXT, x + WHISTLE_TAG_PAD_X, centerY + 0.5);
    ctx.font = prevFont;
  }
  ...
  function renderLadder(canvas, ladderData) {
    ...
    items.forEach(function (item, idx) {
      ...
      var whistleValue = getWhistleValue(item);
      var showWhistleTag = shouldShowWhistleTag(item, ladderData.ladderType);
      var whistleTagWidth = showWhistleTag ? measureWhistleTagWidth(ctx) : 0;
      var trailingTagWidth = 0;
      if (showRgbTag) trailingTagWidth += rgbTagWidth + RGB_TAG_GAP;
      if (showWhistleTag) trailingTagWidth += whistleTagWidth + WHISTLE_TAG_GAP;
      var modelMaxWidth = TEXT_W - trailingTagWidth;
      ...
      if (showRgbTag) {
        ...
        tagX += rgbTagWidth + RGB_TAG_GAP;
      }

      if (showWhistleTag && whistleValue !== null) {
        drawWhistleTag(ctx, tagX, cy - 9.2, whistleValue);
      }
