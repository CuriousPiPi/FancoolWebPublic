...
(function initLadderModalModule() {
  ...
  var TITLE_H         = 92;   // title + date + note area
  ...
  var C_MODEL         = '#111827';
  var C_BRAND         = '#6b7280';
  var C_PRICE_BAR     = '#f59e0b';
  var C_PRICE_TEXT    = '#92400e';
  ...
  function renderLadder(canvas, ladderData) {
    ...
    noteLines.forEach(function (line, i) {
      if (noteY + i * noteLineH > TITLE_H - NOTE_BOTTOM_PAD) return;
      ctx.fillText(line, noteX, noteY + i * noteLineH);
    });
    ...
    items.forEach(function (item, idx) {
      ...
      var modelText = fillTextClipped(ctx, item.model_name || '', TEXT_X, cy - 4, modelMaxWidth);
      if (showRgbTag) {
        var modelTextWidth = ctx.measureText(modelText).width;
        drawRgbTag(ctx, TEXT_X + modelTextWidth + RGB_TAG_GAP - 2, cy - 9);
      }
