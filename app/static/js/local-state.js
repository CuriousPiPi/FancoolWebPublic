...
(function(global){
  const LS_KEYS = {
    ...
    LIGHTING_LIKES: 'fc_lighting_likes_v1',
    // Radar selection – model identity, visibility and active conditions are persisted (no scores)
    RADAR_MODELS: 'fc_radar_models_v1',  // [{modelId, brand, label, hidden?}]
    RADAR_CONDS:  'fc_radar_conds_v1',   // [cid strings]
  };

  const MAX_RECENTLY_REMOVED = 20;
  const DEFAULT_X_AXIS = 'rpm';

  // Keep one-time legacy cleanup for older localStorage payloads.
  ...
  const persistedLightingArray = readJSON(LS_KEYS.LIGHTING_LIKES, []);
  let _lightingLiked = new Set(Array.isArray(persistedLightingArray)?persistedLightingArray:[]);
  ...
  function _recomputeLocalFP(){
    ...
    _lightingLiked.forEach(mid=>{
      const k = `lighting_${mid}`;
      const hv = _hash64(k);
      xor_v ^= hv;
      sum_v = (sum_v + hv) & MASK_64;
      c++;
  ...
  function _persistLightingLikes(){
    writeJSON(LS_KEYS.LIGHTING_LIKES, Array.from(_lightingLiked));
  }

  const lightingLikesAPI = {
    add(mid){
      const key = String(mid);
      if (!_lightingLiked.has(key)){
        _lightingLiked.add(key);
        _recomputeLocalFP();
        _persistLightingLikes();
      }
    },
    remove(mid){
      const key = String(mid);
      if (_lightingLiked.has(key)){
        _lightingLiked.delete(key);
        _recomputeLocalFP();
        _persistLightingLikes();
      }
    },
    has: (mid) => _lightingLiked.has(String(mid)),
    getAll: () => Array.from(_lightingLiked),
    setAll(arr){
      _lightingLiked = new Set(Array.isArray(arr) ? arr.map(String) : []);
      _recomputeLocalFP();
      _persistLightingLikes();
    },
  };
  ...
  function addOrUpdateRemoved(info){
    ...
    // Enrich with model-level metadata from the persistent cache so that
    // browsing-history cards show brand/model/price/speed/size correctly
    // even after a page reload (the metadata is stored alongside the key).
    const mid = info.model_id != null ? String(info.model_id) : null;
    const meta = (mid && window.__modelMetaCache) ? (window.__modelMetaCache[mid] || {}) : {};
    const idx = removed.findIndex(r=>r.key===info.key);
    if (idx >= 0){
      const rec = removed[idx];
      rec.removed_time = now;
      // Refresh metadata from cache when available, so stale entries get updated.
      if (meta.brand)     rec.brand     = meta.brand;
      if (meta.model)     rec.model     = meta.model;
      if (meta.reference_price != null) rec.reference_price = meta.reference_price;
      if (meta.max_speed) rec.max_speed  = meta.max_speed;
      if (meta.size)      rec.size       = meta.size;
      if (meta.thickness) rec.thickness  = meta.thickness;
      if (meta.rgb_names_zh) rec.rgb_names_zh = meta.rgb_names_zh;
      ...
      removed.unshift({
        ...
        removed_time:    now,
        brand:           info.brand           || meta.brand           || '',
        model:           info.model           || meta.model           || '',
        reference_price: info.reference_price ?? meta.reference_price ?? null,
        max_speed:       info.max_speed       || meta.max_speed       || null,
        size:            info.size            || meta.size            || '',
        thickness:       info.thickness       || meta.thickness       || '',
        rgb_names_zh: info.rgb_names_zh || meta.rgb_names_zh || '',
      });
    }
  ...
  /**
   * Remove a selected pair by key WITHOUT adding it to browsing history.
   * Use this when pairs are removed from LocalState for non-membership reasons
   * (e.g. model hide/show, condition activate/deactivate) so that browsing
   * history is only updated on true radar model removal or clear.
   */
  function removePairSilent(key){
      const idx=findSelectedIndex(key);
      if (idx<0) return false;
      selected.splice(idx,1);
      persistAll();
  ...
  /**
   * Remove ALL browsing-history entries for a given model (by model_id).
   * Called when a model is added to radar comparison so it no longer appears
   * in Browsing History.
   */
  function removeModelFromHistory(modelId){
    const mid = Number(modelId);
    if (!Number.isFinite(mid)) return;
    const before = removed.length;
    removed = removed.filter(r => Number(r.model_id) !== mid);
    if (removed.length !== before){
      persistAll();
      dispatchChange('history_sync',{ modelId: mid });
    }
  }

  /**
   * Add (or refresh) a model-level entry in Browsing History.
   * Uses key "<modelId>_0" (condition_id = 0) as the model-level sentinel.
   * Ordering is newest-first via addOrUpdateRemoved (unshift + timestamp update).
   * Called when a model is removed from radar comparison.
   */
  function addModelToHistory(modelId, brand, label){
    const mid = Number(modelId);
    if (!Number.isFinite(mid)) return;
    addOrUpdateRemoved({
      key:          makeKey(mid, 0),
  ...
  // v2 最小存储不再写条件文本，保留空实现以兼容旧调用
  function patchCondition(_mid, _cid, _conditionText){ /* v2: condition text not persisted in LocalState */ }

  // ---- Radar selection persistence (model identity + active conditions only) ----
  function getRadarModels() { return readJSON(LS_KEYS.RADAR_MODELS, []); }
  function setRadarModels(arr) { writeJSON(LS_KEYS.RADAR_MODELS, Array.isArray(arr) ? arr : []); }
  function getRadarConditions() { return readJSON(LS_KEYS.RADAR_CONDS, []); }
  function setRadarConditions(arr) { writeJSON(LS_KEYS.RADAR_CONDS, Array.isArray(arr) ? arr : []); }
  ...
  const api = {
    ...
    persistAll,
    likes: likesAPI,
    lightingLikes: lightingLikesAPI,
    patchCondition,
    // Radar selection persistence
    getRadarModels,
    setRadarModels,
    getRadarConditions,
    setRadarConditions,
