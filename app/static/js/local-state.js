...
(function(global){
  const LS_KEYS = {
    ...
    SCORE_PROFILE: 'fc_score_profile_v1', // 'low' | 'med' | 'hi'
  };

  const MAX_RECENTLY_REMOVED = 50;
  const DEFAULT_X_AXIS = 'rpm';

  // TODO: 确认 v1→v2 迁移已普遍完成后可移除此清理块
  ...
  function addOrUpdateRemoved(info){
    ...
    if (idx >= 0){
      ...
      if (meta.rgb_light) rec.rgb_light  = meta.rgb_light;
      ...
      removed.unshift({
        ...
        rgb_light:       info.rgb_light       || meta.rgb_light       || '',
      });
    }
  ...
  const VALID_SCORE_PROFILES = ['low', 'med', 'hi'];
  const DEFAULT_SCORE_PROFILE = 'med';

  function getScoreProfile() {
    const v = (localStorage.getItem(LS_KEYS.SCORE_PROFILE) || '').trim();
    return VALID_SCORE_PROFILES.includes(v) ? v : DEFAULT_SCORE_PROFILE;
  }
  function setScoreProfile(p) {
    const profile = VALID_SCORE_PROFILES.includes(p) ? p : DEFAULT_SCORE_PROFILE;
    try { localStorage.setItem(LS_KEYS.SCORE_PROFILE, profile); } catch(_) {}
    window.dispatchEvent(new CustomEvent('localstate:score_profile_changed', { detail: { profile } }));
    dispatchChange('score_profile_changed', { profile });
    return profile;
  }
