...
const SweepAudioPlayer = (() => {
  ...
  async function fetchBuffer(modelId, conditionId, rpm) {
    ...
    if (!resp.ok) {
      ...
      try { const j = await resp.json(); if (j.error_message) msg = j.error_message; } catch(_) {}
      throw new Error(msg);
    }
...
function getTargetRpmForRow(row, mode) {
  let targetRpm;
  if (mode === 'rpm') {
    targetRpm = Number(getXQueryOrDefault('rpm'));
  } else {
    // noise 轴：cross 是 rpm
    if (Number.isFinite(row.cross) && row.cross > 0) {
      ...
      // 回退：最大 rpm
      targetRpm = rpmMaxForSeries(row.__series || {}, row.__spectrumModel);
    }
  }
  
  // Clamp RPM to series min/max range
