(function(){
  // 对外 API
  const API = { mount, render, resize, setOnXAxisChange, __ensureDock: () => ensureSpectrumDock(), highlightModelSeries, downplayModelSeries, expandMainChart };
  ...
  /* =========================
   * 通用拖拽控制器 DragController
   * ========================= */
  /**
   * Chart/Sidebar drag perf suppression state.
   * This is intentionally scoped to the chart + sidebar drag paths that share
   * expensive observers/cache invalidation behavior; do not reuse as a generic
   * drag bus for unrelated interactions.
   */
  const CHART_SIDEBAR_DRAG_PERF_END_EVENT = 'fc:chart-sidebar-drag-perf-end';
  const LEGACY_DRAG_INTERACTION_END_EVENT = 'fc:drag-interaction-end';
  function isChartSidebarDragPerfActive(){
    return !!window.__chartSidebarDragPerfActive;
  }
  function setChartSidebarDragPerfActive(active){
    const next = !!active;
    if (window.__chartSidebarDragPerfActive === next) return;
    window.__chartSidebarDragPerfActive = next;
    // Backward-compat mirror for older call sites/listeners.
    window.__dragInteractionActive = next;
    if (!next) {
      try { window.dispatchEvent(new CustomEvent(CHART_SIDEBAR_DRAG_PERF_END_EVENT)); } catch(_) {}
      try { window.dispatchEvent(new CustomEvent(LEGACY_DRAG_INTERACTION_END_EVENT)); } catch(_) {}
    }
  }
  function setDragInteractionActive(active){
    setChartSidebarDragPerfActive(active);
  }
  window.__chartSidebarDragPerfActive = false;
  window.__dragInteractionActive = false;
  window.ChartSidebarDragPerf = window.ChartSidebarDragPerf || {
    begin(){ setChartSidebarDragPerfActive(true); },
    end(){ setChartSidebarDragPerfActive(false); },
    isActive(){ return isChartSidebarDragPerfActive(); },
    endEventName: CHART_SIDEBAR_DRAG_PERF_END_EVENT,
  };
...
function ensureSpectrumDock() {
  ...
  btn.id = 'spectrumDock';
  btn.type = 'button';
  btn.className = 'spectrum-dock fc-arrow-btn';
  btn.setAttribute('aria-label', '展开/收起频谱');

  btn.innerHTML = `
    <svg class="fc-chevron" viewBox="0 0 16 16" width="16" height="16" aria-hidden="true" focusable="false">
...
function ensureLegendRail(){
  ...
  let rail = document.getElementById('legendRail');
  if (!rail) {
    ...
    // Place inside chart-main-col so it stacks below the chart in narrow mode
    const mainCol = root && root.closest('.chart-main-col');
    if (mainCol) {
      mainCol.appendChild(rail);
    } else {
      const shell = document.getElementById('chart-settings') || (root && root.closest('.fc-chart-container'));
      if (shell) shell.appendChild(rail);
      else document.body.appendChild(rail);
    }
  }

  __legendRailEl = rail;
  __legendScrollEl = rail.querySelector('.legend-scroll');
...
function renderLegendRailItems(){
  ...
  // 集成拟合模式：新的合并头部（标题 + 输入 + 单位）
  let wrapper = __legendScrollEl.querySelector('.legend-integrated-wrapper.fit-on');
  if (!wrapper || !wrapper.querySelector('.integrated-fit-head') || !wrapper.querySelector('.legend-fit-grid')) {
    const headHtml = `
      <div class="integrated-fit-head is-active">
        <div class="title fit-head-title">
          ${FIT_X_LABEL} 拟合当前位置
          <input id="fitXInputLegend" type="number" step="1" />
          <span id="fitXUnitLegend" class="unit"></span>
        </div>
        <div class="fit-preset-scroll">
          <div id="fitPresetControlsLegend" class="fit-preset-controls" aria-label="常用拟合坐标"></div>
        </div>
      </div>
    `;
    ...
    `;
    wrapper = __legendScrollEl.querySelector('.legend-integrated-wrapper.fit-on');
  }
  renderFitPresetControls();
...
function updateLegendRail(){
  ...
  // 追踪 root 的几何变化（位置/尺寸），用于在容器“移动但不改变尺寸”时重放置拟合气泡
  ...
  const FIT_X_LABEL = 'X轴';
  const DEFAULT_FIT_MODE = 'rpm';
  const FIT_PRESET_LS_KEY = 'fc_fit_preset_x_v3';
  const FIT_PRESET_INPUT_CLASS = 'fit-preset-input';
  const FIT_PRESET_INTERACTIVE_SELECTOR = '.fit-preset-scroll, .fit-preset-controls, .fit-preset-btn, .fit-preset-edit-btn, .fit-preset-input';
  const FIT_PRESET_DEFAULTS = Object.freeze({
    rpm: Object.freeze([1000, 1500, 2000]),
    noise_db: Object.freeze([25, 30, 35, 40]),
    noise_sone: Object.freeze([1.0, 2.0, 3.0, 4.0])
  });
  // Diagonal pencil SVG icon (Lucide "pencil") used in the preset edit button.
  const FIT_PRESET_EDIT_ICON_SVG = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg>';
  let showFitCurves = true;
  let fitUIInstalled = false;
  const DATA_ZOOM_FIT_REFRESH_IDLE_MS = 120;
  let dataZoomInteractionActive = false;
  let dataZoomFitRefreshTimer = null;

  const xQueryByMode = { rpm: null, noise_db: null, noise_sone: null };
  const fitModelsCache = { rpm: new Map(), noise_db: new Map(), noise_sone: new Map() };
  let noiseSubMode = 'db'; // 'db' | 'sone' — secondary toggle active only when xMode=noise_db
  let fitPresetEditMode = false;
  let fitPresetsByMode = loadFitPresets();

  /** Return the effective x-mode for data display: 'noise_sone' when noise axis + sone submode. */
  function getEffectiveXMode(){
    const xMode = currentXModeFromPayload(lastPayload);
    return (xMode === 'noise_db' && noiseSubMode === 'sone') ? 'noise_sone' : xMode;
  }

  function getFitPresetLimits(mode){
    if (mode === 'noise_sone') {
      return { mode: 'noise_sone', min: 0.1, max: 50, decimals: 1, defaults: [...FIT_PRESET_DEFAULTS.noise_sone] };
    }
    if (mode === 'noise_db') {
      return { mode: 'noise_db', min: 1, max: 99, decimals: 0, defaults: [...FIT_PRESET_DEFAULTS.noise_db] };
    }
    return { mode: 'rpm', min: 600, max: 9999, decimals: 0, defaults: [...FIT_PRESET_DEFAULTS.rpm] };
  }

  function normalizeFitPresetValues(mode, values){
    const { min, max, decimals, defaults } = getFitPresetLimits(mode);
    const src = Array.isArray(values) ? values : defaults;
    return Array.from({ length: defaults.length }, (_, idx) => {
      const raw = Number(src[idx]);
      if (!Number.isFinite(raw)) return defaults[idx];
      const clamped = Math.max(min, Math.min(max, raw));
      return decimals > 0 ? Number(clamped.toFixed(decimals)) : Math.round(clamped);
    });
  }

  function loadFitPresets(){
    const defaults = {
      rpm: [...FIT_PRESET_DEFAULTS.rpm],
      noise_db: [...FIT_PRESET_DEFAULTS.noise_db],
      noise_sone: [...FIT_PRESET_DEFAULTS.noise_sone]
    };
    try {
      const raw = localStorage.getItem(FIT_PRESET_LS_KEY);
      if (!raw) return defaults;
      const parsed = JSON.parse(raw);
      return {
        rpm: normalizeFitPresetValues('rpm', parsed && parsed.rpm),
        noise_db: normalizeFitPresetValues('noise_db', parsed && parsed.noise_db),
        noise_sone: normalizeFitPresetValues('noise_sone', parsed && parsed.noise_sone)
      };
    } catch(_) {
      return defaults;
    }
  ...
  function saveFitPresets(){
    fitPresetsByMode = {
      rpm: normalizeFitPresetValues('rpm', fitPresetsByMode.rpm),
      noise_db: normalizeFitPresetValues('noise_db', fitPresetsByMode.noise_db),
      noise_sone: normalizeFitPresetValues('noise_sone', fitPresetsByMode.noise_sone)
    };
    try { localStorage.setItem(FIT_PRESET_LS_KEY, JSON.stringify(fitPresetsByMode)); } catch(_){}
  }

  function getFitPresetsForMode(mode){
    const key = (mode === 'noise_sone') ? 'noise_sone' : (mode === 'noise_db') ? 'noise_db' : 'rpm';
    return normalizeFitPresetValues(key, fitPresetsByMode[key]);
  }
  ...
  function isFitTextInput(el){
    return !!(el && (
      el.id === 'fitXInput' ||
      el.id === 'fitXInputLegend' ||
      (el.classList && el.classList.contains(FIT_PRESET_INPUT_CLASS))
    ));
  }

  function syncFitNumberInput(input, value, attrs){
    if (!input) return;
    if (attrs && attrs.min != null) input.setAttribute('min', String(attrs.min));
    if (attrs && attrs.max != null) input.setAttribute('max', String(attrs.max));
    if (attrs && attrs.step != null) input.setAttribute('step', String(attrs.step));
    if (document.activeElement === input && isFitTextInput(input)) return;
  ...
  function bindTouchFocusForFitInput(input){
    if (!input || input.__fitTouchFocusBound) return;
    input.addEventListener('touchend', (e) => {
      if (document.activeElement === input) return;
      if (e.cancelable) e.preventDefault();
      try { input.focus(); } catch(_) { warnOnce('[fit] touch focus fallback failed'); }
    }, { passive:false });
    input.__fitTouchFocusBound = true;
  }
  function getById(id){
    if (!id) return null;
    let el = null;
...
function bindGlobalListeners(){
  ...
  (function hookLayoutMovers(){
    ...
    const kickWatch = () => {
      // 拖动期间（例如拖侧栏分隔条）class/style 每帧都在变，逐个 mutation 立即重排
      // 等于一帧多次强制同步布局。此时交给 rootPosWatch 的 rAF 循环每帧做一次即可。
      if (!isChartSidebarDragPerfActive()) watchMovement();
...
function bindChartListeners(){
  ...
  // Reverse hover linkage: hovering a curve in the main chart highlights the
  // owning model in the radar overview.
  _bindReverseHoverListeners(chart);

  chart.on('dataZoom', () => {
    dataZoomInteractionActive = true;
    if (dataZoomFitRefreshTimer) {
      clearTimeout(dataZoomFitRefreshTimer);
      dataZoomFitRefreshTimer = null;
    }
    dataZoomFitRefreshTimer = setTimeout(() => {
      dataZoomInteractionActive = false;
      dataZoomFitRefreshTimer = null;
      if (showFitCurves) {
        layoutScheduler.mark('fitUI');
        refreshFitPanel();
      }
    }, DATA_ZOOM_FIT_REFRESH_IDLE_MS);

    clampXQueryIntoVisibleRange();
    layoutScheduler.mark('pointer');

    if (__skipSpectrumOnce || suppress.active('spectrum')) return;
    SpectrumController.onXQueryChange(xQueryByMode[getEffectiveXMode()]);
  });
}
...
function layoutIsNarrow() {
  // 以图表外层容器宽度判定，避免 rail 自身宽度引起反馈抖动。
...
function mount(rootEl) {
  ...
  if (shell) {
    ...
    // In the current chart layout, #spectrumHost may live beside .chart-upper-row inside the shell.
    if (spectrumRoot && spectrumRoot.parentElement !== stack && spectrumRoot.parentElement !== shell) {
...
function ensureSpectrumHost() {
  ...
  if (!stack && root && root.parentElement) {
    ...
    // Don't move host if already a direct child of the shell (e.g. placed after .chart-upper-row in new layout)
    // Only relocate into stack if spectrum is inside shell but in an unexpected position
    if (stack && host.parentElement !== stack && host.parentElement !== shell) stack.appendChild(host);
...
function render(payload){
  ...
  // Update chart-col layout (width transition + fit-UI suspension)
  updateChartColLayout(nextEmpty);

  requestAnimationFrame(() => {
    updateAxisSwitchPosition({ force:true, animate:false });
    updateNoiseUnitToggleVisibility();
  });
  if (option.__empty) {
  ...
  // When the chart transitions between empty and non-empty, the integrated fit
  // panel visibility must be refreshed immediately (renderLegendRailItems decides
  // whether to render the fit-panel or the basic legend based on sList.length).
  // Without this call the panel only updates on the next ResizeObserver tick.
  if (prevEmpty !== nextEmpty && isIntegratedLegendMode()) {
    try { renderLegendRailItems(); } catch(_) {}
  }
...
function measureText(text, size, weight, family){
  ...
  const X_MIN_CLAMP = 0;
  // Keep baseline overlay sampling visually close to fit dashed lines.
  const BASELINE_SAMPLE_EXTRA = 4;
  const MIN_BASELINE_SAMPLES = 16;
  const MAX_BASELINE_SAMPLES = 60;
  const BASELINE_LINE_WIDTH_NARROW = 1;
  const BASELINE_LINE_WIDTH_NORMAL = 1.25;
  const BASELINE_OVERLAY_COLOR = 'rgba(148, 163, 184, 0.60)';
  const BASELINE_OVERLAY_POINT_CACHE_LIMIT = 12;
  const baselineOverlayPointCache = new Map();
...
function buildSeries(rawSeries, xMode) {
  ...
  const series = (rawSeries || []).map(s => {
    ...
    const soneArr     = Array.isArray(s?.data?.noise_sone) ? s.data.noise_sone : [];
    const flowArr     = Array.isArray(s?.data?.airflow) ? s.data.airflow : [];
    const pressureArr = Array.isArray(s?.data?.pressure_mmh2o) ? s.data.pressure_mmh2o : [];

    const xSrc   = (xMode === 'noise_sone') ? soneArr
                 : (xMode === 'noise_db')   ? noiseArr
                 : rpmArr;
    const tipSrc = (xMode === 'noise_db' || xMode === 'noise_sone') ? rpmArr : noiseArr;
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (Number.isFinite(yv)) {
        if (Number.isFinite(xv) && xv !== X_PLACEHOLDER_NEG) {
          ...
          const pressureRaw = pressureArr[i];
          const pressure = (pressureRaw != null && Number.isFinite(Number(pressureRaw))) ? Number(pressureRaw) : undefined;
          data.push({ value: [xv, yv], tip, pressure });
        } else {
...
function getActiveConditionIds(seriesList) {
  const ids = new Set();
  (seriesList || []).forEach(s => {
    const cidRaw = (s?.conditionId != null) ? s.conditionId : s?.condition_id;
    const cid = Number(cidRaw);
    if (Number.isInteger(cid)) ids.add(cid);
  });
...
function buildBaselineOverlaySeries({ payload, seriesList, xMode, isNarrow }) {
  if (xMode !== 'noise_db') return null;
  const activeConditionIds = getActiveConditionIds(seriesList);
  if (activeConditionIds.length !== 1) return null;

  const activeCid = activeConditionIds[0];
  const overlays = payload?.chartData?.baseline_overlays || {};
  const meta = overlays[String(activeCid)] || null;
  if (!meta || meta.available !== true) return null;
  if (meta.target_chart && meta.target_chart !== 'airflow_vs_db_main') return null;
  if (meta.fit_type && meta.fit_type !== 'exp_ln') return null;

  const params = meta.parameters || {};
  const intercept = Number(params.intercept);
  const slope = Number(params.slope);
  const validMin = Number(meta.valid_db_min);
  const validMax = Number(meta.valid_db_max);

  if (
    !Number.isFinite(intercept)
    || !Number.isFinite(slope)
    || !Number.isFinite(validMin)
    || !Number.isFinite(validMax)
    || !(validMax > validMin)
  ) {
    return null;
  }

  // Use fit-line-like width-driven density to keep interactions smooth.
  const chartWidth = Number(chart?.getWidth?.());
  const widthForSampling = (Number.isFinite(chartWidth) && chartWidth > 0) ? chartWidth : 800;
  const fitLikeCount = computeSampleCount(widthForSampling);
  const sampleCount = Math.max(
    MIN_BASELINE_SAMPLES,
    Math.min(MAX_BASELINE_SAMPLES, fitLikeCount + BASELINE_SAMPLE_EXTRA),
  );
  ...
  let points = baselineOverlayPointCache.get(cacheKey);
  if (points) {
    // Map iteration order is insertion order; delete+set promotes this key to MRU.
    baselineOverlayPointCache.delete(cacheKey);
    baselineOverlayPointCache.set(cacheKey, points);
  }
  if (!points) {
    points = [];
    for (let i = 0; i < sampleCount; i++) {
      const ratio = sampleCount <= 1 ? 0 : i / (sampleCount - 1);
      const db = validMin + (validMax - validMin) * ratio;
      const airflow = Math.exp(intercept + slope * db);
      if (isFinitePositive(airflow)) points.push([db, airflow]);
    }
    if (baselineOverlayPointCache.size >= BASELINE_OVERLAY_POINT_CACHE_LIMIT) {
      const oldestKey = baselineOverlayPointCache.keys().next().value;
      if (oldestKey !== undefined) baselineOverlayPointCache.delete(oldestKey);
    }
    baselineOverlayPointCache.set(cacheKey, points);
  }
  ...
  return {
    id: `baseline-overlay:${activeCid}`,
    name: `baseline-overlay:${activeCid}`,
    type: 'line',
    smooth: false,
    showSymbol: false,
    connectNulls: false,
    ...
    animation: false,
    lineStyle: {
      // Narrow layout uses slightly thinner overlays to reduce visual clutter on smaller viewports.
      width: isNarrow ? BASELINE_LINE_WIDTH_NARROW : BASELINE_LINE_WIDTH_NORMAL,
      type: 'dashed',
      color: BASELINE_OVERLAY_COLOR,
    },
    emphasis: { disabled: true },
    data: points,
    z: 1,
    isBaselineOverlay: true,
    excludeFromFitPanel: true,
    excludeFromInteraction: true,
  };
}
...
function buildOption(payload) {
  ...
  const effMode = getEffectiveXMode(); // 'rpm' | 'noise_db' | 'noise_sone'
  ...
  // 使用预构建快照（effMode 感知）
  const snap = getSnapshotForMode(effMode);
  const built = snap || buildSeries(sList, effMode);

  const xName = effMode==='rpm' ? '转速(RPM)' : effMode==='noise_sone' ? '噪音(sone)' : '噪音(dBA)';
  const titlePrefix = effMode==='rpm' ? '转速' : '噪音';
  const titleTop = 10;
  ...
  const defaultViewXMin = built.xMin;
  const defaultViewXMax = built.xMax;
  const defaultViewYMax = built.yMax;
  let axisXMin = built.xMin;
  let axisXMax = built.xMax;
  let axisYMax = built.yMax;

  const baselineOverlaySeries = buildBaselineOverlaySeries({ payload, seriesList: sList, xMode: effMode, isNarrow });
  if (baselineOverlaySeries) {
    finalSeries.push(baselineOverlaySeries);
  }

  if (showFitCurves) {
    ensureFitModels(sList, effMode);
    ...
    sList.forEach(s => {
      const name = s.name || `${s.brand||''} ${s.model||''} - ${s.condition||''}`;
      const model = (fitModelsCache[effMode] || fitModelsCache['noise_db']).get(name);
  ...
  const rawMin = Math.max(X_MIN_CLAMP, axisXMin);
  const rawMax = axisXMax * 1.2;
  let xMinForAxis = Math.floor(rawMin);
  let xMaxForAxis = Math.ceil(rawMax);
  if (!(xMaxForAxis > xMinForAxis)) xMaxForAxis = xMinForAxis + 1;

  const defaultRawMin = Math.max(X_MIN_CLAMP, defaultViewXMin);
  const defaultRawMax = defaultViewXMax * 1.2;
  let defaultXMinForView = Math.floor(defaultRawMin);
  let defaultXMaxForView = Math.ceil(defaultRawMax);
  if (!(defaultXMaxForView > defaultXMinForView)) defaultXMaxForView = defaultXMinForView + 1;
  ...
  return {
    ...
    yAxis:{
      type:'value', name:'风量(CFM)', min:0, max: axisYMax * 1.3,
    ...
    dataZoom: [
      { type: 'inside', xAxisIndex: 0, throttle: 50, zoomOnMouseWheel: true, moveOnMouseWheel: true, moveOnMouseMove: true ,filterMode: "none", startValue: defaultXMinForView, endValue: defaultXMaxForView * 0.85 },
      { type: 'inside', yAxisIndex: 0, throttle: 50, zoomOnMouseWheel: 'alt', moveOnMouseWheel: 'alt', moveOnMouseMove: true ,filterMode: "none", endValue: defaultViewYMax}
    ],
    series: finalSeries
  };
}
...
function optionTooltipBase(t){
  return {
    ...
    formatter: function(p){
      ...
      const effModeNow = getEffectiveXMode();
      const xLabel = xModeNow==='rpm' ? 'RPM, ' : effModeNow==='noise_sone' ? 'sone, ' : 'dBA, ';
      const infoLabel = xModeNow==='rpm' ? 'dBA' : 'RPM';
      const xRaw = p.value?.[0], y = p.value?.[1];
      const tipRaw = p.data?.tip;
      ...
      const xFmt = effModeNow === 'noise_sone' ? Number(xRaw).toFixed(2)
                 : effModeNow === 'noise_db'   ? Number(xRaw).toFixed(1)
                 : Math.round(xRaw);
      const tipFmt = Number.isFinite(Number(tipRaw))
        ? (xModeNow === 'rpm' ? Number(tipRaw).toFixed(1) : Math.round(Number(tipRaw)))
        : '';
      const pressureRaw = p.data?.pressure;
      const pressureLine = (pressureRaw != null && Number.isFinite(Number(pressureRaw)))
        ? `<br/>&nbsp;&nbsp;&nbsp;&nbsp;最大静压: ${Number(pressureRaw).toFixed(2)}mmH₂O`
        : '';
      return `${dot}${p.seriesName}<br/>&nbsp;&nbsp;&nbsp;&nbsp;${y}CFM @${xFmt}${xLabel}${tipFmt}${infoLabel}${pressureLine}`;
    }
  };
}
...
function bindXAxisSwitch(){
  ...
  function applyType(newType) {
    ...
    pos(normalized, true);
    // Hide sone toggle when switching to RPM axis
    updateNoiseUnitToggleVisibility();
  ...
  /** Create (once) and update the dBA / sone sub-toggle that appears below the noise x-axis. */
  function ensureNoiseUnitToggle(){
    let tog = getById('noiseUnitToggle');
    if (!tog){
      tog = document.createElement('div');
      tog.id = 'noiseUnitToggle';
      tog.setAttribute('aria-label', '噪音单位切换');
      tog.style.cssText = [
        'position:absolute',
        'display:flex',
        'gap:4px',
      ...
      const btnDb   = makePill('dBA', 'db');
      const btnSone = makePill('sone', 'sone');
      tog.appendChild(btnDb);
      tog.appendChild(btnSone);
      appendToRoot(tog);

      function refresh(){
        const isDark = (document.documentElement.getAttribute('data-theme') === 'dark');
        const activeBg  = isDark ? '#4b5563' : '#1e3a5f';
        const activeClr = '#fff';
        const inactiveBg  = 'transparent';
        const inactiveClr = isDark ? '#9ca3af' : '#374151';
        [btnDb, btnSone].forEach(b => {
          const active = (b.dataset.unit === noiseSubMode);
          b.style.background = active ? activeBg : inactiveBg;
          b.style.color = active ? activeClr : inactiveClr;
      ...
      tog.addEventListener('click', (e) => {
        ...
        if (newUnit === noiseSubMode) return;
        noiseSubMode = newUnit;
        refresh();
        stopAllSweepAudio();
        // Reset sone query to mid-range when switching
        if (newUnit === 'sone') xQueryByMode['noise_sone'] = null;
        if (lastPayload) render(lastPayload);
      });

      tog._refresh = refresh;
    }
    const isDark = (document.documentElement.getAttribute('data-theme') === 'dark');
    tog.style.background = isDark ? 'var(--bg-card,#1f2937)' : 'var(--bg-card,#fff)';
    if (tog._refresh) tog._refresh();
    return tog;
  }

  function updateNoiseUnitToggleVisibility(){
    const tog = ensureNoiseUnitToggle();
    const xMode = currentXModeFromPayload(lastPayload);
    const chartEl = chart && chart.getDom ? chart.getDom() : null;
    const isNoise = (xMode === 'noise_db') && !!chartEl && !lastOption?.__empty;
    if (!isNoise){ tog.style.visibility = 'hidden'; return; }

    // Position below the x-axis label center
    try {
      const chartH = chart.getHeight ? chart.getHeight() : 0;
      const chartW = chart.getWidth ? chart.getWidth() : 0;
      const rootRect = getRootRect();
      const grid = lastOption?.grid || {};
      const gridBottom = (typeof grid.bottom === 'number') ? grid.bottom : 40;
      const togW = tog.offsetWidth || 80;
      const togH = tog.offsetHeight || 22;
      const left = Math.round((chartW - togW) / 2);
      const top  = Math.round(chartH - gridBottom + 22);
      tog.style.left = left + 'px';
      tog.style.top  = top + 'px';
...
function ensureFitUI(){
  // Fit toggle button removed; fit is always active
 ...
 if (!bubble){
   ...
   bubble.innerHTML = `
     <div class="head">
       ...
       <div class="fit-preset-scroll">
         <div id="fitPresetControls" class="fit-preset-controls" aria-label="常用拟合坐标"></div>
       </div>
     </div>
  ...
  // Tracks the currently-registered transitionend hide callback so it can be
  // cancelled when setFitBubbleVisible(true) is called, preventing a stale
  // fade-out listener from hiding the bubble after it has been made visible.
  let _fitBubbleOnEnd = null;
...
function bindBubbleDrag(bubble){
  ...
  createDragController(bubble, {
    ...
    onStart: (e) => {
      ...
      if (e.target && e.target.closest(FIT_PRESET_INTERACTIVE_SELECTOR)) return false;
...
// ---- Panel Switcher: slide-based panel state management ----
let __colLayoutInitialized = false;
// Matches the CSS transform transition (0.4s) plus a small buffer
const COL_TRANSITION_MS = 400;
// True while extra panel is active; gates fit-bubble visibility in toggleFitUI
// so any downstream call (render, resize, scroll) cannot re-show the bubble
// while the main chart panel is off-screen.
let __mainColCollapsed = false;
// Timer id for the post-transition resize/restore callback.
let __mainColToggleTimer = null;

/**
 * Apply aria-hidden and inert attributes to the two panels so screen readers
 * and keyboard navigation only reach the active panel.
 * @param {Element} switcher - .chart-panel-switcher element
 * @param {boolean} showExtra - true = extra panel is active, false = main panel is active
 */
function _setSwitcherExtraAriaInert(switcher, showExtra) {
  const mainPanel  = switcher.querySelector('.chart-main-panel');
  const extraPanel = switcher.querySelector('.chart-extra-panel');
  if (mainPanel) {
    mainPanel.setAttribute('aria-hidden', showExtra ? 'true' : 'false');
    if (showExtra) mainPanel.setAttribute('inert', '');
    else           mainPanel.removeAttribute('inert');
  }
  if (extraPanel) {
    extraPanel.setAttribute('aria-hidden', showExtra ? 'false' : 'true');
    if (!showExtra) extraPanel.setAttribute('inert', '');
    else            extraPanel.removeAttribute('inert');
  }
}

/**
 * Central function to switch between main chart panel and extra panel.
 * Manages CSS class, fit UI lifecycle, and post-animation resize.
 * @param {Element} switcher - .chart-panel-switcher element
 * @param {boolean} showExtra - true = show extra panel, false = show main panel
 * @param {boolean} animate - false = suppress transition (initial load)
 */
function _setSwitcherExtra(switcher, showExtra, animate) {
  if (!switcher) return;
  const prev = switcher.classList.contains('show-extra');
  if (prev === showExtra) return;

  if (!animate) {
    switcher.classList.add('no-panel-transition');
    void switcher.offsetWidth; // force reflow
    switcher.classList.toggle('show-extra', showExtra);
    void switcher.offsetWidth;
    setTimeout(() => switcher.classList.remove('no-panel-transition'), 50);
  } else {
    switcher.classList.toggle('show-extra', showExtra);
  }

  _updateChartPanelArrowAria(switcher);

  // Hide the off-screen panel from assistive technology and prevent keyboard
  // focus from reaching it. In desktop slide mode the panel is off-screen but
  // still in the DOM; without this, screen readers and keyboard navigation
  // would reach elements that are visually hidden.
  _setSwitcherExtraAriaInert(switcher, showExtra);

  // Cancel any pending restore timer
  if (__mainColToggleTimer !== null) {
    clearTimeout(__mainColToggleTimer);
    __mainColToggleTimer = null;
  }

  if (showExtra) {
    // Moving away from main chart: immediately hide fit UI
    _suspendFitUI();
  } else {
    // Returning to main chart: keep the guard active during the slide animation,
    // then resize + restore only after the transition has fully completed.
    // __mainColCollapsed intentionally stays true until the timer fires so that
    // any toggleFitUI() calls during the animation (e.g. from render()) cannot
    // show the fit bubble while the panel is still sliding into view.
    __mainColToggleTimer = setTimeout(() => {
      __mainColToggleTimer = null;
      __mainColCollapsed = false;
      try {
        if (chart) chart.resize();
        _restoreFitUI();
        placeFitUI();
...
function updateChartColLayout(nextEmpty) {
  const upperRow = document.querySelector('.chart-upper-row');
  const switcher = document.querySelector('.chart-panel-switcher');
  if (!upperRow) return;

  const prevHasNoCurves = upperRow.classList.contains('has-no-curves');
  const nextHasNoCurves = !!nextEmpty;

  if (!__colLayoutInitialized) {
    // On first render: apply state immediately without animation
    __colLayoutInitialized = true;
    upperRow.classList.toggle('has-no-curves', nextHasNoCurves);
    if (nextHasNoCurves && switcher) {
      _setSwitcherExtra(switcher, true, false);
    } else if (switcher) {
      // Curves exist: main panel is already active; set initial aria state
      _setSwitcherExtraAriaInert(switcher, false);
    }
    return;
  }

  if (prevHasNoCurves === nextHasNoCurves) return;

  upperRow.classList.toggle('has-no-curves', nextHasNoCurves);

  if (nextHasNoCurves) {
    // Curves gone: slide to extra panel
    if (switcher) _setSwitcherExtra(switcher, true, true);
  } else {
    // Curves returned: slide back to main panel
    if (switcher) _setSwitcherExtra(switcher, false, true);
  }
}

// ---- Desktop panel arrow button ----

function _updateChartPanelArrowAria(switcher) {
  const sw = switcher || document.querySelector('.chart-panel-switcher');
  const btn = sw && sw.querySelector('.chart-panel-arrow');
  if (!btn) return;
  const showingExtra = sw.classList.contains('show-extra');
  btn.setAttribute('aria-expanded', showingExtra ? 'true' : 'false');
  btn.setAttribute('aria-label', showingExtra ? '切换回主图' : '切换到点评/图库');
}

function _bindChartPanelArrow() {
  const btn = document.querySelector('.chart-panel-arrow');
  if (!btn || btn._bound) return;
  btn._bound = true;
  btn.addEventListener('click', () => {
    const upperRow = document.querySelector('.chart-upper-row');
    const switcher = document.querySelector('.chart-panel-switcher');
    if (!upperRow || !switcher) return;
    // Ignore clicks when no curves (button is hidden by CSS anyway, but guard in JS)
    if (upperRow.classList.contains('has-no-curves')) return;
    const willShowExtra = !switcher.classList.contains('show-extra');
    _setSwitcherExtra(switcher, willShowExtra, true);
  });
}

/** Slide back to main panel (called from fancool.js on radar label click). */
function expandMainChart() {
  const switcher = document.querySelector('.chart-panel-switcher');
  if (!switcher) return;
  if (!switcher.classList.contains('show-extra')) return;
  _setSwitcherExtra(switcher, false, true);
}

function _suspendFitUI() {
  // Set the module-level guard so toggleFitUI cannot re-show the bubble via
  // any downstream call (render, resize, layoutScheduler, scroll) while the
  // main chart column is collapsed.
  __mainColCollapsed = true;
  // Immediately hide bubble and pointer via the canonical helper.
  setFitBubbleVisible(false, { immediate: true });
  const ptr = document.getElementById('fitPointer');
  if (ptr) ptr.style.visibility = 'hidden';
}

function _restoreFitUI() {
  // showFitCurves is the module-level toggle flag controlling fit-curve display
  toggleFitUI(showFitCurves);
}
...
function toggleFitUI(showFit){
  ...
  // 集成模式（窄屏）：气泡隐藏，图例 rail 中显示；浮动气泡仅在非集成模式下显示
  // Also suppress the bubble whenever the extra panel is active (tracked by
  // __mainColCollapsed): any downstream caller (render, resize, scroll) must
  // not re-show the overlay while the main chart panel is off-screen.
  const showFloating = showFit && !empty && !integrated && !__mainColCollapsed;
  setFitBubbleVisible(showFloating);
  if (ptr) ptr.style.visibility = (showFit && !empty && !__mainColCollapsed) ? 'visible' : 'hidden';

  // When the chart is empty, always clear the integrated fit grid so stale values vanish.
  if (empty && integrated) {
...
function placeFitUI(){
  ...
  if (btns && __legendActionsEl) {
    ...
    btns.style.visibility = (lastOption.__empty || __mainColCollapsed) ? 'hidden' : 'visible';
  }

  const unitStr = (currentXModeFromPayload(lastPayload) === 'rpm') ? 'RPM' : (getEffectiveXMode() === 'noise_sone') ? 'sone' : 'dBA';
  ...
  if (!integrated && bubble){
    ...
    const showFloating = showFitCurves && !lastOption.__empty && !__mainColCollapsed;
...
function bindPointerDrag(){
  ...
  let baseX = 0;
  let dragGrid = null; // 拖拽期间图表尺寸不变，onStart 量一次即可
  ...
  createDragController(handle, {
    ...
    onStart: (e) => {
      ...
      dragGrid = measureGrid();
      handle.style.cursor = 'grabbing';
    },
    onMove: ({ dx }) => {
      const grid = dragGrid || measureGrid();
  ...
  function repaintPointer(){
    ...
    // 窄屏也显示拟合指针
    if (!showFitCurves || lastOption.__empty || __mainColCollapsed) { ptr.style.visibility = 'hidden'; return; }
    ...
    syncBubbleInput();
    if (showFitCurves && !dataZoomInteractionActive) refreshFitPanel();
  }

  function setFitXInputLiveValue(mode, value){
  ...
  function setFitXQueryAndRefresh(mode, value){
    const raw = Number(value);
    if (!Number.isFinite(raw)) return false;
    xQueryByMode[mode] = clampXDomain(raw);
    clampXQueryIntoVisibleRange(mode);
    stopAllSweepAudio();
    repaintPointer();
    refreshFitPanel();
    SpectrumController.onXQueryChange(xQueryByMode[mode]);
  ...
  function focusActiveFitPresetEditor(){
    const host = isIntegratedLegendMode() ? getById('fitPresetControlsLegend') : getById('fitPresetControls');
    const firstInput = host ? host.querySelector('.fit-preset-input') : null;
  ...
  function commitFitPresetEdits(host){
    const mode = getEffectiveXMode();
    const current = getFitPresetsForMode(mode);
    const inputs = host ? Array.from(host.querySelectorAll('.fit-preset-input')) : [];
    fitPresetsByMode[mode] = normalizeFitPresetValues(mode, current.map((value, idx) => {
      const raw = Number(inputs[idx] ? inputs[idx].value : value);
      return Number.isFinite(raw) ? raw : value;
    }));
    saveFitPresets();
    fitPresetEditMode = false;
    renderFitPresetControls();
  }

  function renderFitPresetHost(host, mode){
    if (!host) return;
    const presets = getFitPresetsForMode(mode);
    const { min, max, decimals } = getFitPresetLimits(mode);
    const stepStr = decimals > 0 ? String(Math.pow(10, -decimals).toFixed(decimals)) : '1';

    // Preserve draft edits across bubble/integrated hosts, but avoid leaking drafts across axis modes.
    const modeStr = String(mode || 'rpm');
    const prevMode = host.getAttribute('data-fit-preset-mode');
    let draftValues = [];
    if (fitPresetEditMode && prevMode === modeStr) {
      draftValues = Array.from(host.querySelectorAll('.fit-preset-input')).map((input) => input.value);
    }
    if (fitPresetEditMode && draftValues.length !== presets.length) {
      const otherHost = (host.id === 'fitPresetControls')
        ? getById('fitPresetControlsLegend')
        : getById('fitPresetControls');
      if (otherHost && otherHost.getAttribute('data-fit-preset-mode') === modeStr) {
        draftValues = Array.from(otherHost.querySelectorAll('.fit-preset-input')).map((input) => input.value);
      }
    }
    host.setAttribute('data-fit-preset-mode', modeStr);
    host.classList.toggle('is-editing', fitPresetEditMode);
    const activeEl = document.activeElement;
    const activeEditInput = !!(fitPresetEditMode && activeEl && activeEl.classList && activeEl.classList.contains('fit-preset-input') && host.contains(activeEl));
    if (fitPresetEditMode) {
      const inputValues = (draftValues.length === presets.length) ? draftValues : presets.map((value) => String(value));
      const renderState = `edit:${modeStr}:${inputValues.join('|')}`;
      if (activeEditInput && prevMode === modeStr) return;
      if (host.__fitPresetRenderState === renderState) return;
      host.__fitPresetRenderState = renderState;
      host.innerHTML = `
        ${presets.map((value, idx) => `
          <input
            class="fit-preset-input"
            type="number"
            inputmode="numeric"
            step="${stepStr}"
            min="${min}"
            max="${max}"
            value="${escapeHtml(inputValues[idx] ?? value)}"
            data-fit-preset-input="${idx}"
            aria-label="常用拟合坐标 ${idx + 1}"
          />
        `).join('')}
        <button type="button" class="fit-preset-edit-btn is-editing" aria-label="确认常用拟合坐标">✓</button>
      `;
      const confirmBtn = host.querySelector('.fit-preset-edit-btn');
      if (confirmBtn) {
        confirmBtn.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          commitFitPresetEdits(host);
        });
      }
      host.querySelectorAll('.fit-preset-input').forEach((input) => {
        ...
        bindTouchFocusForFitInput(input);
      });
      return;
    }

    const renderState = `view:${modeStr}:${presets.join('|')}`;
    if (host.__fitPresetRenderState === renderState) return;
    host.__fitPresetRenderState = renderState;
    const fmtPreset = (v) => decimals > 0 ? Number(v).toFixed(decimals) : String(Math.round(v));
    host.innerHTML = `
      ${presets.map((value) => `
        <button type="button" class="fit-preset-btn" data-fit-preset-value="${escapeHtml(value)}">${escapeHtml(fmtPreset(value))}</button>
      `).join('')}
      <button type="button" class="fit-preset-edit-btn" aria-label="编辑常用拟合坐标">${FIT_PRESET_EDIT_ICON_SVG}</button>
    `;
    host.querySelectorAll('.fit-preset-btn[data-fit-preset-value]').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        setFitXQueryAndRefresh(mode, Number(btn.getAttribute('data-fit-preset-value')));
      });
    });
    const editBtn = host.querySelector('.fit-preset-edit-btn');
    if (editBtn) {
      editBtn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        fitPresetEditMode = true;
        renderFitPresetControls();
        focusActiveFitPresetEditor();
      });
    }
  }

  function renderFitPresetControls(){
    const mode = getEffectiveXMode();
    const activeEl = document.activeElement;
    if (fitPresetEditMode && activeEl?.classList?.contains(FIT_PRESET_INPUT_CLASS)) {
      const activeHost = activeEl.closest('#fitPresetControls, #fitPresetControlsLegend');
      const activeMode = activeHost ? activeHost.getAttribute('data-fit-preset-mode') : null;
      if (activeHost && activeMode === String(mode || DEFAULT_FIT_MODE)) {
        return;
      }
    }
    renderFitPresetHost(getById('fitPresetControls'), mode);
    renderFitPresetHost(getById('fitPresetControlsLegend'), mode);
  }
  ...
  function syncBubbleInput(){
    ...
    const isNoiseLike = (mode === 'noise_db' || mode === 'noise_sone');
    const rounded = isNoiseLike ? Number(val.toFixed(mode === 'noise_sone' ? 2 : 1)) : Math.round(val);
    const [vx0, vx1] = getVisibleXRange();
    syncFitNumberInput(inp, rounded, {
      min: Math.floor(vx0),
      max: Math.ceil(vx1),
      step: (mode === 'noise_sone') ? '0.01' : (mode === 'noise_db') ? '0.1' : '1'
    });
  }
...
function onBubbleInputLive(){
  ...
  if (!inp) return;
  const mode = getEffectiveXMode();
  setFitXInputLiveValue(mode, inp.value);
...
function onBubbleInputCommit(){
  ...
  if (!inp) return;
  const mode = getEffectiveXMode();
  if (!setFitXQueryAndRefresh(mode, inp.value)) {
    syncBubbleInput();
    if (showFitCurves) refreshFitPanel();
    return;
  }
  const committed = Number(xQueryByMode[mode]);
  const isNoiseLike = (mode === 'noise_db' || mode === 'noise_sone');
  const roundedCommitted = isNoiseLike ? Number(committed.toFixed(mode === 'noise_sone' ? 2 : 1)) : Math.round(committed);
  inp.value = String(roundedCommitted);
}
...
function ensureFitModels(sList, xMode){
  ...
  sList.forEach(s => {
    const ph = s && s.pchip
      ? (xMode === 'noise_sone' ? s.pchip.sone_to_airflow
         : xMode === 'noise_db' ? s.pchip.noise_to_airflow
         : s.pchip.rpm_to_airflow)
      : null;
...
function evalPchipJS(model, x){
  ...
  let __dragVisibleXRangeCache = null;
  function getVisibleXRange(){
    if (!lastOption) return [0,1];
    // 拖拽期间 dataZoom 不会变，缓存以避开每帧一次 chart.getOption() 深拷贝
    if (isChartSidebarDragPerfActive() && __dragVisibleXRangeCache) return __dragVisibleXRangeCache;
    const xAxis = lastOption.xAxis || {};
    const xmin = Number(xAxis.min); const xmax = Number(xAxis.max);
    ...
    const range = [vmin, vmax];
    if (isChartSidebarDragPerfActive()) __dragVisibleXRangeCache = range;
    return range;
  }
  window.addEventListener(CHART_SIDEBAR_DRAG_PERF_END_EVENT, () => { __dragVisibleXRangeCache = null; });

  function clampXQueryIntoVisibleRange(modeKey){
    const mode = modeKey || getEffectiveXMode();
  ...
  function ensureSpectrumChart() {
    ...
    // Reverse hover linkage: hovering a spectrum curve highlights the owning model in radar
    _bindReverseHoverListeners(spectrumChart);
  }
...
async function toggleSpectrumUI(show) {
  ...
  if (show) {
    ...
    if (isFs) {
      ...
      requestAndRenderSpectrum(true).catch(()=>{}).then(() => {
...
function getSeriesRpmForCurrentX(series, spectrumModel){
  const effMode = getEffectiveXMode();
  if (effMode === 'rpm') {
    const rpm = Number(getXQueryOrDefault('rpm'));
    return Number.isFinite(rpm) && rpm > 0 ? rpm : 0;
  } else if (effMode === 'noise_sone') {
    const soneX = Number(getXQueryOrDefault('noise_sone'));
    const ph = series?.pchip?.sone_to_rpm;
    if (ph && Array.isArray(ph.x) && Array.isArray(ph.y) && Array.isArray(ph.m)) {
      const rpm = Number(evalPchipJS(ph, soneX));
      return Number.isFinite(rpm) && rpm > 0 ? rpm : 0;
    }
...
async function requestAndRenderSpectrum(forceFull = false) {
  ...
  try {
    ...
    buildAndSetSpectrumOption(forceFull);
    try { if (showFitCurves) refreshFitPanel(); } catch (_) {}
...
let __dragLegendSelCache = null;
window.addEventListener(CHART_SIDEBAR_DRAG_PERF_END_EVENT, () => { __dragLegendSelCache = null; });
function getLegendSelectionMap(){
  try {
    // 拖拽期间图例选中不会变，缓存以避开 chart.getOption() 的整份 option 深拷贝
    if (isChartSidebarDragPerfActive() && __dragLegendSelCache) return __dragLegendSelCache;
    const opt = chart.getOption() || {};
    const sel = (opt.legend && opt.legend[0] && opt.legend[0].selected) || {};
    if (isChartSidebarDragPerfActive()) __dragLegendSelCache = sel;
    return sel;
  } catch(_) { return {}; }
}
...
function buildAndSetSpectrumOption(fullRefresh = false, themeOverride) {
  ...
  spectrumChart.setOption(optionObj, true);
  ...
  return {
    ...
    formatter: function (params) {
      ...
      const linesHtml = params.map(p => {
        ...
        const dB = Number.isFinite(v) ? v.toFixed(1) + ' dBA' : '-';
...
function buildSpectrumTitle() {
  const effMode = getEffectiveXMode();
  const x = Number(getXQueryOrDefault(effMode));
  ...
  if (effMode === 'rpm') {
    ...
    const v = Number.isFinite(x) ? x.toFixed(2) : '-';
    return `${resLabel}A计权声级频谱 @ ${v} sone`;
  } else {
...
function isIntegratedLegendMode() {
  // On narrow screens the legend rail is embedded inside the chart container
  return layoutIsNarrow();
}
...
const layoutScheduler = (() => {
  ...
  function flush() {
    rafId = null;
    try {
      if (dirty.has('legend')) { ensureLegendRail(); updateLegendRailLayout(); renderLegendRailItems(); }
...
function createDragController(element, opts) {
  ...
  let moved = false;
  // Perf: pointermove 触发频率可以远高于刷新率（高刷/合并事件关闭时更甚），
  // 而 onMove 里会写样式并读布局，逐个事件执行等于每帧多次强制同步重排。
  // 这里把移动合并到一帧一次，只处理最新一次指针位置。
  ...
  function flushMove() {
    ...
    pendingMove = null;
    if (!m) return;
    if (typeof onMove === 'function') onMove(m);
  }
  ...
  function onPointerUpOrCancel() {
    ...
    moved = false;
    // 松手前把最后一次合并掉的移动补上，保证终点位置精确
    cancelPendingMove();
    flushMove();
    if (wasMoved) setDragInteractionActive(false);
...
function getTargetRpmForRow(row, mode) {
  ...
  } else if (mode === 'noise_sone') {
    // Sone noise mode: use sone_to_rpm pchip for spectrum lookup
    const soneQuery = Number(getXQueryOrDefault('noise_sone'));
    const pchipModel = series?.pchip?.sone_to_rpm;
    const domain = getPchipDomainRange(pchipModel);
    if (domain && Number.isFinite(soneQuery)) {
      const clamped = Math.max(domain.min, Math.min(domain.max, soneQuery));
      targetRpm = evalPchipJS(pchipModel, clamped);
    } else {
...
function computeFitRows(mode, x, seriesList, selectionMap) {
  ...
  seriesList.forEach(s => {
    ...
    const crossModel = (s && s.pchip)
      ? (mode === 'noise_sone' ? s.pchip?.sone_to_rpm
         : mode === 'rpm' ? s.pchip?.rpm_to_noise_db
         : s.pchip?.noise_to_rpm)
      : null;
    ...
    // Sone mode fallback: sone→rpm→airflow when sone_to_airflow PCHIP is absent
    if (!Number.isFinite(y) && mode === 'noise_sone' && s && s.pchip) {
      const soneToRpmM = s.pchip.sone_to_rpm;
      const rpmToAirM  = s.pchip.rpm_to_airflow;
      if (soneToRpmM && soneToRpmM.x0 != null && rpmToAirM && rpmToAirM.x0 != null) {
        const s0 = Math.min(soneToRpmM.x0, soneToRpmM.x1);
        const s1 = Math.max(soneToRpmM.x0, soneToRpmM.x1);
        const rpm = evalPchipJS(soneToRpmM, Math.max(s0, Math.min(x, s1)));
        if (Number.isFinite(rpm)) {
          const r0 = Math.min(rpmToAirM.x0, rpmToAirM.x1);
          const r1 = Math.max(rpmToAirM.x0, rpmToAirM.x1);
          y = evalPchipJS(rpmToAirM, Math.max(r0, Math.min(rpm, r1)));
          outOfDomain = x < s0 || x > s1;
        }
      }
    }
...
function buildDerivedSnapshots(seriesCanonical){
  ...
  snapshotCache.noise_sone = buildSeries(seriesCanonical, 'noise_sone');
}
...
function setFitBubbleVisible(visible, { immediate = false } = {}) {
  ...
  // Cancel any pending fade-out transitionend listener to prevent a stale callback
  // from hiding the bubble after it has been made visible again.  This fixes the
  // race where: (1) setFitBubbleVisible(false) registers onEnd, (2) chart becomes
  // non-empty and setFitBubbleVisible(true) shows the bubble, (3) the opacity
  // transition from 0→1 fires transitionend and the stale onEnd sets
  // visibility:'hidden', making the bubble disappear.
  if (_fitBubbleOnEnd) {
    try { bubble.removeEventListener('transitionend', _fitBubbleOnEnd); } catch (_) {}
    _fitBubbleOnEnd = null;
  }
  ...
  const thisOnEnd = (e) => {
    ...
    // Only act if this is still the registered callback (not superseded by a newer call).
    if (_fitBubbleOnEnd === thisOnEnd) {
      try { bubble.removeEventListener('transitionend', thisOnEnd); } catch (_) {}
      _fitBubbleOnEnd = null;
      bubble.style.visibility = 'hidden';
    }
  };
  _fitBubbleOnEnd = thisOnEnd;

  try {
    bubble.addEventListener('transitionend', thisOnEnd);
  } catch (_) {
...
let __dragSelMapCache = null;
window.addEventListener(CHART_SIDEBAR_DRAG_PERF_END_EVENT, () => { __dragSelMapCache = null; });

function refreshFitPanel() {  const integrated = isIntegratedLegendMode();
  const sList = getSeriesArray();
  ...
  // chart.getOption() 会深拷贝整个 option（含全部 series 数据），逐帧调用非常贵。
  // 拖拽期间图例选中状态不会变，缓存一次即可。
  let selMap;
  if (isChartSidebarDragPerfActive()) {
    if (!__dragSelMapCache) {
      const opt = chart.getOption() || {};
      __dragSelMapCache = (opt.legend && opt.legend[0] && opt.legend[0].selected) || {};
    }
    selMap = __dragSelMapCache;
  } else {
    __dragSelMapCache = null;
    const opt = chart.getOption() || {};
    selMap = (opt.legend && opt.legend[0] && opt.legend[0].selected) || {};
  }
  ...
  const unit = (mode === 'rpm') ? 'RPM' : (mode === 'noise_sone') ? 'sone' : 'dBA';
  renderFitPanel({
...
/**
 * 拖拽期间的"只更新数值"快路径。
 * 完整渲染会重建整段 innerHTML 并重新绑定每行事件，逐帧执行代价极高；
 * 拖动时行集合不变，只有数值在变，因此复用已有行节点、冻结行序，
 * 仅改写 .col-val / .col-pct / .col-cross 与状态 class。
 * 行集合一旦对不上就返回 false，交回完整渲染。
 */
function fitRowsFastUpdate(containerEl, rowSelector, rows, unit) {
  if (!containerEl || !Array.isArray(rows) || !rows.length) return false;
  const nodes = containerEl.querySelectorAll(rowSelector + '[data-name]');
  if (!nodes.length || nodes.length !== rows.length) return false;

  const byName = new Map();
  rows.forEach(r => { if (r && r.name != null) byName.set(String(r.name), r); });
  if (byName.size !== rows.length) return false;
  for (const node of nodes) {
    if (!byName.has(node.getAttribute('data-name') || '')) return false;
  }

  const withVal = rows.filter(r => r.selected && Number.isFinite(r.y)).sort((a, b) => b.y - a.y);
  const base = (withVal.length && withVal[0].y > 0) ? withVal[0].y : 0;

  nodes.forEach(node => {
    const it = byName.get(node.getAttribute('data-name') || '');
    if (!it) return;
    const hasY = Number.isFinite(it.y);
    const valText = hasY ? `${Math.round(it.y)} CFM` : '—';
    const pct = (hasY && base > 0) ? Math.round((it.y / base) * 100) : null;
    const pctText = (pct == null) ? '—' : `(${pct}%)`;

    const valEl = node.querySelector('.col-val');
    if (valEl && valEl.textContent !== valText) valEl.textContent = valText;
    const pctEl = node.querySelector('.col-pct');
    if (pctEl && pctEl.textContent !== pctText) pctEl.textContent = pctText;

    const crossEl = node.querySelector('.col-cross');
    if (crossEl) {
      if (Number.isFinite(it.cross)) {
        const num = (unit === 'RPM') ? Number(it.cross).toFixed(1) : String(Math.round(it.cross));
        const suffix = (unit === 'RPM') ? ' dBA' : ' RPM';
        let numEl = crossEl.firstElementChild;
        if (!numEl) {
          crossEl.textContent = '';
          numEl = document.createElement('span');
          numEl.style.fontVariantNumeric = 'tabular-nums';
          crossEl.appendChild(numEl);
          crossEl.appendChild(document.createTextNode(suffix));
        }
        if (numEl.textContent !== num) numEl.textContent = num;
        const tail = crossEl.lastChild;
        if (tail && tail.nodeType === 3 && tail.nodeValue !== suffix) tail.nodeValue = suffix;
      } else if (crossEl.textContent !== '—') {
        crossEl.textContent = '—';
      }
    }

    node.classList.toggle('is-off', !it.selected);
    node.classList.toggle('is-out-of-domain', !!it.outOfDomain);
  });
  return true;
}

/** 拖拽期间只把 x 输入框的显示值同步过去，不触碰 min/max（避免 chart.getOption()）。 */
function syncFitXInputValueOnly(input) {
  if (!input) return;
  const modeKey = getEffectiveXMode();
  const val = Number(xQueryByMode[modeKey]);
  if (!Number.isFinite(val)) return;
  const rounded = (modeKey === 'noise_sone') ? Number(val.toFixed(2))
                : (modeKey === 'noise_db')   ? Number(val.toFixed(1))
                : Math.round(val);
  syncFitNumberInput(input, rounded, null);
}
...
function renderFitPanel({ mode, rows, xValue, unit, onToggleSeries }) {
      ...
      btn.onclick = async (e) => {
        ...
        try {
          ...
          if (isPlayClick && window.Analytics && window.Analytics.logPlayAudio) {
            try {
              const xMode = currentXModeFromPayload(lastPayload);
              const effM = getEffectiveXMode();
              const pointerX = xQueryByMode[effM];
              const dbVal = (xMode === 'noise_db')
                ? (effM === 'noise_sone' ? null : pointerX)
  ...
  if (mode === 'integrated') {
    ...
    // 拖拽期间走只更新数值的快路径，行序冻结、事件不重绑
    if (isChartSidebarDragPerfActive() && fitRowsFastUpdate(grid, '.legend-row', rows, unit)) {
      syncFitXInputValueOnly(getById('fitXInputLegend'));
      return;
    }
    ...
    grid.innerHTML = ordered.map(it => {
      ...
      if (Number.isFinite(it.cross)) {
        crossText = (unit === 'RPM')
          ? `<span style="font-variant-numeric:tabular-nums;">${Number(it.cross).toFixed(1)}</span> dBA`
    ...
    grid.querySelectorAll('.legend-row[data-name]').forEach(node => {
      ...
      node.addEventListener('mouseenter', () => {
        ...
        try {
          const mid = _getModelIdForSeriesName(name);
          if (mid != null && window.RadarOverview && typeof window.RadarOverview.reverseHoverModel === 'function') {
            window.RadarOverview.reverseHoverModel(mid);
          }
        } catch(_){}
      }, { passive:true });
      node.addEventListener('mouseleave', () => {
        ...
        try {
          if (window.RadarOverview && typeof window.RadarOverview.reverseHoverClear === 'function') {
            window.RadarOverview.reverseHoverClear();
          }
        } catch(_){}
      }, { passive:true });
    });
    ...
    if (inp) {
      ...
      const isSone = (modeKey === 'noise_sone');
      const rounded = (modeKey === 'noise_sone') ? Number(val.toFixed(2)) : (modeKey === 'noise_db') ? Number(val.toFixed(1)) : Math.round(val);
      const [vx0, vx1] = getVisibleXRange();
      syncFitNumberInput(inp, rounded, {
        min: Math.floor(vx0),
        max: Math.ceil(vx1),
        step: isSone ? '0.01' : (modeKey === 'noise_db') ? '0.1' : '1'
      });

      // 统一与原行为一致
      // oninput: Allow free typing without immediate clamping
      inp.oninput = () => {
        setFitXInputLiveValue(modeKey, inp.value);
        // No UI updates during typing - happens on commit
      };
      // onchange: Clamp and update on commit (blur/Enter)
      const commitLegendInput = () => {
        if (!setFitXQueryAndRefresh(modeKey, inp.value)) {
          refreshFitPanel();
          return;
        }
        const committed = Number(xQueryByMode[modeKey]);
        const roundedCommitted = (modeKey === 'noise_sone') ? Number(committed.toFixed(2)) : (modeKey === 'noise_db') ? Number(committed.toFixed(1)) : Math.round(committed);
        inp.value = String(roundedCommitted);
      };
      inp.onchange = commitLegendInput;
      inp.onkeydown = (e) => {
        if (e.key === 'Enter') {
          e.preventDefault();
          commitLegendInput();
  ...
  // 拖拽期间走只更新数值的快路径，行序冻结、事件不重绑
  if (isChartSidebarDragPerfActive() && fitRowsFastUpdate(rowsEl, '.row', rowsSafe, unit)) {
    syncFitXInputValueOnly(bubble.querySelector('#fitXInput'));
    return;
  }
  ...
  if (inp) {
    ...
    const val = Number(xQueryByMode[modeKey] ?? xValue);
    const isSone = (modeKey === 'noise_sone');
    const rounded = isSone ? Number(val.toFixed(2)) : (modeKey === 'noise_db') ? Number(val.toFixed(1)) : Math.round(val);
    const [vx0, vx1] = getVisibleXRange();
    syncFitNumberInput(inp, rounded, {
      min: Math.floor(vx0),
      max: Math.ceil(vx1),
...
/** Returns the display name for a series object (mirrors buildSeries naming). */
function _getSeriesName(s) {
  return s.name || `${s.brand || ''} ${s.model || ''} - ${s.condition || ''}`;
}

/**
 * Look up the numeric modelId for a rendered series name.
 * Returns null if no matching canonical series is found.
 * @param {string} name  ECharts series name
 * @returns {number|null}
 */
function _getModelIdForSeriesName(name) {
  const s = getSeriesArray().find(s => _getSeriesName(s) === name);
  if (!s) return null;
  const mid = s.modelId != null ? s.modelId : s.model_id;
  return mid != null ? Number(mid) : null;
}

/**
 * Bind reverse hover listeners on an ECharts instance so that hovering any
 * series curve highlights the owning model in the radar overview.
 * Uses `mouseover` (per-series) to update and `globalout` (mouse leaves chart)
 * to clear, avoiding flicker when moving between adjacent data points.
 * @param {object} ecInstance  ECharts chart or spectrumChart instance
 */
function _bindReverseHoverListeners(ecInstance) {
  if (!ecInstance) return;
  ecInstance.on('mouseover', (p) => {
    if (!p || !p.seriesName) return;
    const mid = _getModelIdForSeriesName(p.seriesName);
    if (mid == null) return;
    if (window.RadarOverview && typeof window.RadarOverview.reverseHoverModel === 'function') {
      window.RadarOverview.reverseHoverModel(mid);
    }
  });
  ecInstance.on('globalout', () => {
    if (window.RadarOverview && typeof window.RadarOverview.reverseHoverClear === 'function') {
      window.RadarOverview.reverseHoverClear();
    }
  });
...
/**
 * Collect the rendered series indices from an ECharts instance whose names
 * are in the given Set.  Including fit-curve series that share the same name
 * as their raw-data counterpart.
 * @param {object} ecInstance  ECharts instance (chart or spectrumChart)
 * @param {Set<string>} nameSet  Set of series names to match
 * @returns {number[]}  Series indices in the rendered option
 */
function _resolveSeriesIndices(ecInstance, nameSet) {
  try {
    const rendered = ecInstance.getOption().series || [];
    const indices = [];
    rendered.forEach((s, i) => { if (nameSet.has(s.name)) indices.push(i); });
    return indices;
  } catch (_) { return []; }
}

/**
 * Highlight all currently visible series belonging to modelId in both main
 * chart and spectrum chart.
 *
 * Root-cause note: ECharts processes `batch` highlight items one-by-one.
 * With `focus:'series'` enabled, each item blurs all others, so only the
 * LAST item remains highlighted.  The fix is to pass `seriesIndex` as an
 * array – ECharts then applies emphasis to all targets simultaneously.
 * Fit-curve series share the same name as raw-data series, so resolving
 * indices from the actual rendered option automatically includes them.
 *
 * Debug: open DevTools console, run `window.FC_DEBUG_RADAR_HIGHLIGHT = true`,
 * then hover a radar legend item.  Look for console groups labelled
 * "[RadarHighlight] highlightModelSeries(…)".
 *
 * @param {string|number} modelId
 */
function highlightModelSeries(modelId) {
  if (!chart) return;
  const mid = Number(modelId);
  const selMap = getLegendSelectionMap();

  // Gather all canonical candidates for this model
  const allCandidates = [];
  getSeriesArray().forEach(s => {
    const sMid = Number((s.modelId != null) ? s.modelId : s.model_id);
    if (sMid !== mid) return;
    const sCid = Number((s.conditionId != null) ? s.conditionId : s.condition_id);
    allCandidates.push({ modelId: sMid, conditionId: sCid, name: _getSeriesName(s) });
  });

  // Filter by legend visibility
  const visibleNames = allCandidates
    .filter(c => !(selMap && selMap[c.name] === false))
    .map(c => c.name);
  const hiddenNames = allCandidates
    .filter(c => selMap && selMap[c.name] === false)
    .map(c => c.name);

  // Resolve actual rendered series indices (includes fit-curve duplicates)
  const nameSet = new Set(visibleNames);
  const mainIndices = _resolveSeriesIndices(chart, nameSet);
  const specIndices = (spectrumEnabled && spectrumChart)
    ? _resolveSeriesIndices(spectrumChart, nameSet)
    : [];

  // -- Targeted debug logging (opt-in: set window.FC_DEBUG_RADAR_HIGHLIGHT=true in DevTools) --
  if (window.FC_DEBUG_RADAR_HIGHLIGHT) {
    console.group(`[RadarHighlight] highlightModelSeries(modelId=${mid})`);
    console.log('canonical candidates:', allCandidates);
    console.log('legend-hidden (skipped):', hiddenNames);
    console.log('visible names:', visibleNames);
    console.log('main chart → seriesIndex:', mainIndices);
    if (spectrumEnabled && spectrumChart) {
      console.log('spectrum chart → seriesIndex:', specIndices);
    }
    console.groupEnd();
  }
  ...
  // Use seriesIndex array: ECharts highlights all indices simultaneously,
  // avoiding the sequential-blur side-effect of batch dispatch.
  if (mainIndices.length) {
    try { chart.dispatchAction({ type: 'highlight', seriesIndex: mainIndices }); } catch (_) {}
  } else {
    // Fallback if getOption failed: seriesName array (ECharts 5 supports arrays)
    try { chart.dispatchAction({ type: 'highlight', seriesName: visibleNames }); } catch (_) {}
  }
  if (spectrumEnabled && spectrumChart) {
    if (specIndices.length) {
      try { spectrumChart.dispatchAction({ type: 'highlight', seriesIndex: specIndices }); } catch (_) {}
    } else {
      try { spectrumChart.dispatchAction({ type: 'highlight', seriesName: visibleNames }); } catch (_) {}
    }
  }
}

/**
 * Remove highlight (downplay) for all series belonging to modelId in both
 * main chart and spectrum chart.  Downplays all conditions for the model
 * (including legend-hidden ones) to fully clear any residual emphasis state.
 *
 * Debug: open DevTools console, run `window.FC_DEBUG_RADAR_HIGHLIGHT = true`,
 * then hover and un-hover a radar legend item.  Look for console groups
 * labelled "[RadarHighlight] downplayModelSeries(…)".
 *
 * @param {string|number} modelId
 */
function downplayModelSeries(modelId) {
  if (!chart) return;
  const mid = Number(modelId);

  // Include all conditions (even hidden) to clear any residual emphasis
  const names = [];
  getSeriesArray().forEach(s => {
    const sMid = Number((s.modelId != null) ? s.modelId : s.model_id);
    if (sMid !== mid) return;
    names.push(_getSeriesName(s));
  });
  ...
  if (window.FC_DEBUG_RADAR_HIGHLIGHT) {
    console.group(`[RadarHighlight] downplayModelSeries(modelId=${mid})`);
    console.log('names to downplay:', names);
    console.log('main chart → seriesIndex:', mainIndices);
    if (spectrumEnabled && spectrumChart) {
      console.log('spectrum chart → seriesIndex:', specIndices);
    }
  ...
  if (mainIndices.length) {
    try { chart.dispatchAction({ type: 'downplay', seriesIndex: mainIndices }); } catch (_) {}
  } else {
    try { chart.dispatchAction({ type: 'downplay', seriesName: names }); } catch (_) {}
  }
  if (spectrumEnabled && spectrumChart) {
    if (specIndices.length) {
      try { spectrumChart.dispatchAction({ type: 'downplay', seriesIndex: specIndices }); } catch (_) {}
    } else {
      try { spectrumChart.dispatchAction({ type: 'downplay', seriesName: names }); } catch (_) {}
    }
  }
}
