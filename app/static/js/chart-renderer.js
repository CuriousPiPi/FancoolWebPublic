...
function _updateChartPanelArrowAria(switcher) {
  ...
  btn.setAttribute('aria-label', showingExtra ? '切换回主图' : '展开点评/图库');
}
...
/**
 * Bind touch-swipe gesture on the panel switcher for narrow mode.
 * Extra panel is the left slot, main panel is the right slot.
 * Swipe right → switch to extra panel (reveal the left slot).
 * Swipe left  → switch to main panel (reveal the right slot).
 * A simple threshold-based detection; no drag-following physics needed.
 */
function _bindNarrowSwipe() {
  const switcher = document.querySelector('.chart-panel-switcher');
  if (!switcher || switcher._swipeBound) return;
  switcher._swipeBound = true;

  const SWIPE_THRESHOLD = 40; // px horizontal distance to count as a swipe
  let startX = 0;
  let startY = 0;
  ...
  switcher.addEventListener('touchend', (e) => {
    ...
    const upperRow = document.querySelector('.chart-upper-row');
    if (!upperRow || upperRow.classList.contains('has-no-curves')) return;
    const dx = e.changedTouches[0].clientX - startX;
    const dy = e.changedTouches[0].clientY - startY;
    // Require horizontal dominance and minimum distance
    if (Math.abs(dx) < SWIPE_THRESHOLD || Math.abs(dy) > Math.abs(dx)) return;
    // Extra panel is the left slot, main panel the right slot.
    // Swipe right (dx > 0) reveals the left/extra panel; swipe left (dx < 0) reveals the right/main panel.
    const showExtra = dx > 0;
    _setSwitcherExtra(switcher, showExtra, true);
  }, { passive: true });
}
