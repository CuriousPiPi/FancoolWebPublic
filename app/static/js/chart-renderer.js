...
function updateLegendRail(){
  ...
  const FIT_PRESET_LS_KEY = 'fc_fit_preset_x_v2';
  ...
  function getFitPresetLimits(mode){
    const key = (mode === 'noise_db') ? 'noise_db' : 'rpm';
    if (key === 'noise_db') {
      return { mode: key, min: 1, max: 99, defaults: [...FIT_PRESET_DEFAULTS.noise_db] };
    }
    return { mode: key, min: 600, max: 9999, defaults: [...FIT_PRESET_DEFAULTS.rpm] };
  }

  function normalizeFitPresetValues(mode, values){
    const { min, max, defaults } = getFitPresetLimits(mode);
...
function buildOption(payload) {
  ...
  const baselineOverlaySeries = buildBaselineOverlaySeries({ payload, seriesList: sList, xMode, isNarrow });
...
function bindPointerDrag(){
  ...
  function renderFitPresetHost(host, mode){
    ...
    const { min, max } = getFitPresetLimits(mode);
    ...
    host.innerHTML = `
      ${presets.map((value) => `
        <button type="button" class="fit-preset-btn" data-fit-preset-value="${escapeHtml(value)}">${escapeHtml(value)}</button>
      `).join('')}
...
function onBubbleInputCommit(){
  ...
  const roundedCommitted = (mode === 'noise_db') ? Number(committed.toFixed(1)) : Math.round(committed);
...
function evalPchipJS(model, x){
  ...
  function clampXQueryIntoVisibleRange(modeKey){
    const mode = modeKey || currentXModeFromPayload(lastPayload);
  ...
  if (mode === 'integrated') {
    ...
    if (inp) {
      ...
      const commitLegendInput = () => {
        ...
        const roundedCommitted = (modeKey === 'noise_db') ? Number(committed.toFixed(1)) : Math.round(committed);
        inp.value = String(roundedCommitted);
      };
