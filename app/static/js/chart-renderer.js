(function(){
  // 对外 API
  const API = { mount, render, resize, setOnXAxisChange, __ensureDock: () => ensureSpectrumDock(), highlightModelSeries, downplayModelSeries };
...
// ---- Chart-col layout: width-transition + fit-UI suspension ----
let __colLayoutTransitionTimer = null;
...
function updateChartColLayout(nextEmpty) {
  ...
  if (!__colLayoutInitialized) {
    // On first render: apply state immediately, no transition
    __colLayoutInitialized = true;
    if (nextHasNoCurves !== prevHasNoCurves) {
      upperRow.classList.add('no-col-transition');
      upperRow.classList.toggle('has-no-curves', nextHasNoCurves);
      // Force reflow then re-enable transition
      void upperRow.offsetWidth;
      upperRow.classList.remove('no-col-transition');
    }
    return;
  }
  ...
  if (__colLayoutTransitionTimer !== null) {
    clearTimeout(__colLayoutTransitionTimer);
    __colLayoutTransitionTimer = null;
  }

  // Suspend fit UI during the transition to avoid positioning bugs
  _suspendFitUI();

  upperRow.classList.toggle('has-no-curves', nextHasNoCurves);

  // After the CSS max-width transition completes (350ms) plus a small buffer
  const TRANSITION_MS = 400;
  __colLayoutTransitionTimer = setTimeout(() => {
    __colLayoutTransitionTimer = null;
    if (!nextHasNoCurves) {
      // Curves are now present: resize chart so it fills the restored column width
      try { if (chart) chart.resize(); } catch(_) {}
      _restoreFitUI();
      try { placeFitUI(); repaintPointer(); } catch(_) {}
    }
    // When collapsing (no curves), fit UI remains suppressed (no curves = nothing to fit)
  }, TRANSITION_MS);
}
...
function toggleFitUI(showFit){
  ...
  const showFloating = showFit && !empty && !integrated;
