const ColorManager = (function() {
  ...
  /**
   * Re-randomizes the base-color assignments for the given set of model IDs.
   * Palette colors not currently used by the active radar models are always
   * assigned first (preferred), so each click brings genuinely new hues into
   * the view rather than just permuting the existing ones.  Currently-used
   * colors serve as a fallback so the full palette is always reachable.
   *
   * Stale modelColorIndexMap entries left over from previously-removed models
   * are intentionally ignored: they are NOT treated as "locked" here because
   * treating them as locked is what caused certain palette slots to become
   * unreachable over time.
   *
   * Does not affect hidden/visibility state or per-curve slot assignments.
  ...
  function reassignModelColors(modelIds) {
    ...
    // Record which palette indices are currently assigned to the models being
    // reassigned BEFORE releasing them.  These are the "currently visible"
    // colors that we want to deprioritise in the new assignment so that the
    // reset feels like a true recolouring rather than a simple swap.
    const currentIndices = new Set(
      keys
        .filter(k => Object.prototype.hasOwnProperty.call(modelColorIndexMap, k))
        .map(k => modelColorIndexMap[k])
    );

    // Release all assignments for the models being reassigned.
    keys.forEach(key => { delete modelColorIndexMap[key]; });
    ...
    // Partition ALL palette slots into two groups:
    //   unusedIndices – not currently assigned to any of the target models
    //                   (these are the preferred / "fresh" colors).
    //   usedIndices   – currently assigned to the target models
    //                   (fallback; only reached when the palette is nearly
    //                    exhausted relative to the number of models).
    //
    // Stale entries for already-removed models that remain in
    // modelColorIndexMap are deliberately ignored: we do NOT lock those
    // indices.  Locking them is what historically prevented palette slots
    // 4-9 from ever being chosen again after a few add-remove cycles.
    const unusedIndices = [];
    const usedIndices   = [];
    ...
    // Shuffle each group independently so the order within each group is
    // random on every call.
    _shuffle(unusedIndices);
    _shuffle(usedIndices);

    // Pool: preferred (unused) colors come first; fallback (currently used)
    // colors are appended.  The combined pool covers the full palette, so
    // every key always gets a unique index as long as keys.length ≤ paletteSize.
    const pool = [...unusedIndices, ...usedIndices];
