...
async function addModelToRadar(modelId, brand, label) {
  ...
  if (window.Analytics && typeof window.Analytics.logRadarModels === 'function') {
    window.Analytics.logRadarModels('add', modelIdNum, 'radar_panel');
  }
...
function _updateExtraPanel(highlightedModelId) {
  ...
  if (titleEl) {
    ...
    titleEl.textContent = prefix ? `${prefix} 测试点评` : '测试点评';
  }
}
