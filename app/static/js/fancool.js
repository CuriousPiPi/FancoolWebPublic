...
window.APP_CONFIG.conditionDisplayOrder = normalizeConditionDisplayOrder(window.APP_CONFIG.conditionDisplayOrder);
/* ==== 命名空间根 ==== */
...
(async function fetchAppConfig(){
  try {
    ...
    if (resp.ok) {
      ...
      window.APP_CONFIG.conditionDisplayOrder = normalizeConditionDisplayOrder(cfg.condition_display_order);
...
/* =========================================================
   ...
   Stores: { "<mid>": { brand, model, reference_price, max_speed, size, thickness, rgb_names_zh } }
   ========================================================= */
const MODEL_META_LS_KEY = 'fc_model_meta_v4';
const MODEL_DETAIL_SCHEMA = window.ModelDetailPanel ? window.ModelDetailPanel.FULL_SCHEMA : null;
const MODEL_DETAIL_SCHEMA_KEYS = Array.isArray(MODEL_DETAIL_SCHEMA)
  ? Array.from(new Set(MODEL_DETAIL_SCHEMA.filter(key => typeof key === 'string' && key.length > 0)))
  : [
      'size',
      'max_speed',
      'rgb_names_zh',
...
const MODEL_META_REQUIRED_KEYS = [
  'brand',
  'model',
  'review',
  'purchase_links',
  ...MODEL_DETAIL_SCHEMA_KEYS,
];
...
function _hasCompleteModelMeta(meta) {
  if (!meta || typeof meta !== 'object') return false;
  return MODEL_META_REQUIRED_KEYS.every(key => Object.prototype.hasOwnProperty.call(meta, key));
}
...
function _fetchAndCacheModelMeta(modelIds) {
  ...
  const need = modelIds.map(Number).filter(mid => {
    ...
    return !_hasCompleteModelMeta(cached); // re-fetch if only partial data stored
  });
  if (!need.length) {
...
function installRemovedRenderHookOnce(){
  try{
    ...
    function enrichRemovedList(list){
      const arr = Array.isArray(list) ? list : [];
      const sig = arr.map(it => `${it?.key || ''}:${it?.removed_time || ''}`).join('|');
      if (sig === lastSig) return lastEnriched;
      lastSig = sig;
      lastEnriched = arr.map(it => {
        const meta = (window.__modelMetaCache && window.__modelMetaCache[String(it.model_id)]) || {};
        return {
          ...it,
          brand:     meta.brand     || it.brand || '',
          model:     meta.model     || it.model || '',
          condition: it.condition || '',
          reference_price: meta.reference_price != null ? meta.reference_price : (it.reference_price ?? null),
          max_speed:  meta.max_speed  || it.max_speed || null,
          size:       meta.size       || it.size || '',
          thickness:  meta.thickness  || it.thickness || '',
          rgb_names_zh: meta.rgb_names_zh || it.rgb_names_zh || '',
          review:     meta.review != null ? meta.review : (it.review ?? null),
        };
      });
      return lastEnriched;
    }
    ...
    mod.rebuild = function(list){
      return orig(enrichRemovedList(list));
    };
    mod.__ID_PATCHED__ = true;
    return true;
  }catch(_){ return false; }
}
// Try once now; later lifecycle hooks retry if the module is still unavailable.
...
function initSnapTabScrolling(opts){
  const {
    ...
    defaultTab,
    // Perf opt-in: once scrolling settles, take the inactive panes out of
    // rendering with `content-visibility:hidden` (see .is-pane-skipped in
    // fancool.css).  Panes keep their box size, so scroll-snap offsets are
    // unchanged; only their subtree stops participating in layout/paint.
    skipInactivePanes = false
  } = opts || {};
  ...
  const panes = skipInactivePanes ? Array.from(container.querySelectorAll('.fc-tab-pane')) : [];
  // Panes are revealed again while the container is scrolling, so the sliding
  // animation never shows an empty pane.
  const PANE_SKIP_DELAY_MS = 600; // > the .45s pane slide transition
  let paneSkipTimer = null;
  let panesSkipped = false;
  function revealPanes(){
    if (!panesSkipped) return;
    panesSkipped = false;
    panes.forEach(p => p.classList.remove('is-pane-skipped'));
  }
  function scheduleSkipInactivePanes(idx){
    if (!panes.length) return;
    clearTimeout(paneSkipTimer);
    paneSkipTimer = setTimeout(()=>{
      panesSkipped = true;
      panes.forEach((p,i)=>p.classList.toggle('is-pane-skipped', i !== idx));
    }, PANE_SKIP_DELAY_MS);
  }
...
function createToast(msg, type='info', opts={}) {
  ...
  div.id = id;
  div.innerHTML = `${iconMap[type]||iconMap.info}<div class="msg">${escapeHtml(msg)}</div><span class="fc-toast__close" data-close="1">&times;</span>`;
...
function normalizeApiResponse(json){
  ...
  if (json.success === true) {
    return { ok:true, data: json.data, meta: json.meta || {}, raw: json };
  }
  ...
  return { ok:false, error_code:'LEGACY_FORMAT', error_message:'不支持的旧响应格式', meta:{}, raw: json };
}
...
// Keep this fallback in sync with mini-radar-card.js.
const RADAR_CIDS_COLOR   = window.RADAR_CIDS_COLOR   || [1, 2, 3, 8, 7, 10];

// Backward-compatible shims until mini-radar-card.js is ready.
...
/* ===========================================================
   ...
   =========================================================== */
const RADAR_CURVE_CAP    = FRONT_MAX_ITEMS; // max simultaneous chart series (models × conditions)
const MAX_RADAR_MODELS   = FRONT_MAX_ITEMS; // hard cap on the number of models in radar comparison
const CAP_EXCEEDED_MSG   = '右侧数据过多，请清除部分条目';
const MODEL_LIMIT_MSG    = `雷达对比最多添加 ${FRONT_MAX_ITEMS} 个型号，请先移除部分型号`;
...
const RadarState = (function () {
  ...
  return {
    ...
    /**
     * Add a model to the radar comparison.
     * Returns true if the model was added, false if the hard cap (MAX_RADAR_MODELS)
     * would be exceeded.  Callers that want to show a user-facing message should
     * check the return value and use addModelToRadar() instead.
     */
    addModel(modelId, brand, label) {
      if (radarModels.size >= MAX_RADAR_MODELS && !radarModels.has(String(modelId))) {
        return false; // hard cap – caller is responsible for any user feedback
      }
...
/**
 * When true, persistRadarState() is a no-op.  Set to true at the start of
 * restoreRadarFromStorage() and cleared (with one final explicit commit) at the
 * end of that function.  This prevents any auto-persist wrapper (e.g. the
 * setModelHidden wrapper below) from writing a partially-restored snapshot to
 * localStorage while the in-memory RadarState is not yet fully reconciled.
 */
let _radarPersistSuspended = false;

/** Persist current RadarState selection (model identities + active conditions + visibility) to localStorage. */
function persistRadarState() {
  // Guard: skip all writes while a restore transaction is in progress.
  // restoreRadarFromStorage() issues one explicit final commit after it finishes.
  if (_radarPersistSuspended) return;
  ...
  RadarState.getModels().forEach((model) => {
    models.push({
      ...
      brand:   model.brand,
      label:   model.label,
      hidden:  RadarState.isModelHidden(String(model.modelId)),
    });
  });
...
// Ensure model visibility changes are persisted. Wrap RadarState.setModelHidden
// so that any successful hide/show toggle will trigger persistRadarState().
// During restoreRadarFromStorage() the _radarPersistSuspended guard keeps this
// wrapper from writing to localStorage mid-restore.
 if (window.RadarState && typeof window.RadarState.setModelHidden === 'function') {
   (function () {
    const _origSetModelHidden = window.RadarState.setModelHidden;
    window.RadarState.setModelHidden = function () {
      const result = _origSetModelHidden.apply(this, arguments);
      // After hidden state is updated, persist the current RadarState.
      try {
        persistRadarState();
      } catch (e) {
         // Swallow errors to avoid breaking existing call sites.
      }
      return result;
    };
  })();
 ...
 *
 * The entire restore sequence runs inside a persistence-suspend window
 * (_radarPersistSuspended = true) so that no intermediate call to
 * setModelHidden(), activateCondition(), or any other auto-persist wrapper can
 * write a partially-restored snapshot to localStorage.  A single explicit
 * persistRadarState() commit fires in the finally block once in-memory state is
 * fully reconciled.
 */
async function restoreRadarFromStorage() {
  ...
  if (!savedModels.length) {
    // No radar state to restore.  If the early-boot refreshChartFromLocal call
    // was suppressed (because models existed in storage at script-load time but
    // were then removed by syncPageStateWithBackend), we must trigger a chart
    // render now so that any non-radar selected pairs still get displayed.
    refreshChartFromLocal(false);
    return;
  }

  // Suspend all auto-persist writes for the duration of this restore transaction.
  _radarPersistSuspended = true;
  try {
    // Re-populate RadarState with identity data (no scores yet).
    // Cap to MAX_RADAR_MODELS so that old localStorage entries (from before the
    // cap was enforced) cannot restore more models than the hard limit allows.
    const modelsToRestore = savedModels.slice(0, MAX_RADAR_MODELS);
    modelsToRestore.forEach(m => {
      if (!RadarState.hasModel(m.modelId)) {
        RadarState.addModel(m.modelId, m.brand, m.label);
      }
      // Restore per-model visibility.  Old payloads without an explicit `hidden`
      // field default to visible (false) so they degrade safely.
      // NOTE: setModelHidden() normally triggers persistRadarState() via its
      // wrapper, but _radarPersistSuspended suppresses that write here so the
      // partially-restored conditions set is never committed to localStorage.
      RadarState.setModelHidden(m.modelId, !!m.hidden);
    });
    ...
    if (typeof LocalState.removeModelFromHistory === 'function') {
      modelsToRestore.forEach(m => LocalState.removeModelFromHistory(m.modelId));
    }
    ...
    const modelIds = modelsToRestore.map(m => m.modelId).filter(Boolean);
    ...
    // Always resume persistence and commit the fully-reconciled state, even if
    // an error occurred mid-restore.  This is the single canonical write for
    // this restore transaction: conditions + models + visibility all settled.
    _radarPersistSuspended = false;
    persistRadarState();
  }
}
 ...
 * validates them against backend model metadata availability, then:
 ...
 */
/**
 * Validate model visibility against backend availability.
 * @param {Array<number|string>} modelIds
 * @returns {Promise<{validMeta: Object, invalidIds: Set<number>}|null>}
 */
async function _validateModelsWithBackend(modelIds) {
  const ids = Array.from(new Set((modelIds || [])
    .map(Number)
    .filter(mid => Number.isFinite(mid) && mid > 0)));
  if (!ids.length) {
    return { validMeta: {}, invalidIds: new Set() };
  }
  try {
    const url = '/api/validate_models?model_ids=' + ids.join(',');
    const resp = await fetch(url);
    ...
    return {
      validMeta,
      invalidIds: new Set(invalidArr.map(Number)),
    };
  } catch (e) {
    typeof console !== 'undefined' && console.warn('[validate_models] request failed:', e);
    return null;
  }
}

/**
 * Merge validated model metadata into the shared frontend cache.
 * @param {Object} validMeta
 */
function _mergeModelMetaCache(validMeta) {
  if (!validMeta || !Object.keys(validMeta).length) return;
...
async function syncPageStateWithBackend() {
  ...
  try {
    const validation = await _validateModelsWithBackend(Array.from(allIds));
    if (!validation) return;
    const validMeta = validation.validMeta || {};
    const invalidArr = Array.from(validation.invalidIds || []);
    if (!invalidArr.length && !Object.keys(validMeta).length) return;

    // Refresh __modelMetaCache with up-to-date metadata from backend
    _mergeModelMetaCache(validMeta);
...
// ---- Radar Condition Onboarding ----
// Guides first-time users to discover that the 6 condition labels are clickable.
const _RADAR_ONBOARDING_KEY = 'radarConditionGuideDone';
// Session flag: set to true once onboarding starts; never reset so re-adding
// models after completion cannot re-trigger onboarding.
let _radarOnboardingStarted = false;

function _isRadarOnboardingDone() {
  try { return localStorage.getItem(_RADAR_ONBOARDING_KEY) === '1'; } catch (_) { return false; }
}

/** Start breathing-highlight onboarding if not already done. Only triggers once per session. */
function _maybeStartRadarOnboarding() {
  if (_radarOnboardingStarted || _isRadarOnboardingDone()) return;
  const api = window.RadarOverview;
  if (api && typeof api.startOnboarding === 'function') {
    _radarOnboardingStarted = true;
    api.startOnboarding();
  }
}

/** Complete onboarding: remove highlights and persist to localStorage. */
function _completeRadarOnboarding() {
  // Guard: only act if onboarding was started this session and not yet persisted.
  if (!_radarOnboardingStarted || _isRadarOnboardingDone()) return;
  if (window.RadarOverview && typeof window.RadarOverview.stopOnboarding === 'function') {
    window.RadarOverview.stopOnboarding();
  }
  try { localStorage.setItem(_RADAR_ONBOARDING_KEY, '1'); } catch (_) {}
}


async function addModelToRadar(modelId, brand, label, options = {}) {
  const modelIdNum = Number(modelId);
  const modelIdStr = String(modelIdNum);
  const source = String(options?.source || 'unknown');
  if (!Number.isFinite(modelIdNum) || modelIdNum <= 0) {
    return false;
  }

  const validation = await _validateModelsWithBackend([modelIdNum]);
  if (!validation) {
    typeof showInfo === 'function' && showInfo('网络或服务器异常，暂时无法验证型号可用性，请稍后重试');
    return false;
  }
  _mergeModelMetaCache(validation.validMeta);
  if (validation.invalidIds.has(modelIdNum)) {
    if (window.LocalState && typeof LocalState.removeModelFromHistory === 'function') {
      LocalState.removeModelFromHistory(modelIdNum);
      _rebuildBrowsingHistory();
    }
    if (window.__modelMetaCache) delete window.__modelMetaCache[modelIdStr];
    if (window.__radarCache) delete window.__radarCache[modelIdStr];
    typeof showInfo === 'function' && showInfo('该型号已下架，已从浏览历史移除');
    return false;
  }

  if (RadarState.hasModel(modelIdNum)) {
    typeof showInfo === 'function' && showInfo('该型号已在雷达图中');
    return false;
  }
  // Hard cap: reject if already at maximum model count.
  if (RadarState.getModels().size >= MAX_RADAR_MODELS) {
    typeof showInfo === 'function' && showInfo(MODEL_LIMIT_MSG);
    return false;
  }
  ...
  RadarState.addModel(modelIdNum, brand, label);
  if (addHidden) {
    ...
    RadarState.setModelHidden(String(modelIdNum), true);
  ...
  if (window.Analytics && typeof window.Analytics.logRadarModels === 'function') {
    window.Analytics.logRadarModels('add', modelIdNum, source);
  }

  const midStr = String(modelIdNum);
  ...
  if (window.__modelMetaCache && window.__modelMetaCache[midStr]) {
    delete window.__modelMetaCache[midStr].review;
  }
  _fetchAndCacheModelMeta([modelIdNum]);

  // Sync like state for newly added model's radar conditions
  _syncRadarModelLikeState(modelIdNum);
  ...
  _updateExtraPanel(null);
  _galleryCurrentModelId = null;
  _updateGallery(null);
  // Trigger onboarding the first time a model is successfully added to the radar.
  _maybeStartRadarOnboarding();
  return true;
}
function removeModelFromRadar(modelId, options = {}) {
  const source = String(options?.source || 'unknown');
  const model = RadarState.getModels().get(String(modelId));
  RadarState.removeModel(modelId);
  // Release the model's base color so the palette slot is freed for future
  // reassignments and doesn't accumulate as a stale entry in modelColorIndexMap.
  if (window.ColorManager && typeof window.ColorManager.releaseModelColor === 'function') {
    window.ColorManager.releaseModelColor(modelId);
  }
  ...
  if (window.Analytics && typeof window.Analytics.logRadarModels === 'function') {
    window.Analytics.logRadarModels('remove', modelId, source);
  }
...
function clearAllRadarModels() {
  ...
  models.forEach(model => {
    RadarState.removeModel(String(model.modelId));
    // Release each model's base color to keep modelColorIndexMap clean.
    if (window.ColorManager && typeof window.ColorManager.releaseModelColor === 'function') {
      window.ColorManager.releaseModelColor(model.modelId);
    }
  });
  _rebuildBrowsingHistory();
...
function activateRadarCondition(cid) {
  // Complete onboarding on the first condition label click.
  _completeRadarOnboarding();
  const cidStr = String(cid);
  if (RadarState.getActiveConditions().has(cidStr)) {
    ...
    persistRadarState();
    // Auto-expand main chart if curves still remain after deactivation
    _autoExpandMainChartIfCurves();
    return;
  }
  ...
  persistRadarState();
  // Auto-expand main chart if curves still remain after activation
  _autoExpandMainChartIfCurves();
}

/**
 * Auto-expand the main chart panel (slide back from extra panel) when radar condition
 * labels are clicked and there are still curves to display.
 */
function _autoExpandMainChartIfCurves() {
  if (RadarState.computeSeriesCount() > 0) {
    if (window.ChartRenderer && typeof window.ChartRenderer.expandMainChart === 'function') {
      window.ChartRenderer.expandMainChart();
    }
  }
}

/**
 * Diff RadarState into LocalState while enforcing the series cap.
 ...
 *   - Pass 'restore': log all currently desired pairs (not just the diff)
 *     so that page-restore is recorded even when LocalState already matches
 *     RadarState. The backend's same-visit suppression prevents duplicate writes.
 *   - Pass another string event_type (e.g. 'model_add', 'model_remove',
 *     'model_show', 'model_hide', 'radar_clear_all', 'reset_condition'):
 *     overrides the event_type sent for the diff.
...
function syncFromRadarState(eventTypeOrSuppress = false, actionId) {
  ...
  if (!suppressLog && window.Analytics && typeof window.Analytics.logCurvePairs === 'function') {
    ...
    if (overrideType === 'restore') {
      // On restore, log all currently desired pairs so the backend can decide
      // whether to write user_query_logs (same-visit suppression handles dedup).
      const restorePairs = Array.from(desired.values());
      if (restorePairs.length > 0) {
        window.Analytics.logCurvePairs('restore', restorePairs, 'radar', actionId);
      }
    } else {
      if (toAdd.length > 0) {
...
function _wireRadarOverviewCallbacks() {
  ...
  if (typeof window.RadarOverview.onModelRemove === 'function') {
    window.RadarOverview.onModelRemove((modelId) => removeModelFromRadar(modelId, { source: 'radar_panel' }));
  }
  ...
  if (typeof window.RadarOverview.onColorChange === 'function') {
    window.RadarOverview.onColorChange(() => {
      // Refresh the performance chart so derived/extended curve colours also update.
      if (typeof lastChartData !== 'undefined' && lastChartData) postChartData(lastChartData);
    });
  }
}
...
const CHART_EXTRA_DETAIL_SCHEMA = [
  'size',
  'max_speed',
  'rgb_names_zh',
  'bearing_type_zh',
...
const _extraPanelThumbCache = new Map();

function _loadExtraPanelThumbUrl(modelId) {
  ...
  if (_extraPanelThumbCache.has(cacheKey)) return _extraPanelThumbCache.get(cacheKey);
  ...
  const p = fetch(`/api/gallery/${mid}`)
    ...
    .catch(() => {
      _extraPanelThumbCache.delete(cacheKey);
      return null;
    });

  _extraPanelThumbCache.set(cacheKey, p);
  return p;
}

let _reviewJumpHandlersBound = false;

function _renderExtraPanelReviewBody(reviewBody, reviewText) {
  if (!reviewBody) return;

  const text = typeof reviewText === 'string' ? reviewText : String(reviewText || '');
  reviewBody.innerHTML = '';
  if (!text) {
    reviewBody.textContent = '暂无点评';
    return;
  }

  const parser = window.__APP && window.__APP.reviewJump;
  const reviewJumpTargetFileShare = (parser && parser.REVIEW_JUMP_TARGET_FILE_SHARE) || 'file-share';
  const reviewJumpDefaultLabel = (parser && parser.REVIEW_JUMP_DEFAULT_LABEL) || '资源下载';
  const parseReviewInlineTokens = parser && typeof parser.parseReviewInlineTokens === 'function'
    ? parser.parseReviewInlineTokens
    : null;
  const tokens = parseReviewInlineTokens ? parseReviewInlineTokens(text) : [{ type: 'text', value: text }];

  const fragment = document.createDocumentFragment();
  tokens.forEach((token) => {
    if (!token || token.type !== 'jump' || token.target !== reviewJumpTargetFileShare) {
      fragment.appendChild(document.createTextNode((token && token.value) || ''));
      return;
    }
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'fc-review-inline-jump';
    btn.setAttribute('data-jump-target', reviewJumpTargetFileShare);
    btn.textContent = token.label || reviewJumpDefaultLabel;
    btn.setAttribute('aria-label', `跳转到资源下载：${btn.textContent}`);
    fragment.appendChild(btn);
  });
  reviewBody.appendChild(fragment);
}

function _activateSidebarTopTab(tabName) {
  const nav = document.querySelector('.fc-tabs[data-tab-group="sidebar-top"]');
  if (!nav) return false;
  const tabItem = nav.querySelector(`.fc-tabs__item[data-tab="${tabName}"]`);
  if (!tabItem) return false;
  tabItem.click();
  return true;
}

function _jumpToFileShareTab() {
  try {
    window.__APP?.sidebar?.expandSidebarIfCollapsed?.();
  } catch (err) {
    console.warn('Failed to expand sidebar:', err);
  }
  if (!_activateSidebarTopTab('file-share')) {
    loadFileShareIfNeeded();
  }
}

function _jumpFromReviewToFileShareTab() {
  _jumpToFileShareTab();
}

let _fileShareEntryBtnBound = false;
(function _bindFileShareEntryBtn() {
  if (_fileShareEntryBtnBound) return;
  _fileShareEntryBtnBound = true;
  document.addEventListener('click', (e) => {
    const trigger = safeClosest(e.target, '#fileShareEntryBtn');
    if (!trigger) return;
    _jumpToFileShareTab();
  });
})();

function _bindReviewJumpHandlersOnce() {
  if (_reviewJumpHandlersBound) return;
  _reviewJumpHandlersBound = true;
  document.addEventListener('click', (e) => {
    const trigger = safeClosest(e.target, '.fc-review-inline-jump');
    if (!trigger) return;
    const parser = window.__APP && window.__APP.reviewJump;
    const reviewJumpTargetFileShare = (parser && parser.REVIEW_JUMP_TARGET_FILE_SHARE) || 'file-share';
    if ((trigger.getAttribute('data-jump-target') || '') !== reviewJumpTargetFileShare) return;
    e.preventDefault();
    _jumpFromReviewToFileShareTab();
  });
}
...
function _updateExtraPanel(highlightedModelId) {
  ...
  _renderExtraPanelReviewBody(reviewBody, review);

  // Update the panel title to include brand and model name when available
  const titleEl = document.querySelector('.extra-panel-title');
  if (titleEl) {
    const brand = (meta && meta.brand) || '';
    const model = (meta && meta.model) || '';
    titleEl.textContent = [brand, model].filter(Boolean).join(' ');
  }

  // Render model detail panel (尺寸, 最大转速, RGB灯光, 参考价格 + extras if available)
  const detailHost = document.querySelector('.extra-panel-model-detail');
  if (detailHost && window.ModelDetailPanel) {
    const detailItem = meta ? { ...meta, model_id: targetModelId } : null;
    const panel = window.ModelDetailPanel.renderInto(detailHost, detailItem, {
      showPhoto: true,
      showPurchase: true,
      schema: CHART_EXTRA_DETAIL_SCHEMA,
      loadThumb: _loadExtraPanelThumbUrl,
      imageLoading: 'eager',
      purchaseSource: 'fc_model_detail',
    });
    panel.ensureImageLoaded();
  }

  // Notify the issue-feedback modal about the current model
  if (window.IssueFeedbackModal && typeof window.IssueFeedbackModal.setCurrentModel === 'function') {
    const brand = (meta && meta.brand) || '';
    const model = (meta && meta.model) || '';
    const label = [brand, model].filter(Boolean).join(' ');
    window.IssueFeedbackModal.setCurrentModel(targetModelId ? Number(targetModelId) : null, label);
  }
}
...
function _onRadarHighlightChange(highlightedModelId) {
  ...
  _updateGallery(highlightedModelId);
}

// ---- Gallery display ----
let _galleryCurrentModelId = null;
let _galleryFetchController = null;
let _galleryLightboxItems = [];
let _galleryLightboxIndex = -1;

/** Determine the effective model ID for the gallery (same rules as review panel). */
function _galleryTargetModelId(highlightedModelId) {
  const radarModelIds = Array.from(RadarState.getModels().keys());
  if (radarModelIds.length === 0) return null;
  if (radarModelIds.length === 1) return radarModelIds[0];
  const hlStr = highlightedModelId != null ? String(highlightedModelId) : null;
  if (hlStr && radarModelIds.includes(hlStr)) return hlStr;
  return _lastReviewModelId && radarModelIds.includes(_lastReviewModelId)
    ? _lastReviewModelId : radarModelIds[0];
}

/** Fetch and render gallery thumbnails for the effective model. */
function _updateGallery(highlightedModelId) {
  const thumbsEl = document.querySelector('.gallery-thumbs');
  if (!thumbsEl) return;

  const targetId = _galleryTargetModelId(highlightedModelId);
  if (targetId === _galleryCurrentModelId) return;
  _galleryCurrentModelId = targetId;

  // Cancel any in-flight request
  if (_galleryFetchController) {
    _galleryFetchController.abort();
  }

  if (!targetId) {
    _renderGallery(thumbsEl, []);
    _renderGalleryVideo(null);
    return;
  }

  const ctrl = new AbortController();
  _galleryFetchController = ctrl;

  fetch(`/api/gallery/${targetId}`, { signal: ctrl.signal })
    .then(r => r.json())
    .then(j => {
      if (ctrl.signal.aborted) return;
      _galleryFetchController = null;
      const items = (j.success && j.data && j.data.items) ? j.data.items : [];
      const videoBV = (j.success && j.data && j.data.video_BV) ? j.data.video_BV : null;
      const videoPosterUrl = (j.success && j.data && j.data.video_poster_url) ? j.data.video_poster_url : null;
      _renderGallery(thumbsEl, items);
      _renderGalleryVideo(videoBV, videoPosterUrl);
    })
    .catch(err => {
...
/**
 * Show or hide the Bilibili video panel based on video_BV.
 * If bvid is present: show the panel; if posterUrl is supplied, use it as the
 * poster image, otherwise leave the poster area empty.  Wires up inline
 * play-on-click with a smooth poster→iframe fade transition.
 * If bvid is absent: hide the panel so it takes no space.
 */
function _renderGalleryVideo(bvid, posterUrl) {
  const panel = document.querySelector('.gallery-video-panel');
  if (!panel) return;

  const posterEl = panel.querySelector('.gallery-video-poster');
  const iframeEl = panel.querySelector('.gallery-video-iframe');
  const posterImg = panel.querySelector('.gallery-video-poster-img');

  if (iframeEl) { iframeEl.src = ''; iframeEl.style.display = 'none'; }
  if (posterEl) { posterEl.style.opacity = '1'; posterEl.style.pointerEvents = ''; }
  if (posterImg) { posterImg.src = ''; posterImg.style.display = ''; }

  // Clone the panel to drop all old event listeners cleanly.
  const fresh = panel.cloneNode(true);
  panel.parentNode.replaceChild(fresh, panel);
  const newPanel = fresh;
  const newPoster = newPanel.querySelector('.gallery-video-poster');
  const newIframe = newPanel.querySelector('.gallery-video-iframe');
  const newPosterImg = newPanel.querySelector('.gallery-video-poster-img');
  ...
  // Load poster from the locally-cached URL returned by the API.
  // If absent (cache miss or no BV set), leave the poster area empty so the
  // play button remains visible without a broken-image placeholder.
  if (newPosterImg) {
    if (posterUrl) {
      newPosterImg.src = posterUrl;
      // If the poster fails to load, hide the broken-image icon; the play button remains visible.
      newPosterImg.addEventListener('error', () => { newPosterImg.style.display = 'none'; }, { once: true });
    } else {
      newPosterImg.style.display = 'none';
    }
  }

  const embedSrc =
    `https://player.bilibili.com/player.html?isOutside=true` +
    `&bvid=${encodeURIComponent(bvid)}&p=1&autoplay=1`;

  // On click/Enter/Space: load the iframe behind the poster, then fade the poster out.
  function _startPlay() {
    newPanel.removeEventListener('click', _startPlay);
    newPanel.removeEventListener('keydown', _onKeyDown);

    if (newIframe) {
      newIframe.style.display = 'block';
      newIframe.src = embedSrc;

      // Guard so doFade runs only once (load event + setTimeout fallback can both fire).
      let faded = false;
      const doFade = () => {
        if (faded) return;
        faded = true;
        if (newPoster) newPoster.style.opacity = '0';
        // Remove poster from pointer-event flow after the 0.35 s CSS transition finishes
        // so player controls underneath become fully accessible.
        setTimeout(() => {
          if (newPoster) newPoster.style.pointerEvents = 'none';
        }, 380);
      };
      newIframe.addEventListener('load', doFade, { once: true });
      // 1.5 s fallback: Bilibili iframe may not reliably fire the load event.
  ...
  newPanel.addEventListener('click', _startPlay);
  newPanel.addEventListener('keydown', _onKeyDown);
}

function _renderGallery(thumbsEl, items) {
  thumbsEl.innerHTML = '';
  _galleryLightboxItems = Array.isArray(items) ? items.slice() : [];

  if (_galleryLightboxItems.length === 0) {
    _closeLightbox();
  } else if (_galleryLightboxIndex >= 0) {
    _galleryLightboxIndex = Math.min(_galleryLightboxIndex, _galleryLightboxItems.length - 1);
    _setLightboxImageByIndex(_galleryLightboxIndex);
  }

  _galleryLightboxItems.forEach((item, index) => {
    const wrap = document.createElement('div');
    wrap.className = 'gallery-thumb-item' + (item.is_primary ? ' is-primary' : '');
    const img = document.createElement('img');
    img.src = item.thumb_url;
    img.alt = item.filename;
    img.loading = 'lazy';
    img.addEventListener('click', () => _openLightbox(index, item.original_url));
...
/** Lightbox logic */
function _setLightboxNavState() {
  const prevBtn = document.getElementById('galleryLightboxPrev');
  const nextBtn = document.getElementById('galleryLightboxNext');
  const prevBottomBtn = document.getElementById('galleryLightboxPrevBottom');
  const nextBottomBtn = document.getElementById('galleryLightboxNextBottom');
  const counter = document.getElementById('galleryLightboxCounter');
  const total = _galleryLightboxItems.length;
  const atStart = _galleryLightboxIndex <= 0;
  const atEnd = _galleryLightboxIndex >= total - 1;

  [prevBtn, prevBottomBtn].forEach(btn => {
    if (!btn) return;
    btn.disabled = atStart;
    btn.classList.toggle('is-disabled', atStart);
  });
  [nextBtn, nextBottomBtn].forEach(btn => {
    if (!btn) return;
    btn.disabled = atEnd;
    btn.classList.toggle('is-disabled', atEnd);
  });
  if (counter) {
    counter.textContent = `${_galleryLightboxIndex + 1} / ${total}`;
  }
}

function _setLightboxImageByIndex(index) {
  const lb = document.getElementById('galleryLightbox');
  const img = document.getElementById('galleryLightboxImg');
  if (!lb || !img || _galleryLightboxItems.length === 0) return;
  if (index < 0 || index >= _galleryLightboxItems.length) return;
  _galleryLightboxIndex = index;
  const current = _galleryLightboxItems[_galleryLightboxIndex];
  img.src = current.original_url;
  lb.classList.add('is-open');
  _setLightboxNavState();
}

function _openLightbox(indexOrSrc, srcFallback) {
  if (_galleryLightboxItems.length === 0) return;
  if (typeof indexOrSrc === 'number' && Number.isFinite(indexOrSrc)) {
    _setLightboxImageByIndex(indexOrSrc);
    return;
  }
  const src = typeof indexOrSrc === 'string' ? indexOrSrc : srcFallback;
  const index = _galleryLightboxItems.findIndex(item => item && item.original_url === src);
  _setLightboxImageByIndex(index >= 0 ? index : 0);
}

function _showPrevLightboxImage() {
  if (_galleryLightboxIndex <= 0) return;
  _setLightboxImageByIndex(_galleryLightboxIndex - 1);
}

function _showNextLightboxImage() {
  if (_galleryLightboxIndex >= _galleryLightboxItems.length - 1) return;
  _setLightboxImageByIndex(_galleryLightboxIndex + 1);
}
...
function _closeLightbox() {
  ...
  if (img) img.src = '';
  _galleryLightboxIndex = -1;
  _setLightboxNavState();
}

// Wire up lightbox close button and backdrop click
(function _initLightbox() {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (prevBtn) prevBtn.addEventListener('click', e => {
      e.stopPropagation();
      _showPrevLightboxImage();
    });
    if (nextBtn) nextBtn.addEventListener('click', e => {
      e.stopPropagation();
      _showNextLightboxImage();
    });
    if (prevBottomBtn) prevBottomBtn.addEventListener('click', _showPrevLightboxImage);
    if (nextBottomBtn) nextBottomBtn.addEventListener('click', _showNextLightboxImage);
    if (lb) {
      lb.addEventListener('click', e => {
        if (e.target === lb) _closeLightbox();
      });
    }
...
function syncRadarOverview(series) {
  ...
  RadarState.getModels().forEach((model, mid) => {
    ...
    if (radar) {
      if (radar.conditions && typeof radar.conditions === 'object' && !Array.isArray(radar.conditions)) {
        Object.entries(radar.conditions).forEach(([cid, sc]) => {
          scores[Number(cid)] = sc.score_total;
        });
      }
      compositeScore = radar.composite_score != null ? radar.composite_score : null;
    }
    modelList.push({
      modelId:  model.modelId,
      brand:    model.brand,
...
let _lastRecentLikesList = null; // stored for profile-switch re-render
let _recentLikesRenderPending = false;
const recentLikesListEl = $('#recentLikesList');

/* ---------------------------------------------------------
   侧栏顶部页签的按需渲染
   迷你雷达卡很"重"（每张 ~40 个 SVG 盒子），在页签未激活或侧栏收起时
   建卡纯属浪费，还会把这些节点长期留在布局树里放大后续重排成本。
   这里只打脏标记，等到真正可见时再渲染。
   --------------------------------------------------------- */
const SIDEBAR_TOP_RENDER_LIMIT = 30; // 单个列表最多渲染多少张卡片

function isSidebarContentVisible(){
  const sb = document.getElementById('sidebar');
  if (!sb) return true; // 没有侧栏时不做推迟
  return !sb.classList.contains('collapsed');
}
function isSidebarTopTabActive(tabName){
  const nav = document.querySelector('.fc-tabs[data-tab-group="sidebar-top"]');
  if (!nav) return true; // 没有页签结构时不做推迟
  const tab = nav.querySelector(`.fc-tabs__item[data-tab="${tabName}"]`);
  return !!(tab && tab.classList.contains('active'));
}
function shouldRenderSidebarTopPane(tabName){
  return isSidebarContentVisible() && isSidebarTopTabActive(tabName);
}
/** Render whatever was deferred while a pane was hidden. */
function flushPendingSidebarTopRender(){
  if (_recentLikesRenderPending && shouldRenderSidebarTopPane('recent-liked')) {
    rebuildRecentLikes(_lastRecentLikesList);
  }
  try { window.__APP?.features?.recentlyRemoved?.flushPending?.(); } catch(_) {}
}
window.shouldRenderSidebarTopPane = shouldRenderSidebarTopPane;
window.SIDEBAR_TOP_RENDER_LIMIT = SIDEBAR_TOP_RENDER_LIMIT;
window.flushPendingSidebarTopRender = flushPendingSidebarTopRender;
  ...
  fetch('/api/like_keys')
    ...
    .then(j=>{
      ...
      if (Array.isArray(arr)){
        // Separate condition keys from lighting keys
        const conditionKeys = arr.filter(k => !k.startsWith('lighting_'));
        const lightingMids = arr.filter(k => k.startsWith('lighting_'))
          .map(k => k.replace('lighting_', ''));
        LocalState.likes.setAll(conditionKeys);
        LocalState.lightingLikes.setAll(lightingMids);
        conditionKeys.forEach(k=>{
          const [m,c] = k.split('_');
          if (m && c) updateLikeIcons(m, c, true);
        });
        lightingMids.forEach(mid => updateLightingLikeIcons(mid, true));
      }
      if (data.fp){
        LocalState.likes.updateServerFP(data.fp);
      }
...
function rebuildRecentLikes(list){
  _lastRecentLikesList = list; // store for profile-switch re-render
  const wrap = recentLikesListEl;
  if (!wrap) return;

  // 页签未激活 / 侧栏收起时不建卡，激活时由 flushPendingSidebarTopRender 补渲染。
  if (!shouldRenderSidebarTopPane('recent-liked')) {
    _recentLikesRenderPending = true;
    return;
  }
  _recentLikesRenderPending = false;
  wrap.innerHTML = '';
  ...
  // Use canonical backend field names from /api/recent_likes.
  ...
  list.forEach(it => {
    ...
    if (!modelGroups.has(mid)) {
      modelGroups.set(mid, {
        ...
        // Canonical fields from updated API (reference_price, max_speed, rgb_names_zh)
        price:     it.reference_price != null ? it.reference_price : (it.price != null ? it.price : null),
        maxSpeed:  it.max_speed   || '',
        size:      it.size        || '',
        thickness: it.thickness   || '',
        rgbTypeName: it.rgb_names_zh || '',
        hasLightingLike: false,
        rgbFlags:  it.rgb_flags != null ? it.rgb_flags : 0,
      });
    }
    // Track if this model has a lighting like event
    if (it.kind === 'lighting') {
      const g = modelGroups.get(mid);
      if (g) g.hasLightingLike = true;
    }
  });

  // Fetch missing radar scores and condition labels before rendering cards
  const modelIds = Array.from(modelGroups.keys()).map(Number).filter(Boolean);
  const labelReady = (window.__condLabelCacheReady instanceof Promise)
    ? window.__condLabelCacheReady : Promise.resolve();
  Promise.all([_fetchMissingRadarScores(modelIds), labelReady]).then(() => {
...
function _radarItemsFromCache(mid) {
  ...
  const score = radarData.composite_score != null ? radarData.composite_score : null;
  return { items, score };
}

function _renderRecentLikesCards(wrap, modelGroups) {
  const frag = document.createDocumentFragment();
  // 渲染上限只作用于建卡；接口返回的完整列表仍被 reloadRecentLikes 全量遍历，
  // 用于回填 LocalState.likes / lightingLikes 与 recentLikesLoadedCount。
  let rendered = 0;
  modelGroups.forEach(g => {
    if (rendered >= SIDEBAR_TOP_RENDER_LIMIT) return;
    rendered++;
    ...
    const cardEl = buildMiniRadarCardEl({
      ...
      maxSpeed: g.maxSpeed, size: g.size, thickness: g.thickness, rgbTypeName: g.rgbTypeName,
      modelId: g.modelId,
      radarItems: items, compositeScore: score,
      condLabelCache: window.__condLabelCache,
      radarSource: 'recent_likes',
      showHeader: true, showLikeThumbs: true, showQuickAdd: true,
      hasLightingLike: g.hasLightingLike,
      rgbFlags: g.rgbFlags,
    });
    ...
    .then(j=>{
      ...
      list.forEach(it=>{
        if (it.kind === 'lighting') {
          ...
          if (!LocalState.lightingLikes.has(mid)){
            LocalState.lightingLikes.add(mid);
            updateLightingLikeIcons(mid, true);
            changed = true;
          }
        } else if (it.model_id != null && it.condition_id != null){
      ...
      if (changed){
        list.forEach(it=>{
          if (it.kind !== 'lighting' && it.model_id != null && it.condition_id != null){
...
/* =========================================================
   资源下载 (File Share) — Sidebar tab
   ========================================================= */
let _fileShareLoaded = false;
let _fileShareLightboxItems = [];

function loadFileShareIfNeeded() {
  if (_fileShareLoaded) return;
  _fileShareLoaded = true;
  _fetchFileShareData();
}

async function _fetchFileShareData() {
  const listEl = document.getElementById('fileShareList');
  if (!listEl) return;
  try {
    const resp = await fetch('/api/file-share');
    const json = await resp.json();
    if (!json.success) {
      listEl.innerHTML = '<p class="text-gray-500 text-center py-6">加载失败，请稍后重试</p>';
      return;
    }
    _renderFileShareList(listEl, json.data.groups || []);
  } catch (e) {
    listEl.innerHTML = '<p class="text-gray-500 text-center py-6">加载失败，请检查网络</p>';
  }
}

function _renderFileShareList(listEl, groups) {
  listEl.innerHTML = '';
  _fileShareLightboxItems = [];

  if (!groups || groups.length === 0) {
    listEl.innerHTML = '<p class="text-gray-500 text-center py-6">暂无文件</p>';
    return;
  }

  groups.forEach(group => {
    const catEl = document.createElement('div');
    catEl.className = 'fc-file-share-category';

    const header = document.createElement('div');
    header.className = 'fc-file-share-category-header';
    const chevron = document.createElement('i');
    chevron.className = 'fa-solid fa-chevron-down fc-file-share-category-chevron';
    chevron.setAttribute('aria-hidden', 'true');
    const catLabel = document.createElement('span');
    catLabel.textContent = group.category_name_zh || '';
    header.appendChild(chevron);
    header.appendChild(catLabel);
    header.setAttribute('role', 'button');
    header.tabIndex = 0;
    const toggle = () => catEl.classList.toggle('is-collapsed');
    header.addEventListener('click', toggle);
    header.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggle(); } });

    const itemsEl = document.createElement('div');
    itemsEl.className = 'fc-file-share-category-items';

    (group.items || []).forEach(item => {
      const card = _buildFileShareCard(item);
...
function _buildFileShareCard(item) {
  const card = document.createElement('div');
  card.className = 'fc-file-share-card';

  // Thumbnail
  const thumbDiv = document.createElement('div');
  thumbDiv.className = 'fc-file-share-card-thumb';

  if (item.thumb_url) {
    const thumbIdx = _fileShareLightboxItems.length;
    _fileShareLightboxItems.push({
      original_url: `/api/file-share/original/${item.thumb_url.split('/').pop()}`,
      thumb_url: item.thumb_url,
    });
    const img = document.createElement('img');
    img.src = item.thumb_url;
    img.alt = item.title_zh || '';
    img.loading = 'lazy';
    img.addEventListener('click', () => _openFileShareLightbox(thumbIdx));
    thumbDiv.appendChild(img);
  } else {
    const placeholder = document.createElement('span');
    placeholder.className = 'fc-file-share-card-thumb-placeholder';
  ...
  body.className = 'fc-file-share-card-body';

  const title = document.createElement('div');
  title.className = 'fc-file-share-card-title';
  title.textContent = item.title_zh || '';

  const desc = document.createElement('div');
  desc.className = 'fc-file-share-card-desc';
  desc.textContent = item.description_zh || '';

  const footer = document.createElement('div');
  footer.className = 'fc-file-share-card-footer';

  const dlBtn = document.createElement('a');
  dlBtn.href = `/go/file-share/${item.file_id}`;
  dlBtn.target = '_blank';
  dlBtn.rel = 'noopener noreferrer';
  dlBtn.className = 'fc-file-share-download-btn';
  dlBtn.title = '下载';
...
function _openFileShareLightbox(index) {
  if (_fileShareLightboxItems.length === 0) return;
  if (index < 0 || index >= _fileShareLightboxItems.length) return;
  // Reuse the gallery lightbox by temporarily setting its items to the file-share items.
  // The existing close/nav handlers will work correctly.
  _galleryLightboxItems = _fileShareLightboxItems.slice();
  _setLightboxImageByIndex(index);
}

function activateTab(group, tabName, animate = false) {
  // 右侧主容器由 right-panel-v2.js 的 snap 接管，始终跳过
  if (group === 'sidebar-top' || group === 'left-panel' || group === 'right-panel') return;
...
function syncTopTabsViewportHeight(){
  ...
  if (!container) return;
  // 侧栏内容被 content-visibility 跳过时测不到真实尺寸，跳过本次同步；
  // 展开时会重新触发一次。
  const sidebarEl = document.getElementById('sidebar');
  if (sidebarEl && sidebarEl.classList.contains('is-content-skipped')) return;
...
function processState(data, successMsg){
  ...
  if ('like_keys' in data){
    const allKeys = data.like_keys || [];
    const conditionKeys = allKeys.filter(k => !k.startsWith('lighting_'));
    const lightingMids = allKeys.filter(k => k.startsWith('lighting_'))
      .map(k => k.replace('lighting_', ''));
    LocalState.likes.setAll(conditionKeys);
    LocalState.lightingLikes.setAll(lightingMids);
  }
...
function updateLightingLikeIcons(modelId, isLiked){
  window.__APP.dom.all(`.fc-radar-lighting-like-btn[data-model-id="${modelId}"]`)
    .forEach(btn => {
...
// Recent-likes refresh is scheduled by the unified like action layer (toggleConditionLike / toggleLightingLike).

/* =========================================================
   统一点赞动作层（条件点赞 / 灯效点赞）
   主雷达图与迷你雷达卡都通过这两个函数操作，避免分叉实现。
   options.suppressSuccessToast=true：不弹"已点赞/已取消点赞"提示（迷你卡场景）。
   ========================================================= */
function toggleConditionLike(modelId, conditionId, options){
  const { suppressSuccessToast = false } = options || {};
  if (needThrottle('like') && !globalThrottle()) return;
  const midNum = Number(modelId);
  const cidNum = Number(conditionId);
  if (!Number.isFinite(midNum) || !Number.isFinite(cidNum)) { showError('缺少点赞标识'); return; }
  const midStr = String(midNum);
  const cidStr = String(cidNum);
  const keyStr = `${midStr}_${cidStr}`;
  const prevLiked = LocalState.likes.has(keyStr);
  const nextLiked = !prevLiked;
  const url = prevLiked ? '/api/unlike' : '/api/like';

  // Optimistic update
  updateLikeIcons(midStr, cidStr, nextLiked);
  if (nextLiked) LocalState.likes.add(keyStr); else LocalState.likes.remove(keyStr);
  _syncRadarLikeStateFromLocal(midStr);
  ...
  fetch(url, {
    ...
    .then(j => {
      const resp = normalizeApiResponse(j);
      if (!resp.ok) {
        updateLikeIcons(midStr, cidStr, prevLiked);
      ...
      updateLikeIcons(midStr, cidStr, finalLiked);
      _syncRadarLikeStateFromLocal(midStr);
...
function toggleLightingLike(modelId, options){
  const { suppressSuccessToast = false } = options || {};
  if (needThrottle('like') && !globalThrottle()) return;
  const midNum = Number(modelId);
  if (!Number.isFinite(midNum)) { showError('缺少型号标识'); return; }
  const midStr = String(midNum);
  const prevLiked = LocalState.lightingLikes.has(midStr);
  const nextLiked = !prevLiked;
  const url = prevLiked ? '/api/lighting_unlike' : '/api/lighting_like';

  // Optimistic update
  if (nextLiked) LocalState.lightingLikes.add(midStr); else LocalState.lightingLikes.remove(midStr);
  updateLightingLikeIcons(midStr, nextLiked);
  document.dispatchEvent(new CustomEvent('fc:like-changed', {
    detail: { modelId: midStr, conditionId: '0', liked: nextLiked },
  }));

  fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ model_id: Number(midStr) })
  })
    .then(r => r.json())
    .then(j => {
      const resp = normalizeApiResponse(j);
      if (!resp.ok) {
        if (prevLiked) LocalState.lightingLikes.add(midStr); else LocalState.lightingLikes.remove(midStr);
        updateLightingLikeIcons(midStr, prevLiked);
        document.dispatchEvent(new CustomEvent('fc:like-changed', {
          detail: { modelId: midStr, conditionId: '0', liked: prevLiked },
        }));
        showError(resp.error_message || '操作失败');
        return;
      }
      ...
      updateLightingLikeIcons(midStr, LocalState.lightingLikes.has(midStr));
      scheduleRecentLikesRefresh();
      if (!suppressSuccessToast) {
        showSuccess(prevLiked ? '已取消灯效点赞' : '已点赞灯效');
      }
    })
    .catch(err => {
      if (prevLiked) LocalState.lightingLikes.add(midStr); else LocalState.lightingLikes.remove(midStr);
...
// Expose unified like functions globally
window.__APP.toggleConditionLike = toggleConditionLike;
window.__APP.toggleLightingLike  = toggleLightingLike;
...
document.addEventListener('click', e=>{
  ...
  if (likeBtn){
    ...
    // 迷你雷达卡路径：不弹成功 toast，只保留频率限制 / 错误提示
    toggleConditionLike(modelId, conditionId, { suppressSuccessToast: true });
    return;
  }

  // Lighting like button（来自迷你雷达卡，无 stopPropagation）
  const lightingLikeBtn = safeClosest(e.target, '.fc-radar-lighting-like-btn');
  if (lightingLikeBtn){
    const modelId = lightingLikeBtn.dataset.modelId;
    if (!modelId) { showError('缺少型号标识'); return; }
    // 迷你雷达卡路径：不弹成功 toast
    toggleLightingLike(modelId, { suppressSuccessToast: true });
    return;
  }
...
(function initRadarActionsBar(){
  ...
  const resetColorsBtn = document.getElementById('resetRadarColors');
  ...
  if (resetColorsBtn) {
    resetColorsBtn.addEventListener('click', () => {
      if (window.ColorManager && typeof window.ColorManager.reassignModelColors === 'function') {
        const modelIds = Array.from(RadarState.getModels().keys());
        window.ColorManager.reassignModelColors(modelIds);
        syncRadarOverview();
        if (lastChartData) postChartData(lastChartData);
      }
      collapse();
      typeof showSuccess === 'function' && showSuccess('已重置配色');
    });
  }
...
document.addEventListener('click', (e) => {
  ...
  const source = addBtn.dataset.radarSource || 'unknown';
  if (!mid) return;
  if (RadarState.hasModel(mid)) {
    if (typeof removeModelFromRadar === 'function') removeModelFromRadar(mid, { source });
  } else {
    if (typeof addModelToRadar === 'function') addModelToRadar(mid, brand, label, { source });
  }
});
...
/* =========================================================
   scheduleAdjust (retained as no-op for call-site compatibility)
   ========================================================= */
...
(function initLocalSelectionBoot(){
  ...
  applySidebarColors();
  // Only trigger the early chart render if there is no saved radar state pending
  // restore.  When radar models exist in localStorage, restoreRadarFromStorage()
  // will reconcile LocalState.selected with the restored RadarState and then call
  // refreshChartFromLocal via syncFromRadarState('restore').  Firing it here too
  // would create a race where curves rendered from stale fc_selected_v2 entries
  // are immediately overwritten (or left visible while conditions are still
  // inactive), causing a brief/persistent inconsistency between the condition UI
  // and the chart.  restoreRadarFromStorage() also handles the edge-case where
  // all saved models were removed as invalid by syncPageStateWithBackend.
  if (!(window.LocalState && LocalState.getRadarModels().length > 0)) {
...
initSnapTabScrolling({
  ...
  group: 'sidebar-top',
  persistKey: null,
  skipInactivePanes: true,
  onActiveChange: (tab)=> {
    if (tab === 'recent-liked') loadRecentLikesIfNeeded();
    if (tab === 'file-share') loadFileShareIfNeeded();
    flushPendingSidebarTopRender();
  }
});
initSnapTabScrolling({
...
window.__APP.modules = {
  ...
  rankings: {
    reloadTopRatings: function(){},
    loadLikesIfNeeded: function(){}
  },
  state: {
...
(function initGlobalTooltip(){
  const MARGIN = 8;
  const INFO_TIP_MAX_W = 380;  // 说明型 tooltip 最大宽度（优先保证可读性）
  ...
  // 交互型 tooltip"按类型全局只弹一次"的持久化：tab 会话内有效，关闭 tab 后重置
  const ITIP_SHOWN_SS_KEY = 'fc_interactive_tips_shown';
  let _shownInteractiveTips = null;
  function _getShownTips(){
    if (_shownInteractiveTips) return _shownInteractiveTips;
    try {
      const raw = sessionStorage.getItem(ITIP_SHOWN_SS_KEY);
      _shownInteractiveTips = raw ? new Set(JSON.parse(raw)) : new Set();
    } catch(_){ _shownInteractiveTips = new Set(); }
    return _shownInteractiveTips;
  }
  function _markTipShown(txt){
    const s = _getShownTips();
    s.add(txt);
    try { sessionStorage.setItem(ITIP_SHOWN_SS_KEY, JSON.stringify([...s])); } catch(_){}
  }
  function _isTipShown(txt){ return _getShownTips().has(txt); }

  let tip = null, currAnchor = null;
  let hideTimer = null, autoHideTimer = null, showTimer = null;
  // 触屏说明型 tooltip 的当前钉住锚点（tap-to-toggle 模式；null 表示当前无已钉住的说明型 tooltip）
  let touchOpenAnchor = null;

  function ensureTip(){
    if (tip) return tip;
    tip = document.createElement('div');
    tip.id = 'appTooltip';
    // 开启长文本自动换行（maxWidth 由 placeAround 按类型动态设置；CSS 中 #appTooltip 的 max-width 提供默认回退值）
  ...
  // 说明型 tooltip：元素自身无点击交互功能，仅作信息展示（带 fc-tip-info 标记）
  const isInfoTooltip = (el) => !!(el && el.classList.contains('fc-tip-info'));

  function measureTooltip(t){
    t.dataset.measuring = '1';
    t.style.left = '-9999px';
    t.style.top  = '-9999px';
    return { tw: t.offsetWidth, th: t.offsetHeight };
  }

  function resolveVerticalPlacement(preferred, rect, th, vh){
  ...
  function placeInfoTooltip(anchor, preferred){
    const t = ensureTip();
    const rect = anchor.getBoundingClientRect();
    const vw = window.innerWidth, vh = window.innerHeight;
    const anchorCx = rect.left + rect.width / 2;
    const infoMaxWidth = Math.min(vw - MARGIN * 2, INFO_TIP_MAX_W);
    const alignMode = anchor && typeof anchor.getAttribute === 'function'
      ? String(anchor.getAttribute('data-tooltip-align') || '').trim()
      : '';

    t.dataset.infoMode = '1';
    t.style.maxWidth = infoMaxWidth + 'px';
    t.style.width = 'max-content';

    const { tw, th } = measureTooltip(t);
    const placement = resolveVerticalPlacement(preferred, rect, th, vh);
    const maxVisualLeft = vw - MARGIN - tw;
    const centeredLeft = Math.max(MARGIN, Math.min(maxVisualLeft, (vw - tw) / 2));
    let visualLeft;

    // 说明型 tooltip：先按视口安全区确定阅读宽度，再只用锚点位置决定盒子偏向哪一侧放置。
    if (alignMode === 'anchor') {
      visualLeft = Math.max(MARGIN, Math.min(maxVisualLeft, anchorCx - tw / 2));
    } else if (anchorCx <= vw * 0.33) {
      visualLeft = Math.max(MARGIN, Math.min(maxVisualLeft, rect.left));
    } else if (anchorCx >= vw * 0.67) {
      visualLeft = Math.max(MARGIN, Math.min(maxVisualLeft, rect.right - tw));
    } else {
    ...
    const caretPos = Math.max(
      CARET_EDGE_MARGIN,
      Math.min(tw - CARET_EDGE_MARGIN, anchorCx - visualLeft)
    );
    const top = placement === 'top'
      ? (rect.top - th - 10)
      : (rect.bottom + 10);

    t.style.setProperty('--tip-caret', caretPos + 'px');
    t.dataset.placement = placement;
    t.style.left = `${Math.round(visualLeft)}px`;
    t.style.top  = `${Math.round(top)}px`;
    t.removeAttribute('data-measuring');
  }

  // placeAround：定位 tooltip。说明型 tooltip 走独立的阅读宽度优先布局；
  // 交互型 tooltip 使用 left = 视觉左边界（配合 CSS translateY 动画，无水平位移过渡）。
  function placeAround(anchor, preferred='top', infoMode=false){
    if (infoMode) {
      placeInfoTooltip(anchor, preferred);
      return;
    }
    ...
    t.dataset.infoMode = '';
    t.style.maxWidth = '';  // 回退到 CSS 规则：#appTooltip { max-width: min(70vw,320px) }
    t.style.width = '';

    const { tw, th } = measureTooltip(t);
    const placement = resolveVerticalPlacement(preferred, rect, th, vh);

    // 水平定位：直接使用视觉左边界（CSS 不再有水平 translate，无飞入副作用）
    let visualLeft = anchorCx - tw / 2;
    visualLeft = Math.max(MARGIN, Math.min(vw - MARGIN - tw, visualLeft));

    t.style.removeProperty('--tip-caret');
  ...
  function show(anchor){
    ...
    const info = isInfoTooltip(anchor);

    // 交互型 tooltip：按类型全局只弹一次（tab 会话内有效）
    if (!info && _isTipShown(txt)) return;

    setText(txt);
    placeAround(anchor, anchor.getAttribute('data-tooltip-placement') || 'top', info);

    const t = ensureTip();
    // 强制 reflow：确保浏览器将 tooltip 定位到隐藏态后再开始过渡，避免位置突变导致飞入效果
    void t.offsetHeight;
    t.dataset.show = '1';
  ...
  // Hover/focus：进入即托管 title，再调度显示（桌面端路径，触屏跳过）
  document.addEventListener('mouseenter', (e) => {
    if (isTouch) return;
  ...
  // 触屏：按触发方式分层处理
  // - 说明型 tooltip（fc-tip-info）：tap 切换显示/隐藏，点击其他区域关闭，不自动消隐
  // - 交互型 tooltip：保留原有轻量短暂显示（1.2s 自动隐藏），不干扰主操作
  document.addEventListener('touchstart', (e) => {
    const el = safeClosest(e.target, TOOLTIP_ANCHOR_SELECTOR);

    if (!el) {
      // 点击了 tooltip 锚点以外的区域：关闭任何已钉住的说明型 tooltip
      if (touchOpenAnchor) {
        hide(true);
        touchOpenAnchor = null;
      }
      return;
    }
    ...
    if (isInfoTooltip(el)) {
      // 说明型：tap 同一锚点 = 关闭；tap 新锚点 = 打开（关闭旧的）
      if (touchOpenAnchor === el) {
        hide(true);
        touchOpenAnchor = null;
      } else {
        if (touchOpenAnchor) hide(true);
        touchOpenAnchor = el;
        show(el); // 不会设置 autoHideTimer
      }
    } else {
      // 交互型：先关闭任何钉住的说明型 tooltip，再以原有轻量方式显示
      if (touchOpenAnchor) {
        hide(true);
        touchOpenAnchor = null;
      }
      show(el); // 交互型 tooltip 保持 1.2s 自动消隐
    }
  }, { passive: true, capture: true });

  // 点击后处理
  // 桌面端：直接 hide（tooltip 类型已弹过一次的不再重复，无需 silent 状态机）
  // 触屏说明型：由 touchstart 管理状态，click 不再干预
  // 触屏交互型：让 touchstart 设置的 autoHideTimer 负责消隐
  document.addEventListener('click', (e) => {
    const el = safeClosest(e.target, TOOLTIP_ANCHOR_SELECTOR);

    if (!el) {
      // 点击了锚点外区域：关闭任何已钉住的说明型 tooltip
      if (touchOpenAnchor) {
        hide(true);
        touchOpenAnchor = null;
      }
    ...
    if (isTouch) {
      if (isInfoTooltip(el)) {
        // 触屏说明型：touchstart 已处理 toggle，click 无需额外动作
        return;
      }
      // 触屏交互型：让 touchstart 设置的 autoHideTimer 负责消隐
      cancelShow();
    ...
    // 桌面端：hide（交互型已弹过的不再重复，无需 silent 状态机）
    cancelShow();
    hide(true);
  }, true);
...
async function refreshChartFromLocal(showToast=false){
  ...
  try {
    ...
    const chartData = {
      ...
      series: data.series || [],
      baseline_overlays: data.baseline_overlays || {}
    };
...
function generateDarkGradient() {
  ...
  const h2Offset = 0 + Math.floor(Math.random() * 180); // 30~90 之间偏移
...
(function initAnnouncementBar(){
  ...
  const FETCH_REFRESH_MS   = 300000; // 每 5 分钟刷新（低频兜底，业务请求顺带检查为主）
  ...
  const MARQUEE_PAUSE_MS  = 3000; // pause at start and end of each scroll cycle
  const DRAG_RESUME_MS    = 5000; // resume marquee this many ms after drag ends
  const MIN_OVERFLOW_PX   = 4;   // minimum text overflow to activate marquee / allow drag
  ...
  // Fingerprint of the last known effective announcement state
  let knownFingerprint = null;
  // Timer for re-fetching at next_change_at (scheduled announcements)
  ...
  // Track closed announcement IDs (per session) — use a Set so closing one
  // doesn't block later new announcements from appearing.
  let closedIds = new Set();
  try {
    const saved = sessionStorage.getItem('announce_closed_ids');
    if (saved) closedIds = new Set(JSON.parse(saved));
  } catch(_) {}
  ...
  function frameMarquee(ts){
    ...
    const elapsed = ts - marqueeStartTs;
    const cycleMs = MARQUEE_PAUSE_MS + scrollDuration + MARQUEE_PAUSE_MS;
    const phase = elapsed % cycleMs;
    let x;
    if (phase < MARQUEE_PAUSE_MS) {
      x = 0;
    } else if (phase < MARQUEE_PAUSE_MS + scrollDuration) {
      x = -scrollDistance * (phase - MARQUEE_PAUSE_MS) / scrollDuration;
    } else {
      x = -scrollDistance;
    }
  ...
  function startMarqueeIfNeeded(){
    ...
    if (innerW <= boxW + MIN_OVERFLOW_PX) return;
  ...
  closeBtn.addEventListener('click', ()=>{
    ...
    if (id) {
      ...
      try { sessionStorage.setItem('announce_closed_ids', JSON.stringify([...closedIds])); } catch(_) {}
    }
  });
  ...
  async function fetchAnnouncements(){
    try {
      ...
      if (!resp.ok) return;
      const data = resp.data || {};
      let items = Array.isArray(data.active) ? data.active.slice()
                : Array.isArray(data.items)  ? data.items.slice()
                : [];

      // Update known fingerprint from this full fetch
      if (data.fingerprint) knownFingerprint = data.fingerprint;

      // Filter out individually closed announcement IDs; don't block everything
      if (closedIds.size > 0) {
        items = items.filter(i => !closedIds.has(String(i.id)));
      }
      ...
      // Schedule a precise re-fetch when the next scheduled announcement becomes active
      _scheduleNextChangeTimer(data.next_change_at);
    } catch(_){}
  }

  // Schedule a timer to re-fetch at next_change_at so scheduled announcements
  // become visible at the right time without relying only on the background poll.
  const MAX_SCHEDULE_WINDOW_MS = 7200000; // 2 hours: max lookahead for next_change_at timer
  function _scheduleNextChangeTimer(next_change_at){
    if (nextChangeTimer) { clearTimeout(nextChangeTimer); nextChangeTimer = null; }
    if (!next_change_at) return;
    try {
      const msUntil = new Date(next_change_at).getTime() - Date.now();
      if (msUntil > 0 && msUntil < MAX_SCHEDULE_WINDOW_MS) {
        nextChangeTimer = setTimeout(()=> {
          nextChangeTimer = null;
          fetchAnnouncements();
        }, msUntil + 2000); // +2 s buffer for backend cache refresh lag
      }
    } catch(_) {}
  }

  // Called by other modules after receiving any normal business API response.
  // If the announcement fingerprint in the piggybacked meta differs from the
  // last known fingerprint, trigger a full re-fetch of the announcement state.
  window._checkAnnouncementMeta = function(meta) {
    try {
      const am = meta && meta.announcement_meta;
      if (!am) return;
      const fp = am.fingerprint;
      if (!fp) return;
      if (fp !== knownFingerprint) {
        knownFingerprint = fp; // optimistically update to avoid duplicate fetches
        fetchAnnouncements();
      }
    } catch(_) {}
  };

    // 初次加载 + 周期低频刷新（业务请求顺带指纹检查为主要检测手段）
  fetchAnnouncements();
  setInterval(fetchAnnouncements, FETCH_REFRESH_MS);
  ...
  function onPointerDown(e){
    const isMarquee = textSpan.classList.contains('is-marquee');
    const pendingResume = !!dragResumeTimer;
    if (!isMarquee && !pendingResume) return;
    // Capture current offset before stopMarquee resets it, so re-drag is seamless
    const match = (textSpan.style.transform || '').match(/translateX\((-?[\d.]+)px\)/);
    const curX = match ? parseFloat(match[1]) : 0;
    if (dragResumeTimer) { clearTimeout(dragResumeTimer); dragResumeTimer = null; }
    stopMarquee();
    // Restore position so the text doesn't snap back to 0 on re-drag
    if (curX !== 0) textSpan.style.transform = `translateX(${curX}px)`;
  ...
  function onPointerUp(){
    if (!dragState) return;
    const canResume = dragState.minX < -MIN_OVERFLOW_PX; // text was long enough to marquee
    dragState = null;
