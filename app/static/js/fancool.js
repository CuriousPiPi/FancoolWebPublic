...
function syncRadarOverview(series) {
  ...
  RadarState.getModels().forEach((model, mid) => {
    ...
    if (radar) {
      // Prefer profile-specific scores when available (mirrors _radarItemsFromCache logic)
      const profile = typeof window.ScoreProfile !== 'undefined' ? window.ScoreProfile.getProfile() : 'med';
      const profData = radar.score_profiles && radar.score_profiles[profile];
      if (profData && profData.condition_scores) {
        Object.entries(profData.condition_scores).forEach(([cid, sc]) => {
          scores[Number(cid)] = sc;
        });
      } else if (radar.conditions && typeof radar.conditions === 'object' && !Array.isArray(radar.conditions)) {
        Object.entries(radar.conditions).forEach(([cid, sc]) => {
          scores[Number(cid)] = sc.score_total;
        });
      }
      // Resolve composite score for the current profile, with fallback to root value
      compositeScore = (profData && profData.composite_score != null)
        ? profData.composite_score
        : (radar.composite_score != null ? radar.composite_score : null);
    }
    modelList.push({
      modelId:  model.modelId,
      brand:    model.brand,
...
function _radarItemsFromCache(mid) {
  ...
  // Use profile-specific scores if available
  let items, score;
  if (typeof window.ScoreProfile !== 'undefined' && radarData.score_profiles) {
    const profile = window.ScoreProfile.getProfile();
    const profData = radarData.score_profiles[profile];
    if (profData && profData.condition_scores) {
      items = Object.entries(profData.condition_scores).map(([cid, sc]) => ({
        condition_id: Number(cid),
        score_total: sc != null ? sc : null,
      }));
      score = profData.composite_score != null ? profData.composite_score : null;
    }
  }
  if (!items) {
    items = Object.entries(radarData.conditions).map(([cid, sc]) => ({
      condition_id: Number(cid),
      score_total: sc && sc.score_total != null ? sc.score_total : null,
    }));
    score = radarData.composite_score != null ? radarData.composite_score : null;
  }
  return { items, score };
}
...
// Re-render recently liked cards and radar comparison chart when score profile changes
window.addEventListener('localstate:score_profile_changed', () => {
  // Re-sync radar comparison chart with profile-specific scores
  syncRadarOverview();

  if (_lastRecentLikesList) {
    const wrap = recentLikesListEl;
    if (wrap) {
      const modelGroups = new Map();
      _lastRecentLikesList.forEach(it => {
        const mid = it.model_id != null ? String(it.model_id) : null;
        if (!mid) return;
        if (!modelGroups.has(mid)) {
          modelGroups.set(mid, {
