 ...
 * If bvid is present: fetch poster via same-origin proxy, show the panel,
 * and wire up inline play-on-click with a smooth poster→iframe fade transition.
 * If bvid is absent: hide the panel so it takes no space.
 */
function _renderGalleryVideo(bvid) {
  ...
  // Load poster via the same-origin proxy (direct Bilibili CDN requests may fail due to hotlink/CSP protection).
  if (newPosterImg) {
    newPosterImg.src = `/api/bilibili/poster-proxy?bvid=${encodeURIComponent(bvid)}`;
    // If the proxy fails, hide the broken-image icon; the play button remains visible.
