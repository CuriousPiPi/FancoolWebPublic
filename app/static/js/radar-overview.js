...
window.RadarOverview = (function () {
  ...
  // RADAR_CIDS – canonical condition ID list, in CCW-from-UL slot order.
  // Slot semantics (6 slots): UL(0) → L(1) → LL(2) → LR(3) → R(4) → UR(5).
  // Sourced from window.APP_CONFIG.radarCids (injected by the backend template);
  // falls back to the default [1,10,7,8,3,2] for compatibility.
  //
  // This same CCW order is used for:
  //  - DOM overlay label positions (OVERLAY_ANGLES below)
  //  - ECharts series/indicator slot assignment (ECHARTS_CIDS)
  //  - Score overlay positions
  const RADAR_CIDS = (window.APP_CONFIG && window.APP_CONFIG.radarCids) || [1, 10, 7, 8, 3, 2];
  const _N_RADAR = RADAR_CIDS.length;

  // ECHARTS_CIDS – condition IDs for ECharts radar indicators and series values.
  //
  // ECharts 5 radar places slot i at angle (startAngle + i * 360/N) with a Y-flip,
  ...
  //
  // RADAR_CIDS is already in CCW order, so it maps directly to ECharts slots.
  // No reordering is needed: ECHARTS_CIDS === RADAR_CIDS.
  const ECHARTS_CIDS = RADAR_CIDS;

  const RADAR_MODEL_LIMIT = 8;

  // Angles (degrees) for DOM overlay positioning.
  // RADAR_CIDS[i] is placed at OVERLAY_ANGLES[i] (CCW from UL).
  // Formula: slot 0 = UL = 120°, each subsequent slot adds +360/N° (CCW stepping).
  // For N=6: [120°, 180°, −120°, −60°, 0°, 60°] (UL→L→LL→LR→R→UR).
  const OVERLAY_ANGLES = Array.from({length: _N_RADAR}, (_, i) => {
    let a = 120 + i * (360 / _N_RADAR);
    while (a > 180) a -= 360;
    return a;
  });
  ...
  // Indices of pure-horizontal slots (R=0° and L=180°) — these labels extend farther
  // horizontally.  Derived from OVERLAY_ANGLES so it stays correct for any canonical order.
  const SIDE_SLOT_INDICES = new Set(
    OVERLAY_ANGLES.reduce(function (acc, a, i) {
      if (Math.abs(Math.cos(a * Math.PI / 180)) > 0.99) acc.push(i);
      return acc;
    }, [])
  );
  const LEFT_SIDE_CONDITION_IDS = new Set(
    RADAR_CIDS.reduce(function (acc, cid, i) {
      if (Math.cos(OVERLAY_ANGLES[i] * Math.PI / 180) < 0) acc.push(Number(cid));
      return acc;
    }, [])
  );
  ...
  let _onColorChange     = null; // (modelId: string) => void – fires after a model's base colour changes

  // ---- colour picker state ----
  let _colorPickerEl     = null; // singleton popover element
  let _colorPickerMid    = null; // model ID the picker is currently open for
  let _colorPickerAnchorEl = null; // legend dot element the picker follows
  ...
  function _getWhistleLevelClass(value) {
    if (value < 2) return 'is-level-low';
    if (value < 4) return 'is-level-mid';
    return 'is-level-high';
  }

  function _resolveWhistleBadgeContext() {
    const effectiveMid = _effectiveHighlightId();
    const mid = effectiveMid != null ? String(effectiveMid) : (() => {
      const single = _getSingleVisibleModel();
      return single ? String(single.modelId) : null;
    })();
    if (!mid) return null;
    ...
    const value = Number(meta.whistle_value);
    const baselineCid = Number(meta.whistle_baseline_condition_id);
    ...
    return {
      baselineCid,
      value,
      tooltip: `进气遮挡啸叫等级：+${value.toFixed(1)} dB`,
      levelClass: _getWhistleLevelClass(value),
    };
  }
  ...
  function _isLightingLiked(mid) {
    return !!(window.LocalState &&
      window.LocalState.lightingLikes &&
      window.LocalState.lightingLikes.has(String(mid)));
  }
  ...
  function _buildOption() {
    ...
    return {
      radar: {
        ...
        shape: 'polygon',
        // startAngle: 120 → ECHARTS_CIDS[0] lands at 120° (UL).
        // ECharts traverses CCW: slot i is at (startAngle + i*360/N)° with Y-flip rendering.
        // ECHARTS_CIDS === RADAR_CIDS (both CCW), so each CID lands at its canonical slot.
        startAngle: 120,
        splitNumber: 4,
  ...
  function _buildLegendHtml() {
    ...
    return models.map(m => {
      ...
      const isLightingLiked = _isLightingLiked(m.modelId);
      return `<div class="radar-overview-legend-item${isHidden ? ' is-hidden' : ''}" data-legend-model-id="${mid}" title="${displayName}">
        <button type="button" class="radar-overview-legend-thumb thumb-${thumbState}${isLightingLiked ? ' thumb-lighting-liked' : ''}${isPersist ? ' is-persist-active' : ''}" data-legend-thumb-id="${mid}" aria-label="点赞工况 / 持久高亮模式" title="点赞工况 / 持久高亮模式"><span class="radar-overview-legend-thumb-icons" aria-hidden="true"><i class="fa-solid fa-thumbtack radar-overview-legend-thumb-pin"></i><i class="fa-solid fa-thumbs-up radar-overview-legend-thumb-like"></i></span></button>
        <span class="radar-overview-legend-toggle" role="button" tabindex="0" aria-label="${isHidden ? '显示' : '隐藏'} ${displayName}">
          <button type="button" class="radar-overview-legend-dot radar-color-dot" data-legend-color-id="${mid}" style="background:${baseColor}" aria-label="更换 ${displayName} 的颜色" title="点击选择颜色"></button>
  ...
  function _buildOverlay() {
    ...
    RADAR_CIDS.forEach((cid, i) => {
      ...
      textSpan.className = 'radar-overlay-lbl-text';
      textSpan.textContent = _label(cid);
   ...
   * Mirrors radar config: center ['50%','52%'], radius '62%', startAngle: 120.
   *
   * Positioning model:
  ...
  function _positionOverlayLabels() {
    ...
    btns.forEach((btn) => {
      labelByCid.set(btn.dataset.cid, btn);
    });
    const overlayRect = _overlayEl.getBoundingClientRect();
    _overlayEl.querySelectorAll('.radar-overlay-whistle').forEach((badge) => {
      const labelEl = labelByCid.get(String(badge.dataset.cid || ''));
      if (!labelEl || !overlayRect.width || !overlayRect.height) return;
      const labelRect = labelEl.getBoundingClientRect();
      if (!labelRect.width || !labelRect.height) return;
      const isLeft = badge.dataset.badgeSide === 'left';
      const edgeX = isLeft
        ? (labelRect.left - overlayRect.left)
        : (labelRect.right - overlayRect.left);
      const centerY = (labelRect.top - overlayRect.top) + (labelRect.height / 2);
  ...
  function _refreshOverlayState() {
    ...
    const whistleCtx = _resolveWhistleBadgeContext();
    const whistleTooltip = (whistleCtx && typeof whistleCtx.tooltip === 'string') ? whistleCtx.tooltip : '';
    _overlayEl.querySelectorAll('.radar-overlay-whistle').forEach(badge => badge.remove());
    let whistleLabelText = '';
    let whistleLabelBtn = null;
    _overlayEl.querySelectorAll('.radar-overlay-lbl').forEach(btn => {
      const cid = btn.dataset.cid;
      const labelText = _label(Number(cid));
      const isWhistleBaseline = !!(whistleCtx && Number(cid) === whistleCtx.baselineCid);
      btn.classList.toggle('is-active', activeConditions.has(cid));
      const textEl = btn.querySelector('.radar-overlay-lbl-text');
      if (textEl) textEl.textContent = labelText;
      if (isWhistleBaseline) {
        whistleLabelText = labelText;
        whistleLabelBtn = btn;
      }
      btn.setAttribute('aria-label', (isWhistleBaseline && whistleTooltip) ? `${labelText}，${whistleTooltip}` : labelText);
    });
    if (whistleCtx && whistleLabelText && whistleTooltip) {
      const badge = document.createElement('span');
      badge.className = `radar-overlay-whistle fc-tooltip-target fc-tip-info ${whistleCtx.levelClass}`;
      badge.textContent = '啸叫';
      badge.setAttribute('data-tooltip', whistleTooltip);
      badge.setAttribute('data-tooltip-align', 'anchor');
      badge.setAttribute('aria-hidden', 'true');
      badge.setAttribute('title', whistleTooltip);
      badge.dataset.cid = String(whistleCtx.baselineCid);
      badge.dataset.badgeSide = LEFT_SIDE_CONDITION_IDS.has(Number(whistleCtx.baselineCid)) ? 'left' : 'right';
      _overlayEl.appendChild(badge);
      if (whistleLabelBtn) {
        const overlayRect = _overlayEl.getBoundingClientRect();
        const labelRect = whistleLabelBtn.getBoundingClientRect();
        if (overlayRect.width && overlayRect.height && labelRect.width && labelRect.height) {
          const isLeft = badge.dataset.badgeSide === 'left';
          const edgeX = isLeft
            ? (labelRect.left - overlayRect.left)
            : (labelRect.right - overlayRect.left);
  ...
  function _showScoreOverlay(model, color) {
    ...
    const centerEl = _scoreOverlayEl.querySelector('.radar-score-center');
    if (centerEl) {
      const composite = model.compositeScore != null
        ? Math.round(Number(model.compositeScore))
        : null;
  ...
  function _refreshLegendThumbs() {
    ...
    legendEl.querySelectorAll('.radar-overview-legend-thumb').forEach(btn => {
      ...
      btn.classList.toggle('thumb-lighting-liked', _isLightingLiked(mid));
  ...
  /**
   * Show / hide / refresh the lighting-like strip above the radar chart.
   * The strip is rendered as a sibling div of the overlay within the radar wrapper.
   * It is visible when a model is highlighted from radar legend hover/persist mode
   * and that model supports RGB.
   */
  function _refreshLightingLikeStrip() {
    if (!_overlayEl) return;
    const wrap = _overlayEl.parentElement;
    if (!wrap) return;

    const existing = wrap.querySelector('.radar-lighting-like-strip');

    const rawStripModelId = _persistModelId !== null ? _persistModelId : _legendHoverModelId;
    if (rawStripModelId === null) {
      if (existing) existing.remove();
      return;
    }
    const stripModelId = String(rawStripModelId);

    // Check rgb_flags from model meta cache
    const metaCache = window.__modelMetaCache;
    const meta = metaCache && metaCache[stripModelId];
    const rgbFlags = meta ? (meta.rgb_flags || 0) : 0;
    const supportsRgb = !!rgbFlags;
    if (!supportsRgb) {
      if (existing) existing.remove();
      return;
    }
    const isLightingLiked = _isLightingLiked(stripModelId);

    let strip = existing;
    let likeBtn = strip && strip.querySelector('.fc-radar-lighting-like-btn');
    let isNewStrip = false;

    if (!strip || strip.dataset.modelId !== stripModelId || !likeBtn) {
      if (strip) strip.remove();
      strip = document.createElement('div');
      strip.className = 'radar-lighting-like-strip';
      strip.dataset.modelId = stripModelId;
      strip.addEventListener('click', e => { e.stopPropagation(); });

      const rgbTag = (typeof window.buildRgbTagEl === 'function')
        ? window.buildRgbTagEl({ large: true })
        : (() => {
            const tag = document.createElement('span');
            tag.className = 'rpv2-mi-rgb-tag fc-radar-rgb-tag';
            tag.textContent = 'RGB';
            return tag;
          })();
      ...
      likeBtn.type = 'button';
      likeBtn.className = 'fc-radar-lighting-like-btn radar-lighting-like-btn fc-tooltip-target';

      likeBtn.addEventListener('click', e => {
        e.stopPropagation();
        const midStr = likeBtn.dataset.modelId;
        if (!midStr) return;
        // 主雷达图路径：委托全局统一灯效点赞动作层（suppressSuccessToast=true 不弹成功提示）
        if (window.__APP && typeof window.__APP.toggleLightingLike === 'function') {
          window.__APP.toggleLightingLike(midStr, { suppressSuccessToast: true });
        }
      });

      // Keep DOM order aligned with visual order: thumb button (left) then RGB tag.
      strip.appendChild(likeBtn);
      strip.appendChild(rgbTag);
      wrap.appendChild(strip);
      isNewStrip = true;
    }

    strip.dataset.modelId = stripModelId;
    strip.classList.add('is-rgb-enabled');
    strip.classList.toggle('is-liked', isLightingLiked);
    likeBtn.setAttribute('data-model-id', stripModelId);
    likeBtn.setAttribute('data-tooltip', isLightingLiked ? '取消灯效点赞' : '点赞灯效');
    likeBtn.disabled = false;

    const apply = liked => {
      const active = !!liked;
      strip.classList.toggle('is-liked', active);
      likeBtn.classList.toggle('is-liked', active);
      likeBtn.setAttribute('data-tooltip', active ? '取消灯效点赞' : '点赞灯效');
      likeBtn.innerHTML = `<i class="fa-solid fa-thumbs-up ${active ? 'text-red-500' : 'text-gray-400'}"></i>`;
      _refreshLegendThumbs();
    };
    apply(isLightingLiked);

    const shouldPersist = _persistModelId !== null;
    if (isNewStrip && shouldPersist) {
      strip.classList.remove('has-persist');
      requestAnimationFrame(() => {
        if (strip.isConnected && strip.dataset.modelId === stripModelId) {
          strip.classList.add('has-persist');
        }
      });
    } else {
      strip.classList.toggle('has-persist', shouldPersist);
    }
  }
   ...
   * Delegates to the global unified action layer (window.__APP.toggleConditionLike)
   * which handles LocalState, backend, and syncs radar state via setModelLikeState.
   */
  function _toggleConditionLike(mid, cidStr) {
    if (window.__APP && typeof window.__APP.toggleConditionLike === 'function') {
      window.__APP.toggleConditionLike(String(mid), String(cidStr), { suppressSuccessToast: true });
    }
  }
  ...
  function _render() {
    ...
    if (!ecWrap) {
      ...
      contentEl.innerHTML =
        ...
        `<div class="radar-overview-hint">点击工况标签可查看性能曲线和噪音实录</div>` +
  ...
  /** Reposition open colour picker to stay anchored to its legend dot. */
  function _positionColorPicker() {
    if (!_colorPickerEl || !_colorPickerAnchorEl) return;
    if (!_colorPickerAnchorEl.isConnected) {
      _closeColorPicker();
      return;
    }
    const dotRect = _colorPickerAnchorEl.getBoundingClientRect();
    const top = window.scrollY + dotRect.bottom + 4;
    let left = window.scrollX + dotRect.left;

    // Clamp within viewport so the picker stays visible on narrow screens.
    const pickerW = _colorPickerEl.offsetWidth || 0;
    if (pickerW) {
      const margin = 6;
      const minLeft = window.scrollX + margin;
      const maxLeft = window.scrollX + window.innerWidth - pickerW - margin;
      left = Math.max(minLeft, Math.min(left, maxLeft));
    }

    _colorPickerEl.style.top = top + 'px';
    _colorPickerEl.style.left = left + 'px';
  }

  /** Close the colour picker popover and remove the global dismiss listener. */
  function _closeColorPicker() {
    if (_colorPickerAnchorEl) {
      _colorPickerAnchorEl.classList.remove('is-picker-open');
    }
    if (_colorPickerEl) {
      _colorPickerEl.remove();
      _colorPickerEl = null;
    }
    _colorPickerMid = null;
    _colorPickerAnchorEl = null;
    document.removeEventListener('click', _colorPickerDismiss, true);
    window.removeEventListener('resize', _positionColorPicker);
    window.removeEventListener('scroll', _positionColorPicker, true);
  }

  /** Global capture listener – closes picker when clicking outside. */
  function _colorPickerDismiss(e) {
    if (_colorPickerEl && !_colorPickerEl.contains(e.target)) {
      // Allow the click that opened the picker to complete first.
      const dot = e.target.closest('.radar-color-dot');
      if (dot && dot.dataset.legendColorId === _colorPickerMid) return;
      _closeColorPicker();
    }
  }

  /**
   * Open the colour picker popover anchored to `dotEl` for the given model.
   * @param {string} modelId
   * @param {HTMLElement} dotEl – the colour dot button that was clicked
   */
  function _openColorPicker(modelId, dotEl) {
    const mid = String(modelId); // normalise to string at entry
    // Toggle: clicking the same dot again closes the picker.
    if (_colorPickerMid === mid) {
      _closeColorPicker();
      return;
    }
    _closeColorPicker();
    ...
    const palette = (typeof window.ColorManager.getPaletteColors === 'function')
      ? window.ColorManager.getPaletteColors()
      : [];
    const currentIdx = (typeof window.ColorManager.getModelBaseColorIndex === 'function')
      ? window.ColorManager.getModelBaseColorIndex(mid)
      : null;

    // Map palette index → which model currently owns it (within active set).
    const activeIds = models.map(m => String(m.modelId));
    const slotOwner = {}; // { paletteIndex: modelId }
    activeIds.forEach(k => {
      const idx = (typeof window.ColorManager.getModelBaseColorIndex === 'function')
        ? window.ColorManager.getModelBaseColorIndex(k)
        : null;
      if (idx !== null) slotOwner[idx] = k;
    });

    // Build popover element.
    const picker = document.createElement('div');
    picker.className = 'radar-color-picker';
    picker.setAttribute('role', 'dialog');
    picker.setAttribute('aria-label', '选择颜色');

    const grid = document.createElement('div');
    grid.className = 'radar-color-picker-grid';

    palette.forEach((color, idx) => {
      const isCurrent  = idx === currentIdx;
      const ownerMid   = slotOwner[idx];
      const isOccupied = ownerMid && ownerMid !== mid;

      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'radar-color-swatch'
        + (isCurrent ? ' is-current' : '')
        + (isOccupied ? ' is-occupied' : '');
      btn.style.background = color;
      btn.dataset.paletteIndex = String(idx);
      btn.setAttribute('aria-label', isOccupied ? `颜色 ${idx + 1}（已分配）` : `颜色 ${idx + 1}`);

      btn.addEventListener('click', e => {
        e.stopPropagation();
        if (typeof window.ColorManager.setModelBaseColorIndex === 'function') {
          window.ColorManager.setModelBaseColorIndex(mid, idx, activeIds);
        }
        _closeColorPicker();
        _render();
        if (typeof _onColorChange === 'function') {
          try { _onColorChange(mid); } catch (err) {
            console.error('[RadarOverview] Color change callback failed:', err);
          }
        }
      });
    ...
    // Position near the dot button. Mount to body so it is not clipped by panel containers.
    document.body.appendChild(picker);
    _colorPickerEl = picker;
    _colorPickerAnchorEl = dotEl;
    _colorPickerAnchorEl.classList.add('is-picker-open');

    _positionColorPicker();

    // Dismiss on outside click (capture phase so stopPropagation from swatches works).
    document.addEventListener('click', _colorPickerDismiss, true);
    window.addEventListener('resize', _positionColorPicker);
    window.addEventListener('scroll', _positionColorPicker, true);
  }
  ...
  function _bindLegendEvents(contentEl, legendEl) {
    ...
    items.forEach(item => {
      ...
      const colorDotEl = item.querySelector('.radar-color-dot');

      // Color dot click: open/close the colour picker popover
      if (colorDotEl) {
        colorDotEl.addEventListener('click', e => {
          e.stopPropagation();
          _openColorPicker(modelId, colorDotEl);
        });
        colorDotEl.addEventListener('keydown', ev => {
  ...
  function update(modelList) {
    ...
    // Normalize colour assignments for the current active model set before rendering.
    // This ensures all models get unique palette slots (0-9) with no collisions.
    if (window.ColorManager && typeof window.ColorManager.normalizeActiveModelColors === 'function') {
      window.ColorManager.normalizeActiveModelColors(models.map(m => m.modelId));
    }
    // Close any open colour picker when the model list changes.
    _closeColorPicker();
    _render();
  }
  ...
  function onColorChange(fn)      { _onColorChange = fn; }
  ...
  return {
    mount(containerEl) {
      ...
      _bindToggle();
      _render();
      // Refresh lighting strip and legend thumbs only when a lighting-like changes
      document.addEventListener('fc:like-changed', (e) => {
        if (!e.detail || e.detail.conditionId !== '0') return;
        _refreshLightingLikeStrip();
        _refreshLegendThumbs();
      });
    },
    ...
    reverseHoverClear,
    setModelLikeState,
    /** Start onboarding breathing highlight on condition labels and hint. */
    startOnboarding() {
      if (!panelEl) return;
      if (collapsed) {
        collapsed = false;
      ...
      panelEl.classList.add('radar-onboarding-active');
      const reduceMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
      panelEl.scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth', block: 'start' });
    },
    /** Stop onboarding breathing highlight. */
    stopOnboarding() {
      if (!panelEl) return;
      panelEl.classList.remove('radar-onboarding-active');
    },
  };
})();
