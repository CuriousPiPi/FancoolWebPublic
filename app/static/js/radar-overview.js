...
window.RadarOverview = (function () {
  ...
  // ECHARTS_CIDS – condition ID list used for the ECharts radar indicators and series values.
  //
  // Root-cause note: ECharts 5 radar uses the formula
  //   angle_i = startAngle + i * (2π / N)
  // and renders to screen with a Y-flip:  x = cx + r·cos(θ),  y = cy − r·sin(θ).
  // The combination of increasing angle and the Y-flip results in COUNTER-CLOCKWISE
  // traversal on screen.  With startAngle = 120° the six slots land at:
  //   i=0 → 120° (UL),  i=1 → 180° (L),  i=2 → 240°=−120° (LL),
  //   i=3 → 300°=−60° (LR),  i=4 → 0° (R),  i=5 → 60° (UR).
  //
  // RADAR_CIDS follows a clockwise visual order (UL→UR→R→LR→LL→L), which is the
  // REVERSE of ECharts' CCW traversal order (UL→L→LL→LR→R→UR).  Using the same
  // array for both caused 4 of 6 polygon vertices to map to the wrong condition.
  // This is why changing startAngle alone had no visible effect: rotating a polygon
  // whose conditions are in the wrong relative order only moves the mismatch to a
  // different starting point – it cannot fix a CW↔CCW direction reversal.
  //
  // ECHARTS_CIDS re-orders the same 6 conditions to match ECharts' CCW slot sequence:
  //   slot 0 (UL,120°)→1,  slot 1 (L,180°)→10,  slot 2 (LL,−120°)→7,
  //   slot 3 (LR,−60°)→8,  slot 4 (R,0°)→3,     slot 5 (UR,60°)→2.
  ...
  // ---- Parameterized label geometry ----
  //
  // Condition labels use a two-step positioning model:
  //   1. Base radial distance: radarR + CONDITION_RADIUS_OFFSET_PX along the radar angle.
  //   2. Additional horizontal offset applied outward (away from centre):
  //      - SIDE_H_OFFSET_PX   for the left (i=5, 180°) and right (i=2, 0°) middle labels.
  //      - CORNER_H_OFFSET_PX for the four diagonal corner labels (UL/UR/LR/LL, i=0,1,3,4).
  //
  // The horizontal offset is always signed so it moves the label outward:
  //   left-side labels (cos < 0) shift further left (negative dx),
  //   right-side labels (cos > 0) shift further right (positive dx).
  //
  // This produces a hexagon-silhouette layout where:
  //   - L/R middle labels extend farther outward horizontally,
  //   - corner labels extend somewhat less far outward,
  //   - overall vertical footprint remains compact.

  const CONDITION_RADIUS_OFFSET_PX = 0; // base radial gap beyond radar polygon edge for condition labels
  const CORNER_H_OFFSET_PX         = 26; // extra horizontal outward shift for UL/UR/LR/LL corner labels
  const SIDE_H_OFFSET_PX           = 26; // extra horizontal outward shift for L/R middle labels

  // Slot indices for "side" (left/right middle) labels vs "corner" labels.
