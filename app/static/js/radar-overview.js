  ...
  function _buildLegendHtml() {
    ...
    return models.map(m => {
      ...
      return `<div class="radar-overview-legend-item${isHidden ? ' is-hidden' : ''}" data-legend-model-id="${mid}" title="${displayName}">
        <button type="button" class="radar-overview-legend-thumb thumb-${thumbState}${isLightingLiked ? ' thumb-lighting-liked' : ''}${isPersist ? ' is-persist-active' : ''}" data-legend-thumb-id="${mid}" aria-label="点赞 / 进入工况赞模式" title="点赞工况 / 持久高亮模式"><i class="fa-solid fa-thumbs-up"></i></button>
