...
(function initSidebarModule(){
  ...
  if (sidebar){
    // Perf: 折叠完成后让整块侧栏内容退出渲染。collapsed 只是
    // transform:translateX(-100%)，侧栏里成百上千个节点仍留在布局树中，
    // 任何强制同步重排（拖动拟合指针 / 分隔条等）都要遍历它们。
    const SIDEBAR_SKIP_DELAY_MS = 400; // > #sidebar 的 transform .25s 过渡
    let contentSkipTimer = null;
    function syncSidebarContentSkip(collapsed){
      clearTimeout(contentSkipTimer);
      if (collapsed){
        contentSkipTimer = setTimeout(()=>{
          if (sidebar.classList.contains('collapsed')) sidebar.classList.add('is-content-skipped');
        }, SIDEBAR_SKIP_DELAY_MS);
      } else if (sidebar.classList.contains('is-content-skipped')) {
        sidebar.classList.remove('is-content-skipped');
        // 内容重新进入布局后补测一次顶部页签可视高度
        requestAnimationFrame(()=>{
          if (typeof window.syncTopTabsViewportHeight === 'function') window.syncTopTabsViewportHeight();
        });
      }
      if (!collapsed){
        // 侧栏可见了，补渲染折叠期间被推迟的迷你雷达卡
        try { window.flushPendingSidebarTopRender?.(); } catch(_) {}
      }
    }

    let lastCollapsed = sidebar.classList.contains('collapsed');
      ...
      for (const m of muts){
        if (m.type === 'attributes' && m.attributeName === 'class'){
          ...
          if (nowCollapsed !== lastCollapsed){
            ...
            syncSidebarContentSkip(nowCollapsed);
          }
        }
      }
    });
    mo.observe(sidebar, { attributes:true });
    syncSidebarContentSkip(lastCollapsed);
  }
  ...
  if (resizer && sidebar && mainContent){
    let dragging=false, startX=0, startW=0, rafId=null, pendingW=null;
    ...
    function dragStart(clientX){
      ...
      // 通知 chart/sidebar 拖动性能抑制协议：拖动期间跳过可延后的昂贵工作
      if (window.ChartSidebarDragPerf && typeof window.ChartSidebarDragPerf.begin === 'function') {
        window.ChartSidebarDragPerf.begin();
      } else {
        // 兼容旧逻辑（chart-renderer 初始化之前）
        window.__dragInteractionActive = true;
      }
    ...
    function dragMove(clientX){
      ...
      // 记录最新宽度：原实现把首个 newW 闭包进 rAF，后续移动全被丢弃，宽度会滞后于光标
      pendingW = newW;
      if (rafId == null){
        rafId = requestAnimationFrame(()=>{
          rafId = null;
          if (pendingW != null) applyWidth(pendingW);
        });
      }
    }
    function dragEnd(){
          ...
          if (window.ChartSidebarDragPerf && typeof window.ChartSidebarDragPerf.end === 'function') {
            window.ChartSidebarDragPerf.end();
          } else {
            // 兼容旧逻辑（chart-renderer 初始化之前）
            window.__dragInteractionActive = false;
            try { window.dispatchEvent(new CustomEvent('fc:drag-interaction-end')); } catch(_) {}
          }
