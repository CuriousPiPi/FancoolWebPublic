(function initRightPanelV2() {
  ...
  function buildModelCells(item) {
    ...
    if (priceVal != null) subParts.push(`¥${priceVal}`);
  ...
  // Read labels at expand time so async preload has already finished.
  function buildExpandPanel(item) {
    ...
    const hasScores = RADAR_CIDS.some(cid => {
      const cs = getCondScoresForItem(item, RADAR_CIDS);
      const s = cs[cid];
  ...
  function buildSearchResultsRow(item, sortBy) {
    ...
    const expandTr = buildExpandRowEl(expandBtn, item, _isNarrowMode() ? narrowExpandColspan : SEARCH_EXPAND_COLSPAN_WIDE, () => buildSearchExpandPanel(item));
    return { mainTr: tr, expandTr };
  }
