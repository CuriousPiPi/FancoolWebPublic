(function initRightPanelV2() {
  ...
  function buildExtraInfoPanel(item, onHeightChange) {
    ...
    const panel = window.ModelDetailPanel.build(item, {
      ...
      loadThumb: _loadGalleryThumbUrl,
  ...
  function buildUnifiedExpandPanelController(item, metricType) {
    ...
    const extraPanel = buildExtraInfoPanel(item, notifyHeight);
  ...
  function wireExpandRow(expandBtn, mainTr, expandTd, expandWrap, expandTr, item, metricType, onOpen) {
    ...
    const ensurePanel = () => {
      ...
      controller = buildUnifiedExpandPanelController(item, metricType);
  ...
  function buildModelCells(item) {
    ...
    nameInner.textContent = [item.brand_name_zh, item.model_name].filter(Boolean).join(' ') || '—';
    if (item.max_speed != null) {
      const speedSpan = document.createElement('span');
      speedSpan.className = 'rpv2-max-speed';
      speedSpan.textContent = ` (${item.max_speed} RPM)`;
  ...
  function buildExpandRowEl(mainTr, expandBtn, item, colSpan, metricType) {
    ...
    wireExpandRow(expandBtn, mainTr, expandTd, expandWrap, expandTr, item, metricType, onOpen);
    return expandTr;
  }
  ...
  function buildRow(item, boardType) {
    ...
    const source = boardType === 'perf' ? 'ranking_perf_board' : 'ranking_heat_board';
    tr.appendChild(buildActionTd(item, source));
  ...
  /* -------------------------------------------------------
     型号列 touch-marquee — narrow mode only.
     Applies to all tables with .rpv2-col-narrow-info in narrow
     mode: leaderboard, search results, and recent-updates.
     Slides .rpv2-mi-name-inner immediately on touchstart;
     restores on touchend / touchcancel.
     Cancels if the user moves vertically (> 8 px) so list
     scrolling is not impeded.
     ------------------------------------------------------- */
  const _NARROW_TOUCH_VERT_CANCEL = 8; // px

  document.addEventListener('touchstart', (e) => {
    const td = window.safeClosest(e.target, 'td.rpv2-col-narrow-info');
    if (!td) return;
    const card = window.safeClosest(td, '.fc-right-card');
    const isNarrowMode = !!card && (
      card.classList.contains('rp-narrow') ||
      window.matchMedia('(max-width: 600px)').matches
    );
    if (!isNarrowMode) return;
    const inner = td.querySelector('.rpv2-mi-name-inner');
    if (!inner) return;
    const wrap = inner.parentElement; // .rpv2-mi-name
    const delta = inner.scrollWidth - wrap.clientWidth;
    if (delta <= 4) return;
    ...
    function onTouchMove(ev) {
      ...
      if (!t) return; // different finger — ignore
      if (Math.abs(t.clientY - startY) > _NARROW_TOUCH_VERT_CANCEL) {
        ended = true;
        cleanup();
        resetInner();
      }
    ...
    function onTouchEnd(ev) {
      if (ended) return;
      // Only act when the originating touch is in changedTouches
      let found = false;
      for (let i = 0; i < ev.changedTouches.length; i++) {
