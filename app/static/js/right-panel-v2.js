...
(function initRightPanelV2() {
  ...
  const EXPAND_TD_COLSPAN = 9;                  // rank+brand+model+size+rgb+price+narrow-info+score-heat+action
  const NARROW_BOARD_COLSPAN = 4;              // rank + 型号信息 + score-heat + action (narrow layout)
  const SEARCH_COMPOSITE_EXPAND_COLSPAN = 8;    // brand+model+size+rgb+price+narrow-info+score+action
  const RECENT_UPDATES_EXPAND_COLSPAN = 9;      // brand+model+size+rgb+price+narrow-info+score-heat+update-date+action
  ...
  /* -------------------------------------------------------
     ...
     re-synchronizes every expand-row td colspan in the rankings boards.
     Extracted to module scope so it can be called after any async board
  ...
  function _applyRightPanelNarrowLayout() {
    ...
    // Keep expand-row td colspan in sync with visible column count so that
    // table-layout:fixed never allocates ghost slots for the 5 hidden columns.
    const targetColspan = String(isNarrow ? NARROW_BOARD_COLSPAN : EXPAND_TD_COLSPAN);
    card.querySelectorAll('.rpv2-board .rpv2-expand-td').forEach(td => {
      td.setAttribute('colspan', targetColspan);
    });
  }
  ...
  /* -------------------------------------------------------
     ...
     Also updates the colspan of ranking-board expand rows to match
     the actual visible column count (4 in narrow, 9 in wide) so that
     table-layout:fixed never sees phantom ghost columns that could steal
     width from the 型号信息 column.
     -------------------------------------------------------- */
