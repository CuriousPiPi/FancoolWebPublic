...
# ABC Score Config
# =========================================
SCORE_ALPHA_A = float(os.getenv('SCORE_ALPHA_A', '1.0'))
SCORE_ALPHA_B = float(os.getenv('SCORE_ALPHA_B', '0.6'))
SCORE_ALPHA_D = float(os.getenv('SCORE_ALPHA_D', '0.8'))
# Condition-score formula: 0.25*A1 + 0.25*A2 + 0.25*A3 + 0.20*B + 0.05*C + 0.0*D
# (default weights sum to 1.0; each is overridable via the corresponding env var)
# SCORE_WD defaults to 0.0 so D is inert unless explicitly weighted in.
SCORE_WA1 = float(os.getenv('SCORE_WA1', '0.30'))
SCORE_WA2 = float(os.getenv('SCORE_WA2', '0.00'))
SCORE_WA3 = float(os.getenv('SCORE_WA3', '0.45'))
SCORE_WB = float(os.getenv('SCORE_WB', '0.00'))
SCORE_WC = float(os.getenv('SCORE_WC', '0.05'))
SCORE_WD = float(os.getenv('SCORE_WD', '0.20'))
...
# Composite score weights per condition (default equal weight 1.0; configurable via COMPOSITE_W_<cid> env vars).
# composite_score = weighted average of score_total across present conditions.
COMPOSITE_WEIGHTS: Dict[int, float] = {
    cid: float(os.getenv(f'COMPOSITE_W_{cid}', '1.0'))
...
# Per-condition denominator cache (replaces condition_stats DB table).
# {cid: {'low_db': float|None, 'high_db': float|None, 'mid_db': float|None,
#         'max_a1': float, 'max_a2': float, 'max_a3': float,
#         'max_b': float, 'max_c': float, 'max_d': float, 'cached_at': float}}
_cond_denom_cache: Dict[int, dict] = {}
_cond_denom_lock = threading.Lock()

# Per-model ABC cache with soft/hard TTL.
# {model_id: {'conditions': {cid: {...scores...}}, 'updated_at': str, 'cached_at': float}}
_abc_cache: Dict[int, dict] = {}
_abc_cache_lock = threading.Lock()

# Inflight soft-refresh deduplication per model_id
...
def get_user_likes_full(user_identifier: str, limit: int | None = None) -> List[dict]:
    rows = fetch_all("""
        SELECT u.user_identifier, u.model_id, u.condition_id,
               ...
               COALESCE(fm.reference_price, 0) AS reference_price,
               COALESCE(fm.rgb_light, '') AS rgb_light
        FROM user_likes_view u
        LEFT JOIN fan_model fm ON fm.model_id = u.model_id AND fm.is_valid = 1
...
def _compute_abc_for_model(model_id: int) -> dict | None:
    """
    ...
    A1/A2/A3 each use the same A scoring function applied at the three adaptive
    same-dB comparison points (low_db, high_db, mid_db) determined per condition
    from the intersection of all models' raw noise dB ranges.
    ...
    for cid in SCORE_CONDITION_IDS:
        ...
        # A1/A2/A3: evaluate noise_to_airflow PCHIP at low_db, high_db, mid_db
        a1_raw = a2_raw = a3_raw = 0.0
        n2a = pchip_data.get('noise_to_airflow')
        if n2a and isinstance(n2a, dict) and n2a.get('x'):
            for db_pt, idx in [(low_db, 0), (high_db, 1), (mid_db, 2)]:
                try:
                    val = eval_pchip(n2a, db_pt)
                    if val is not None and math.isfinite(val) and val > 0:
                        if idx == 0: a1_raw = float(val)
                        elif idx == 1: a2_raw = float(val)
                        else: a3_raw = float(val)
                except Exception:
                    pass
        ...
        a1_scaled = pow(a1_raw, SCORE_ALPHA_A) if a1_raw > 0 else 0.0
        a2_scaled = pow(a2_raw, SCORE_ALPHA_A) if a2_raw > 0 else 0.0
        a3_scaled = pow(a3_raw, SCORE_ALPHA_A) if a3_raw > 0 else 0.0
        b_scaled  = pow(b_raw,  SCORE_ALPHA_B) if b_raw  > 0 else 0.0
        d_scaled  = pow(d_raw,  SCORE_ALPHA_D) if d_raw  > 0 else 0.0

        # Denominators (pow of raw max to match scaling)
        denom_a1 = pow(max_a1, SCORE_ALPHA_A) if max_a1 > 0 else 1.0
        denom_a2 = pow(max_a2, SCORE_ALPHA_A) if max_a2 > 0 else 1.0
        denom_a3 = pow(max_a3, SCORE_ALPHA_A) if max_a3 > 0 else 1.0
        denom_b  = pow(max_b,  SCORE_ALPHA_B) if max_b  > 0 else 1.0
        denom_c  = max_c if max_c > 0 else 1.0
        denom_d  = pow(max_d,  SCORE_ALPHA_D) if max_d  > 0 else 1.0
        ...
        score_total = round(
            SCORE_WA1 * score_a1_f +
            SCORE_WA2 * score_a2_f +
            SCORE_WA3 * score_a3_f +
            SCORE_WB  * score_b_f  +
            SCORE_WC  * score_c_f  +
            SCORE_WD  * score_d_f
        )
        ...
        result[cid] = {
            ...
            'score_a': round((SCORE_WA1 * score_a1_f + SCORE_WA2 * score_a2_f + SCORE_WA3 * score_a3_f)
                             / (SCORE_WA1 + SCORE_WA2 + SCORE_WA3) if (SCORE_WA1 + SCORE_WA2 + SCORE_WA3) > 0 else 0),
...
def _sync_compute_and_cache(model_id: int) -> dict | None:
    """Compute ABC for model_id, store in _abc_cache, and return the cache entry."""
    conditions = _compute_abc_for_model(model_id)
    ...
    with _abc_cache_lock:
        _abc_cache[model_id] = entry
    return entry


def _trigger_soft_refresh(model_id: int) -> None:
    """Trigger an async background recompute for model_id, skipping if already inflight."""
    with _abc_inflight_lock:
        if model_id in _abc_inflight:
            return
        _abc_inflight.add(model_id)
    ...
    def _refresh():
        try:
            ...
            with _abc_inflight_lock:
                _abc_inflight.discard(model_id)

    threading.Thread(target=_refresh, daemon=True).start()


def _get_abc_cached(model_id: int) -> dict | None:
    """
    Return ABC cache entry for model_id, applying soft/hard TTL logic.
    ...
    if entry is not None:
        ...
        if age < ABC_HARD_TTL_SEC:
            _trigger_soft_refresh(model_id)
            return entry
...
def _build_rankings_v2() -> dict:
    ...
    Each item shape:
      {
        ...
        condition_heat,   # {cid: heat} where heat = cond_query + cond_like * 10
      }
    """
    ...
    for mid, cid_map in cond_data.items():
        ...
        heat_score  = total_query + total_like * 10
        ...
        # Attach composite_score and per-condition scores from ABC cache
        abc_entry = _get_abc_cached(mid)
    ...
    # 性能榜: sort by composite_score DESC (None treated as -1), then heat_score DESC
    perf_board = []
    for i, row in enumerate(
        sorted(
            items,
            key=lambda x: (-(x['composite_score'] if x['composite_score'] is not None else -1), -x['heat_score'])
        )[:limit], 1
    ):
...
def get_rankings_v2() -> dict:
    ...
    with _rankings_v2_cache_lock:
        cached = _rankings_v2_cache.get('data')
    ...
    with _rankings_v2_cache_lock:
        existing = _rankings_v2_cache.get('data')
        if not existing or computed_at > existing[0]:
            _rankings_v2_cache['data'] = (computed_at, result)
...
def search_fans_by_condition_with_fit(condition_id=None, condition_name=None, sort_by='none', sort_value=None,
    ...
    sql = f"""
      SELECT g.model_id, g.condition_id,
             ...
             reference_price,
             (SELECT fm2.max_speed FROM fan_model fm2 WHERE fm2.model_id = g.model_id AND fm2.is_valid=1 LIMIT 1) AS max_speed,
...
def search_fans_composite(size_filter=None, thickness_min=None, thickness_max=None,
    ...
    sql = f"""
      SELECT g.model_id, g.brand_name_zh, g.model_name, g.size, g.thickness,
             (SELECT fm2.max_speed FROM fan_model fm2
              WHERE fm2.model_id = g.model_id AND fm2.is_valid=1 LIMIT 1) AS max_speed,
             reference_price,
             (SELECT fm2.rgb_light FROM fan_model fm2
              WHERE fm2.model_id = g.model_id AND fm2.is_valid=1 LIMIT 1) AS rgb_light
      FROM general_view g
      {"WHERE " + " AND ".join(where) if where else ""}
...
def api_models_by_brand():
    try:
        ...
        for row in rows:
            ...
            item = dict(row)
            cache_entry = _get_abc_cached(mid)
            if cache_entry:
                item['radar'] = {
    ...
    try:
        ...
        sql = f"""
          SELECT v.model_id, v.brand_name_zh, v.model_name,
                 COALESCE(v.reference_price, 0) AS reference_price,
    ...
    try:
        ...
        sql = f"""
          SELECT v.model_id, v.brand_name_zh, v.model_name,
                 ...
                 m.review
          FROM available_models_info_view v
          JOIN fan_model m ON m.model_id = v.model_id
          WHERE v.model_id IN ({placeholders})
        """
        rows = fetch_all(sql, params)
        valid_set: set = set()
        ...
        for model_id in model_ids:
            ...
            if entry is not None:
                ...
                if age < ABC_SOFT_TTL_SEC:
                    ...
                    new_entry = _sync_compute_and_cache(model_id)
                    if new_entry:
                        models_out[str(model_id)] = {
                            'conditions': new_entry['conditions'],
                            'composite_score': new_entry.get('composite_score'),
...
def api_internal_warm_scores():
    ...
    try:
        ...
        def _warm():
            ...
            # Recompute ABC for this model and refresh in-memory cache
            try:
                _sync_compute_and_cache(model_id)
            except Exception as e:
                app.logger.warning('[warm_scores] abc recompute (%s): %s', model_id, e)
...
def update_query_count():
    ...
    while True:
        try:
            result = fetch_all("SELECT COUNT(DISTINCT batch_id) AS c FROM user_query_logs")
        ...
        try:
            ...
            with _cond_denom_lock:
                for r in rows:
                    ...
                    entry = _cond_denom_cache.get(cid)
...
def _do_denom_refresh():
    """Rebuild per-condition denominators for all SCORE_CONDITION_IDS.
    ...
    Then computes A-score denominators at each of the three adaptive dB points:
      max_a1: max airflow at low_db  across all models per condition
      max_a2: max airflow at high_db across all models per condition
    ...
    Also computes:
      ...
      max_d: max(max_airflow / noise_dB_at_max_airflow) per condition from noise_to_airflow PCHIP

    If low_db > high_db for a condition, that condition's low_db/high_db are
    stored as-is (with invalid flag implied by low_db > high_db) so that
    _compute_abc_for_model can detect and skip it explicitly.

    Results are stored in _cond_denom_cache (in-memory, no DB writes).
    """
    try:
        cid_list = list(SCORE_CONDITION_IDS)
        parts = ', '.join([f':c{i}' for i in range(len(cid_list))])
        ...
        # Step 3: compute per-condition max airflow at each of the three dB points
        per_cid_a1: Dict[int, float] = defaultdict(float)
        per_cid_a2: Dict[int, float] = defaultdict(float)
        ...
        for cid in cid_list:
            ...
            if low_db is None or high_db is None or low_db > high_db:
                continue
            for _, n2a in per_cid_n2a.get(cid, []):
                for db_pt, per_cid_ax in [
                    (low_db,  per_cid_a1),
                    (high_db, per_cid_a2),
                    (mid_db,  per_cid_a3),
                ]:
                    try:
                        val = eval_pchip(n2a, db_pt)
                        if val is not None and math.isfinite(val) and val > 0:
                            per_cid_ax[cid] = max(per_cid_ax[cid], float(val))
                    except Exception:
                        pass

        # B maxima: max(airflow_cfm) per condition
        ...
        # C maxima: log1p(max(like_count)) per condition
            ...
            for _, n2a in per_cid_n2a.get(cid, []):
                ...
                if n2a_x and n2a_y and len(n2a_x) == len(n2a_y):
                    try:
                        ...
                        if max_af > 0 and noise_at_max > 0:
                            d_raw = max_af / noise_at_max
                            per_cid_d[cid] = max(per_cid_d[cid], d_raw)
                    except Exception:
                        pass
        ...
        with _cond_denom_lock:
            for cid in cid_list:
                _cond_denom_cache[cid] = {
                    'low_db':  per_cid_low_db.get(cid),
                    'high_db': per_cid_high_db.get(cid),
                    'mid_db':  per_cid_mid_db.get(cid),
                    'max_a1':  per_cid_a1.get(cid, 0.0),
                    'max_a2':  per_cid_a2.get(cid, 0.0),
        ...
        if app.debug:
            app.logger.debug('[denom_refresh] completed for %d conditions', len(cid_list))
    except Exception as e:
