...
SCORE_WA = float(os.getenv('SCORE_WA', '0.60'))
SCORE_WB = float(os.getenv('SCORE_WB', '0.35'))
...
# {cid: {'max_a': float, 'max_b': float, 'max_c': float, 'cached_at': float}}
...
def _a_max_for_n2a(n2a: dict) -> float:
    """Compute the A-score numerator (airflow CFM) for a noise_to_airflow PCHIP curve.

    Search-rule alignment:
      - If model's max noise > 30 dB: evaluate the PCHIP at 30.0 dB.
        Falls back to max(y) on the curve if evaluation fails.
      - Else (max noise <= 30 dB): use the maximum airflow value on the curve.
        Falls back to eval_pchip(n2a, 30.0) if the max-y extraction fails.

    Returns 0.0 on any unrecoverable error; never returns a non-positive value.
    """
    xs = n2a.get('x') or []
    ys = n2a.get('y') or []
    try:
        max_noise = float(max(xs))
    except Exception as e:
        if app.debug:
            app.logger.debug('[a_max] could not determine max_noise (xs=%r): %s', xs, e)
        return 0.0

    if max_noise > 30.0:
        # Primary: PCHIP eval at 30 dB
        try:
            val = eval_pchip(n2a, 30.0)
            if val is not None and math.isfinite(val) and val > 0:
                if app.debug:
                    app.logger.debug('[a_max] max_noise=%.2f>30 → eval@30=%.4f', max_noise, val)
                return float(val)
        except Exception as e:
            if app.debug:
                app.logger.debug('[a_max] max_noise=%.2f>30 eval@30 raised: %s', max_noise, e)
        # Fallback: curve max
        try:
            val = float(max(ys))
            if math.isfinite(val) and val > 0:
                if app.debug:
                    app.logger.debug('[a_max] max_noise=%.2f>30 eval@30 failed → curve_max=%.4f', max_noise, val)
                return val
        except Exception as e:
            if app.debug:
                app.logger.debug('[a_max] max_noise=%.2f>30 curve_max also failed: %s', max_noise, e)
    else:
        # Primary: max airflow on curve
        try:
            val = float(max(ys))
            if math.isfinite(val) and val > 0:
                if app.debug:
                    app.logger.debug('[a_max] max_noise=%.2f<=30 → curve_max=%.4f', max_noise, val)
                return val
        except Exception as e:
            if app.debug:
                app.logger.debug('[a_max] max_noise=%.2f<=30 curve_max raised: %s', max_noise, e)
        # Fallback: PCHIP eval at 30 dB
        try:
            val = eval_pchip(n2a, 30.0)
            if val is not None and math.isfinite(val) and val > 0:
                if app.debug:
                    app.logger.debug('[a_max] max_noise=%.2f<=30 curve_max failed → eval@30=%.4f', max_noise, val)
                return float(val)
        except Exception as e:
            if app.debug:
                app.logger.debug('[a_max] max_noise=%.2f<=30 eval@30 also failed: %s', max_noise, e)
    if app.debug:
        app.logger.debug('[a_max] max_noise=%.2f all paths failed, returning 0.0', max_noise)
    return 0.0
...
def _compute_abc_for_model(model_id: int) -> dict | None:
    """
    Compute A/B/C scores for model_id across all SCORE_CONDITION_IDS.
    ...
    Returns {cid: {score_a, score_b, score_c, score_total, a_raw, b_raw, c_raw}}
    ...
    for cid in SCORE_CONDITION_IDS:
        ...
        # A: airflow at 30 dB noise (noise_to_airflow PCHIP at 30.0)
        a_raw = 0.0
        ...
        max_a = float(denom_entry.get('max_a') or 0.0) if denom_entry else 0.0
        ...
        # Scale: pow for A and B; C is already log-scaled
        a_scaled = pow(a_raw, SCORE_ALPHA_A) if a_raw > 0 else 0.0
        b_scaled = pow(b_raw, SCORE_ALPHA_B) if b_raw > 0 else 0.0

        # Denominators (use pow of raw max for A and B to match scaling)
        denom_a = pow(max_a, SCORE_ALPHA_A) if max_a > 0 else 1.0
        denom_b = pow(max_b, SCORE_ALPHA_B) if max_b > 0 else 1.0
        denom_c = max_c if max_c > 0 else 1.0

        na = min(1.0, a_scaled / denom_a)
        nb = min(1.0, b_scaled / denom_b)
        nc = min(1.0, c_raw / denom_c)
        # min(1.0, ...) caps scores at 100 when denominators are temporarily stale.

        score_a_f = na * SCORE_MAX
        score_b_f = nb * SCORE_MAX
        score_c_f = nc * SCORE_MAX
        score_total = round(SCORE_WA * score_a_f + SCORE_WB * score_b_f + SCORE_WC * score_c_f)

        result[cid] = {
            'score_a': round(score_a_f),
...
def _do_denom_refresh():
    ...
    Scans all (model_id, condition_id) pairs in general_view to find:
      max_a: max A-raw (airflow @ 30 dB) across all models per condition.
    ...
    try:
        ...
        # A maxima: rebuild PCHIP for all pairs and collect per-condition max
        per_cid_a: Dict[int, float] = defaultdict(float)
        if pairs:
            try:
            ...
            for mid, cid in pairs:
                ...
                if mdl:
                    ...
                    if n2a and isinstance(n2a, dict) and n2a.get('x'):
                        val = _a_max_for_n2a(n2a)
                        if val > 0:
                            per_cid_a[cid] = max(per_cid_a[cid], val)
