...
SCORING_PROFILES = scoring_system.SCORING_PROFILES
_DEFAULT_SCORE_PROFILE = scoring_system._DEFAULT_SCORE_PROFILE
SCORE_CONDITION_IDS = scoring_system.SCORE_CONDITION_IDS
COMPOSITE_WEIGHTS = scoring_system.COMPOSITE_WEIGHTS
PROFILE_CONFIGS = scoring_system.PROFILE_CONFIGS
...
_get_abc_all_profiles = scoring_system._get_abc_all_profiles
_get_abc_cached = scoring_system._get_abc_cached
_build_score_profiles_payload = scoring_system._build_score_profiles_payload
_sync_compute_and_cache = scoring_system._sync_compute_and_cache
_trigger_soft_refresh = scoring_system._trigger_soft_refresh
...
ABC_SOFT_TTL_SEC = scoring_system.ABC_SOFT_TTL_SEC
ABC_HARD_TTL_SEC = scoring_system.ABC_HARD_TTL_SEC
...
def api_search_fans():
    try:
        ...
        # This avoids borrowing from a specific profile's rankings cache and makes the
        # search results resilient to cold-start windows where some profiles are not yet warm.
        _cf = get_canonical_facts()
        facts_lookup: dict = _cf.get('model_lookup') or {}
        # Preload like snapshot once for all result rows to avoid per-(model_id,cid) DB
        # queries on ABC cache misses.  lrc_snapshot is set when canonical facts were
        # freshly built; otherwise reuse the like_rank_cache in-proc snapshot (cheap).
        _prefetched_likes: dict | None = _cf.get('lrc_snapshot')
        if _prefetched_likes is None:
            try:
                like_rank_cache.ensure_cache(fetch_all)
                _prefetched_likes = like_rank_cache.get_all_like_counts()
            except Exception as _e:
                app.logger.warning('[api_search_fans] like cache load failed: %s', _e)
        ...
        if composite_mode:
            results.sort(
                key=lambda x: (x['composite_score'] if x['composite_score'] is not None else -1, x['model_id']),
                reverse=True
            )
...
def api_radar_metrics():
    ...
    try:
        ...
        # Pre-load the full like cache once so that any synchronous recomputes below
        # can use it as prefetched_likes instead of issuing per-condition DB queries.
        try:
            like_rank_cache.ensure_cache(fetch_all)
            _radar_prefetched_likes: dict | None = like_rank_cache.get_all_like_counts()
        except Exception:
            _radar_prefetched_likes = None

        now = time.time()
        for model_id in model_ids:
            # Pass prefetched_likes so that any cache-miss recompute triggered
            # inside _get_abc_cached avoids per-condition DB queries.
            all_profiles = _get_abc_all_profiles(model_id, _radar_prefetched_likes)
            ...
            if entry is not None:
                ...
                if age < ABC_SOFT_TTL_SEC:
                    ...
                    for pkey in SCORING_PROFILES:
                        _sync_compute_and_cache(model_id, pkey, _radar_prefetched_likes)
                    all_profiles = _get_abc_all_profiles(model_id, _radar_prefetched_likes)
                    entry = all_profiles.get(_DEFAULT_SCORE_PROFILE)
                if entry:
...
def api_internal_warmup():
    ...
    Triggers in a background thread:
      1. rankings_v2 for all scoring profiles (low/med/hi)
      2. ABC cache flush to shared disk
    ...
    """
    try:
        expected_token = os.getenv('INTERNAL_WARMUP_TOKEN')
        if expected_token:
            supplied = request.headers.get('X-Warmup-Token', '')
            if supplied != expected_token:
                return resp_err('UNAUTHORIZED', 'Invalid or missing X-Warmup-Token', 401)
        _warmup_rankings_async()
        return resp_ok({'status': 'warming_up', 'profiles': list(SCORING_PROFILES)})
    except Exception as e:
        app.logger.exception(e)
        return resp_err('INTERNAL_ERROR', str(e), 500)
...
def api_score_rule_explain():
    """Return per-condition scoring rule details for a given score profile.

    Query params:
      score_profile  'low' | 'med' | 'hi'  (default: 'med')

    Response data:
      score_profile     — the requested profile key
      alpha             — {A, B, D} exponent values for the profile
      generic_formulas  — {dim_key: formula_str} generic formula per dimension type
                          (ALPHA already substituted; condition-specific values shown as placeholders)
      conditions        — list of per-condition rule objects
      model_labels      — {model_id: 'model_name'} display strings (brand excluded)
    """
    try:
        profile = (request.args.get('score_profile') or 'med').strip().lower()
        if profile not in SCORING_PROFILES:
            return resp_err('INVALID_PROFILE', '无效的评分策略', 400)
        ...
        # Snapshot the in-memory denominator cache (thread-safe copy)
        with _cond_denom_lock:
            profile_cache = {
                cid: dict(entry)
                for cid, entry in _cond_denom_cache.get(profile, {}).items()
            }

        cid_list = list(_RADAR_CIDS)

        # Fetch condition display names from DB
        ...
        try:
            ...
            cond_names = {int(r['condition_id']): r['condition_name_zh'] for r in cond_rows}
        except Exception:
            cond_names = {}

        # The WA2 anchor model ID for this profile (0 = no clamping)
        wa2_anchor_model_id: int = pcfg.get('wa2_anchor', 0)

        # Collect all model IDs referenced across dimensions so we can look up names
        model_ids: set = set()
        for entry in profile_cache.values():
            for key in ('model_id_a1', 'model_id_a2', 'model_id_a3',
                        'model_id_b', 'model_id_c', 'model_id_d',
                        'ref_db_model_id_a1', 'ref_db_model_id_a2'):
                mid = entry.get(key)
                if mid is not None:
                    model_ids.add(int(mid))
        # Also include the WA2 anchor model so the frontend can label it in footnotes
        if wa2_anchor_model_id:
            model_ids.add(wa2_anchor_model_id)

        # Return only model_name (no brand) so the table stays compact
        model_labels: dict = {}
        if model_ids:
            m_list = sorted(model_ids)
            m_parts = ', '.join([f':m{i}' for i in range(len(m_list))])
            m_params = {f'm{i}': mid for i, mid in enumerate(m_list)}
            try:
                m_rows = fetch_all(
                    f"SELECT model_id, model_name FROM fan_model "
                    f"WHERE model_id IN ({m_parts})",
                    m_params,
                )
                for r in m_rows:
                    model_labels[str(r['model_id'])] = r['model_name']
            except Exception:
                pass

        # Generic formulas per dimension type — ALPHA substituted, condition-specific
        # values shown as descriptive placeholders so the formula is the same for all conditions.
        # WA1/WA2/WA3 share the same normalisation structure; only the reference dB threshold differs.
        _wa_formula = f'min(100, (参考分贝处风量 ÷ 满分风量)^{alpha_a:.4g} × 100)'
        generic_formulas = {
            'wa1': _wa_formula,
            'wa2': _wa_formula,
            'wa3': _wa_formula,
            'wb':  f'min(100, (最大风量 ÷ 满分风量)^{alpha_b:.4g} × 100)',
            'wc':  'min(100, ln(1+点赞数) ÷ ln(1+满分点赞数) × 100)',
            'wd':  f'min(100, (风量/噪音比 ÷ 满分风噪比)^{alpha_d:.4g} × 100)',
        }
        ...
        conditions = []
        for cid in cid_list:
            entry = profile_cache.get(cid) or {}
            # Skip conditions that have no cache entry at all (denominator refresh not yet run)
            if not entry:
                continue
            cond_name = cond_names.get(cid, f'工况 {cid}')

            low_db  = entry.get('low_db')
            high_db = entry.get('high_db')
            mid_db  = entry.get('mid_db')
            if mid_db is None and low_db is not None and high_db is not None:
                try:
                    mid_db = (float(low_db) + float(high_db)) / 2.0
                except Exception:
                    pass
            # Use None (not 0) when values are absent/zero, to avoid displaying misleading 0.0
            raw_a1     = entry.get('max_a1') or None
            raw_a2     = entry.get('max_a2') or None
            raw_a3     = entry.get('max_a3') or None
            raw_b      = entry.get('max_b') or None
            # wc_raw_max is the raw max like count (0 is a valid value, must not be treated as None).
            raw_wc     = entry['wc_raw_max'] if 'wc_raw_max' in entry else None
            raw_d      = entry.get('max_d') or None
                ...
                'dimensions': [
                    {
                        ...
                        'label':            '低噪点风量',
                        'ref_db':           _fmt(low_db),
                        'ref_value':        _fmt(raw_a1),
                        'weight':           pcfg['score_wa1'],
                        'model_id':         entry.get('model_id_a1'),
                        'ref_db_model_id':  entry.get('ref_db_model_id_a1'),
                    },
                    {
                        'key':              'wa2',
                        'label':            '高噪点风量',
                        'ref_db':           _fmt(high_db),
                        'ref_value':        _fmt(raw_a2),
                        'weight':           pcfg['score_wa2'],
                        'model_id':         entry.get('model_id_a2'),
                        'ref_db_model_id':  entry.get('ref_db_model_id_a2'),
                    },
                    {
                        'key':       'wa3',
                        'label':     '中段风量',
                        'ref_db':    _fmt(mid_db),
                        'ref_value': _fmt(raw_a3),
                        'weight':    pcfg['score_wa3'],
                        'model_id':  entry.get('model_id_a3'),
                    },
                    {
                        'key':       'wb',
                        'label':     '最大风量',
                        'ref_db':    None,
                        'ref_value': _fmt(raw_b),
                        'weight':    pcfg['score_wb'],
                        'model_id':  entry.get('model_id_b'),
                    },
                    {
                        'key':       'wc',
                        'label':     '最大点赞数',
                        'ref_db':    None,
                        'ref_value': None if raw_wc is None else raw_wc,
                        'weight':    pcfg['score_wc'],
                        'model_id':  entry.get('model_id_c'),
                    },
                    {
                        'key':       'wd',
                        'label':     '风噪比峰值',
                        'ref_db':    None,
                        'ref_value': _fmt(raw_d, 4),
                        'weight':    pcfg['score_wd'],
                        'model_id':  entry.get('model_id_d'),
                    },
                ],
            })

        # Build composite weights only for conditions that were actually returned,
        # preserving the same order so the frontend weight table aligns with the main table.
        returned_cids = [c['condition_id'] for c in conditions]

        return resp_ok({
            'score_profile':              profile,
            'alpha':                      {'A': alpha_a, 'B': alpha_b, 'D': alpha_d},
            'generic_formulas':           generic_formulas,
            'conditions':                 conditions,
            'model_labels':               model_labels,
            'wa2_anchor_model_id':        wa2_anchor_model_id,
            'condition_composite_weights': _build_condition_weights(returned_cids),
        })
    except Exception as e:
        app.logger.exception(e)
        return resp_err('INTERNAL_ERROR', str(e), 500)
