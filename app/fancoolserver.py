...
from flask import Flask, request, render_template, session, jsonify, make_response, send_from_directory, Response, abort, g
...
from app import cache_event_bus, cache_event_handlers
...
from app import purchase_links, scoring_system, user_activity, file_share
...
from app import issue_feedback
from app.issue_feedback import bp as issue_feedback_bp
...
def _load_search_condition_ids() -> list[int]:
    raw = (os.getenv('SEARCH_CONDITION_IDS') or '').strip()
    if not raw:
        return []
    ...
    for token in raw.split(','):
        ...
        try:
            ...
            app.logger.warning('Invalid SEARCH_CONDITION_IDS=%r; ignore search condition whitelist', raw)
            return []
        if cid <= 0:
            app.logger.warning('Invalid SEARCH_CONDITION_IDS=%r; ignore search condition whitelist', raw)
            return []
...
SEARCH_CONDITION_IDS = _load_search_condition_ids()
...
RECENT_LIKES_LIMIT = 30
CLICK_COOLDOWN_SECONDS = 0.5
RECENT_UPDATES_LIMIT = 100
SPECTRUM_DOCK_ENABLED = os.getenv('SPECTRUM_DOCK_ENABLED', '1') == '1'
PLAY_AUDIO_ENABLED = os.getenv('PLAY_AUDIO_ENABLED', '1') == '1'
APP_REQUIRE_UID_WHITELIST, APP_REQUIRE_PASSWORD = access_control.get_app_policy_from_env()
APP_LOGIN_MAX_ATTEMPTS = int(os.getenv('APP_LOGIN_MAX_ATTEMPTS', '5'))
APP_LOGIN_LOCK_MINUTES = int(os.getenv('APP_LOGIN_LOCK_MINUTES', '15'))
...
def _app_lock_until_str(minutes: int) -> str:
    return (datetime.now() + timedelta(minutes=minutes)).strftime('%Y-%m-%d %H:%M:%S')


def _app_password_required_response(message: str | None = None):
    msg = message or '当前页面需要用户名和密码验证。'
    if (request.path or '').startswith('/api/'):
        resp = resp_err('APP_PASSWORD_REQUIRED', msg, 401)
    else:
        uid = getattr(g, '_active_uid', None)
        resp = make_response(
            render_template('access_denied.html', user_identifier=uid, error_message=msg),
            401,
        )
    resp.headers['WWW-Authenticate'] = 'Basic realm="Fancool App Access"'
    return resp
...
def _model_has_sweep_payload(model: Dict[str, Any]) -> bool:
    if not isinstance(model, dict):
        return False
    if model.get('has_sweep') is False:
        return False
    if model.get('has_sweep') is True:
        return True
    return sweep_audio_player.validate_model_has_frame_index(model)


def _model_supports_spectrum_payload(model: Dict[str, Any]) -> bool:
    if not isinstance(model, dict):
        return False
    if model.get('supports_spectrum') is False:
        return False
    if model.get('supports_spectrum') is True:
        return True
    centers = model.get('centers_hz') or model.get('freq_hz') or model.get('freq') or []
    bands = model.get('band_models_pchip') or []
    return bool(isinstance(centers, list) and centers and isinstance(bands, list) and bands)


def _slim_spectrum_model_for_client(model: Dict[str, Any]) -> Dict[str, Any]:
    m = model or {}
    calib = m.get('calibration') or {}
    calib_model = calib.get('calib_model') or {}
    has_sweep = _model_has_sweep_payload(m)
    supports_spectrum = _model_supports_spectrum_payload(m)
    supports_audio = has_sweep and sweep_audio_player.validate_model_has_frame_index(m)
    centers = m.get('centers_hz') or m.get('freq_hz') or m.get('freq') or []
    bands = m.get('band_models_pchip') or []
    return {
        'version': m.get('version'),
        'model_kind': m.get('model_kind') or ('full_sweep_spectrum' if supports_spectrum else 'no_sweep_calibration'),
        'preview_mode': m.get('preview_mode') or ('full_sweep' if has_sweep else 'no_sweep'),
        'has_sweep': has_sweep,
        'supports_spectrum': supports_spectrum,
        'centers_hz': centers if supports_spectrum else [],
        'band_models_pchip': bands if supports_spectrum else [],
        'rpm_min': m.get('rpm_min') if m.get('rpm_min') is not None else calib_model.get('x0'),
        'rpm_max': m.get('rpm_max') if m.get('rpm_max') is not None else calib_model.get('x1'),
        'calibration': {
...
def _fetch_active_audio_bindings_for_pairs(pairs: List[Tuple[int, int]]) -> Dict[Tuple[int, int], Dict[str, Any]]:
    tuple_placeholders, params = _build_pair_placeholders(pairs)
    where_pairs = ",".join(tuple_placeholders)
    binding_rows = []
    try:
        binding_rows = fetch_all(f"""
            SELECT v.model_id, v.condition_id, v.audio_batch_id,
                   v.perf_batch_id, v.params_json,
                   v.audio_data_hash
            FROM audio_perf_binding_valid_view v
            WHERE (v.model_id, v.condition_id) IN ({where_pairs})
            ORDER BY v.model_id, v.condition_id
        """, params)
    except sa_exc.OperationalError as view_err:
        slog.info(
            "[/api/spectrum-models] audio_perf_binding_valid_view unavailable, fallback to join query: %s",
            view_err,
        )
        binding_rows = fetch_all(f"""
            SELECT apb.model_id, apb.condition_id, apb.audio_batch_id,
                   apb.perf_batch_id, apb.params_json,
                   ab.data_hash AS audio_data_hash
            FROM audio_perf_binding apb
            JOIN (
                SELECT DISTINCT batch_id, model_id, condition_id
                FROM fan_performance_data
                WHERE is_valid = 1
                  AND (model_id, condition_id) IN ({where_pairs})
            ) valid_batch ON valid_batch.batch_id = apb.perf_batch_id
            LEFT JOIN audio_batch ab ON ab.audio_batch_id = apb.audio_batch_id
            WHERE (apb.model_id, apb.condition_id) IN ({where_pairs})
            ORDER BY apb.model_id, apb.condition_id
        """, params)
    binding_lookup: Dict[Tuple[int, int], Dict[str, Any]] = {}
...
def _serialize_model_meta_payload(meta: dict, *, include_extended: bool = False) -> dict:
    payload = {
        ...
        'purchase_links': [dict(link) for link in (meta.get('purchase_links') or [])],
        'whistle_value': meta.get('whistle_value'),
        'whistle_baseline_condition_id': meta.get('whistle_baseline_condition_id'),
    }
    if include_extended:
        payload.update({
...
cache_event_handlers.setup(app.logger)
cache_event_bus.setup(engine, app.logger)
user_activity.setup(fetch_all, exec_write, engine, app.logger, recent_likes_limit=RECENT_LIKES_LIMIT)
app.register_blueprint(user_activity_bp)
purchase_links.setup(fetch_all, exec_write, app.logger)
app.register_blueprint(purchase_links.bp)
file_share.setup(fetch_all, exec_write, app.logger)
app.register_blueprint(file_share.bp)
issue_feedback.setup(fetch_all, exec_write, app.logger)
app.register_blueprint(issue_feedback_bp)
scoring_system.start_background_threads()
cache_event_bus.start_background_consumer(cache_event_handlers.handle_event)
...
def app_access_gate():
    ...
    if not APP_REQUIRE_UID_WHITELIST and not APP_REQUIRE_PASSWORD:
        return None

    uid = user_activity.get_or_create_user_identifier()
    decision = access_control.evaluate_uid_access(
        fetch_all,
        uid,
        service='app',
        require_uid=APP_REQUIRE_UID_WHITELIST,
        require_password=APP_REQUIRE_PASSWORD,
    )
    if not decision.allowed:
        if path.startswith('/api/'):
            return resp_err(decision.reason_code or 'ACCESS_DENIED', decision.message or '访问受限', 403)
        return render_template(
            'access_denied.html',
            user_identifier=uid,
            error_message=decision.message or '访问受限',
        ), 403

    if not decision.requires_password:
        return None

    sess_uid = session.get('app_auth_user_id')
    if access_control.is_authenticated_user_allowed(decision, sess_uid):
        return None

    auth = request.authorization
    login_name = (auth.username or '').strip() if auth else ''
    password = auth.password if auth else None
    if not login_name or password is None:
        return _app_password_required_response()

    user_row = access_control.fetch_auth_user_by_login(fetch_all, login_name)
    if not user_row or int(user_row.get('is_active') or 0) != 1:
        time.sleep(0.2)
        return _app_password_required_response('用户名或密码不正确')

    locked, left_min = access_control.is_auth_user_locked(user_row)
    if locked:
        return _app_password_required_response(f'账户被临时锁定，请 {left_min} 分钟后再试')
    ...
    if not ok:
        ...
        rows2 = fetch_all("SELECT failed_attempts FROM auth_user WHERE id=:id", {'id': user_row['id']})
        curr = int(rows2[0]['failed_attempts']) if rows2 else 1
        if curr >= APP_LOGIN_MAX_ATTEMPTS:
            exec_write(
                """
                UPDATE auth_user
                ...
                WHERE id=:id
                """,
                {'id': user_row['id'], 'lu': _app_lock_until_str(APP_LOGIN_LOCK_MINUTES)},
            )
            return _app_password_required_response(f'因多次失败，账户已锁定 {APP_LOGIN_LOCK_MINUTES} 分钟')
        exec_write(
            """
            UPDATE auth_user
            ...
            WHERE id=:id
            """,
            {'id': user_row['id'], 'fa': curr},
        )
        return _app_password_required_response('用户名或密码不正确')

    if not access_control.is_authenticated_user_allowed(decision, user_row['id']):
        if path.startswith('/api/'):
            return resp_err('DEVICE_BOUND_CONFLICT', '当前设备绑定账号与认证账号不一致，请联系管理员。', 403)
        return render_template(
            'access_denied.html',
            user_identifier=uid,
            error_message='当前设备绑定账号与认证账号不一致，请联系管理员。',
        ), 403
    ...
    session['app_auth_user_id'] = int(user_row['id'])
    session['app_auth_login_name'] = user_row.get('login_name') or ''
...
def api_access_me():
    return resp_ok({
        'uid_whitelist_required': APP_REQUIRE_UID_WHITELIST,
        'password_required': APP_REQUIRE_PASSWORD,
        'authenticated': bool(session.get('app_auth_user_id')),
        'login_name': session.get('app_auth_login_name'),
    })


@app.post('/api/access/logout')
def api_access_logout():
    session.pop('app_auth_user_id', None)
    session.pop('app_auth_login_name', None)
    return resp_ok(message='已退出应用访问登录')
    ...
    for model_id, meta in all_meta.items():
        ...
        if 'chain' in _other_set:
            try:
                ...
                continue
        if 'stop_pwm' in _other_set:
            raw_stop_pwm = meta.get('stop_pwm')
            if raw_stop_pwm is None or str(raw_stop_pwm).strip() == '':
                continue
...
def search_fans_by_condition_with_fit(condition_id=None, sort_by='none', sort_value=None,
    ...
    for (mid, cid), g in groups.items():
        ...
        items.append({
            ...
            'color_flags': meta.get('color_flags'),
            'reverse_opt': meta.get('reverse_opt'),
            'caution': int(meta.get('caution') or 0),
        })
...
def api_curves():
    ...
    try:
      ...
      # 统一构建四合一拟合模型（含缓存/失效处理）；raw 锚点一并存入 perf cache
      perf_map = spectrum_reader.build_performance_pchips(uniq)  # { "m_c": { pchip:{...}, raw:{...} } }

      # 元信息从 meta cache 获取（品牌/型号/工况名）
      ...
      for mid, cid in uniq:
          ...
          model_meta = model_meta_map.get(mid) or {}
          condition_meta = condition_meta_map.get(cid) or {}

          # 四合一 PCHIP
          pset = (perf.get('pchip') or {})
          supports_audio = perf.get('supports_audio', False)

          # 原始锚点直接从 perf cache raw 块读取
          rpm_arr      = raw.get('rpm') or []
          noise_arr    = raw.get('noise_db') or []
          air_arr      = raw.get('airflow') or []
          pressure_arr = raw.get('pressure_mmh2o') or []

          # sone 只从 perf cache 读取（spectrum cache 不再承担 sone 职责）。
          sone_arr = []
          pchip_rpm_to_sone = pset.get('rpm_to_sone')
          pchip_sone_to_rpm = pset.get('sone_to_rpm')
          pchip_sone_to_airflow = pset.get('sone_to_airflow')
          if pchip_rpm_to_sone:
              for rpm_val in rpm_arr:
                  try:
                      sv = eval_pchip(pchip_rpm_to_sone, float(rpm_val)) if rpm_val is not None else None
                      sone_arr.append(round(float(sv), 4) if sv is not None and math.isfinite(float(sv)) else None)
                  except Exception:
                      sone_arr.append(None)

          series.append(dict(
              key=k,
              name=f"{model_meta.get('brand_name_zh', '')} {model_meta.get('model_name', '')} - {condition_meta.get('condition_name_zh', '')}",
              brand=model_meta.get('brand_name_zh', ''),
              model=model_meta.get('model_name', ''),
              condition=condition_meta.get('condition_name_zh', ''),
              model_id=mid,
              condition_id=cid,
              data={
                  'rpm': rpm_arr,
                  'noise_db': noise_arr,
                  'noise_sone': sone_arr if sone_arr else [],
                  'airflow': air_arr,
                  'pressure_mmh2o': pressure_arr,
              },
              pchip={
                  ...
                  'rpm_to_sone':      pchip_rpm_to_sone,
                  'sone_to_rpm':      pchip_sone_to_rpm,
                  'sone_to_airflow':  pchip_sone_to_airflow,
              },
              supports_audio=supports_audio
          ))
...
def api_search_fans():
    try:
        ...
        allowed_other_features = {'reverse_opt', 'speed_switch', 'chain', 'DBB', 'no-DBB', 'stop_pwm'}
        ...
        if not composite_mode:
            ...
            if not cond_meta:
                return resp_err('SEARCH_INVALID_CONDITION_ID', 'condition_id 非法')
            if SEARCH_CONDITION_IDS and condition_id not in SEARCH_CONDITION_IDS:
                return resp_err('SEARCH_INVALID_CONDITION_ID', 'condition_id 非法')
        ...
        for item in results:
            ...
            if facts:
                ...
                lighting_like_count = facts.get('lighting_like_count', 0)
                radar_add_count = facts.get('radar_add_count', 0)
            else:
                heat_score      = 0
            ...
            item['lighting_like_count'] = lighting_like_count
            item['radar_add_count'] = radar_add_count
            ...
            if isinstance(model_meta, dict):
                ...
                item['stop_pwm'] = model_meta.get('stop_pwm')
                ...
                item['caution'] = int(model_meta.get('caution') or 0)
                item['purchase_links'] = list(model_meta.get('purchase_links') or [])
...
def api_search_metadata():
    ...
    try:
        ...
        # Advanced-search condition order/filter is backend-owned via SEARCH_CONDITION_IDS.
        # Frontend only pins the composite option above this ordered backend payload.
        conditions = []
        if SEARCH_CONDITION_IDS:
            cond_meta_map = condition_meta_cache.get_many_condition_meta(SEARCH_CONDITION_IDS)
            for condition_id in SEARCH_CONDITION_IDS:
                cond = cond_meta_map.get(condition_id)
                if not cond:
                    app.logger.warning(
                        'SEARCH_CONDITION_IDS contains unknown condition_id=%s; skipping advanced-search option',
                        condition_id,
                    )
                    continue
                conditions.append(_serialize_condition_item(cond))
        else:
            conditions = [
                _serialize_condition_item(cond)
                for cond in condition_meta_cache.get_all_conditions()
            ]
        ...
        other_features = [
            ...
            {'key': 'stop_pwm', 'label_zh': '支持停转', 'label_en': 'Stop support'},
...
def api_model_meta():
    ...
    thickness, rgb_flags/names_zh/names_en, bearing_type_zh/en, review,
    purchase_links) for the requested model_ids.
    ...
    Response shape:
      {
        "models": {
          "<model_id>": {
            ...
            "review": "...",
            "purchase_links": [{"id": 1, "platform_code": "jd", ...}]
          }, ...
        }
      }
    """
...
def api_internal_warm_scores():
    ...
    try:
        ...
        def _warm():
            try:
                cache_event_handlers.warm_scores(model_id, condition_id, rebuild_pchip=True)
            except Exception as e:
                app.logger.warning('[warm_scores] model_score recompute failed (%s,%s): %s', model_id, condition_id, e)
...
def api_internal_refresh_scoring_visibility():
    ...
    Steps:
      1. Immediately invalidate ModelMetaCache so that subsequent reads
         (search, suggest, validate_models, etc.) see the new visibility.
      2. Reload ModelMetaCache with a forced refresh.
      3. Rebuild scoring / ranking caches via refresh_visibility_scoring_caches().
    """
    try:
        auth_err = _require_internal_warmup_token()
        if auth_err is not None:
        ...
        old_visibility_scope = data.get('old_visibility_scope')
        new_visibility_scope = data.get('new_visibility_scope')
        wait_for_completion = str(data.get('wait_for_completion', '')).strip().lower() in ('1', 'true', 'yes', 'on')

        def _refresh():
            try:
                cache_event_handlers.refresh_scoring_visibility(
                    model_id,
                    old_is_valid,
                    new_is_valid,
        ...
        if wait_for_completion:
            _refresh()
            return resp_ok({'queued': False, 'refreshed': True})

        threading.Thread(target=_refresh, daemon=True, name='visibility-refresh').start()
        return resp_ok({'queued': True, 'refreshed': False})
    except Exception as e:
        app.logger.exception(e)
        return resp_err('INTERNAL_ERROR', str(e), 500)
...
def api_ladder():
    ...
    Each item in "items":
      { "rank": int, "model_name": str, "brand_name": str, "bearing_type_zh": str,
        "rgb_names_zh": str, "whistle_value": float|null, "score": float, "reference_price": float|null,
        "caution": int }

    caution: 0 = normal display; 1 = show light-orange rounded background on the
      model name in the ladder canvas (indicates possible design/quality defect).
    """
    try:
        _ensure_ladder_warmup_hook_registered()
...
def api_spectrum_models():
    ...
    try:
        ...
        binding_lookup = _fetch_active_audio_bindings_for_pairs(uniq)
        ...
        for mid, cid in uniq:
            ...
            # audio_data_hash comes from audio_batch.data_hash (fetched in the binding query)
            binding_audio_data_hash = str(binding.get('audio_data_hash') or '')
            ...
            # 3) 一致性校验：cache meta 必须与当前有效 binding 完全匹配
            cached_ok = False
            if cur_meta:
                meta_abid = str(cur_meta.get('audio_batch_id') or '')
                ...
                cached_ok = (
                    meta_abid == audio_batch_id and
                    meta_param == param_hash and
                    meta_code == code_ver and
                    bool(meta_audio) and
                    (not binding_audio_data_hash or meta_audio == binding_audio_data_hash) and
                    bool(cur_model_raw)
                )
                slog.info(
                    "  check cache: meta_abid=%s cur_abid=%s meta_param=%s cur_param=%s "
                    "meta_code=%s cur_code=%s meta_audio=%s bind_audio=%s -> ok=%s",
                    meta_abid, audio_batch_id, meta_param, param_hash,
                    meta_code, code_ver, meta_audio, binding_audio_data_hash, cached_ok
                )

            if cached_ok:
                # 4) 缓存一致，返回瘦身后的模型
                slim = _slim_spectrum_model_for_client(cur_model_raw)
                models.append({
                    'key': f'{mid}_{cid}',
                    'model_id': mid,
                    'condition_id': cid,
...
def api_gallery_list(model_id):
    """Return gallery metadata for a model, including video_BV and derived poster URLs."""
    try:
        ...
        # Compute video_poster_url: local BV poster cache URL, or None.
        video_poster_url = _get_or_cache_bv_poster(video_bv) if video_bv else None
        video_poster_url = video_poster_url or None  # normalise '' → None

        # Compute lighting_preferred_thumb_url:
        #   1. BV poster (local cache)
        #   2. first gallery thumb
        #   3. None
        first_thumb = items[0]['thumb_url'] if items else None
        lighting_preferred_thumb_url = video_poster_url or first_thumb or None

        return resp_ok({
            'items': items,
            'video_BV': video_bv,
            'video_poster_url': video_poster_url,
            'lighting_preferred_thumb_url': lighting_preferred_thumb_url,
        })
    except Exception as e:
...
def _is_valid_bv_poster_filename(filename: str) -> bool:
    """Return True if *filename* is a safe BV poster filename: {BVxxx}.{allowed_ext}."""
    return _gallery.is_valid_bv_poster_filename(filename)


@app.route('/api/gallery/bilibili-poster/<path:filename>')
def api_gallery_bilibili_poster(filename):
    """Serve a locally-cached BV poster image."""
    if not _is_valid_bv_poster_filename(filename):
        abort(404)
    poster_dir = _gallery.bilibili_posters_dir()
...
# SSRF guard: only allow fetching poster images from known Bilibili CDN domains.
_BILIBILI_ALLOWED_POSTER_HOSTS = frozenset({
...
def _ext_from_content_type(content_type: str) -> str:
    ...
    return _gallery.ext_from_content_type(content_type)


def _ext_from_url(url: str) -> str:
    """Return a normalised image extension from a URL path, or ''."""
    return _gallery.ext_from_url(url)


def _get_or_cache_bv_poster(bvid: str) -> str:
    """
    Return a same-origin URL for the BV poster, using local disk cache.

    Strategy:
      1. Check bilibili_posters/ for an already-cached file.
      2. If missing, fetch from Bilibili, determine extension from
         Content-Type / URL, and save the raw bytes to bilibili_posters/.
      3. Return the local /api/gallery/bilibili-poster/<file> URL on success,
         or '' on any failure (so callers can treat empty string as unavailable).

    Failures are logged at DEBUG level and never propagate to the caller.
    """
    if not bvid:
        return ''
    local_url = _gallery.get_cached_bv_poster_url(bvid)
    if local_url:
        return local_url
    try:
        ...
        if parsed.hostname not in _BILIBILI_ALLOWED_POSTER_HOSTS:
            app.logger.debug(
                '_get_or_cache_bv_poster: untrusted host %s for bvid=%s',
                parsed.hostname, bvid,
            )
            return ''
        ...
        with requests.get(safe_url, headers=_BILIBILI_HEADERS, timeout=10, stream=True, allow_redirects=False) as img_resp:
            if img_resp.is_redirect or img_resp.is_permanent_redirect:
                app.logger.debug('_get_or_cache_bv_poster: redirect blocked for bvid=%s', bvid)
                return ''
            ...
            try:
                if cl is not None and int(cl) > _MAX_POSTER_BYTES:
                    app.logger.debug('_get_or_cache_bv_poster: poster too large for bvid=%s', bvid)
                    return ''
            except ValueError:
                pass
            for chunk in img_resp.iter_content(chunk_size=65536):
                ...
                chunks.append(chunk)
        data = b''.join(chunks)
        ext = _ext_from_content_type(content_type) or _ext_from_url(poster_url)
        if not ext:
            app.logger.debug('_get_or_cache_bv_poster: unrecognised format for bvid=%s ct=%s', bvid, content_type)
            return ''
        cached_url = _gallery.cache_bv_poster(bvid, data, ext)
        return cached_url or ''
    except Exception as exc:
        app.logger.debug('_get_or_cache_bv_poster error bvid=%s: %s', bvid, exc)
        return ''
...
def api_bilibili_poster_proxy():
    ...
    try:
        ...
        try:
            ...
            if parsed_poster.hostname not in _BILIBILI_ALLOWED_POSTER_HOSTS:
                app.logger.warning(
...
# =========================================
# BV Poster Orphan Cleanup (daily, file-locked)
# =========================================

def _bv_poster_cleanup_loop():
    """Background daemon thread: remove orphan BV poster files once per day.

    Cross-worker deduplication uses the same startup_lock pattern as other
    background threads in this file (a cross-process file lock so that only
    one Gunicorn worker runs each daily sweep).
    """
    import random as _rand
    # Stagger initial sleep across workers (1–5 min) to avoid simultaneous
    # DB queries right after all workers start up.
    _CLEANUP_STARTUP_STAGGER_SEC = (60, 300)
    time.sleep(_rand.uniform(*_CLEANUP_STARTUP_STAGGER_SEC))
    while True:
        _lock_path = os.path.join(tempfile.gettempdir(), 'fancool_bv_poster_cleanup.lock')
        try:
            with startup_lock(_lock_path) as acquired:
                if acquired:
                    try:
                        rows = fetch_all("SELECT DISTINCT video_BV FROM fan_model WHERE video_BV IS NOT NULL AND video_BV != ''")
                        active_bvids = {(r.get('video_BV') or '').strip() for r in rows}
                        active_bvids.discard('')
                        deleted = _gallery.cleanup_orphan_bv_posters(active_bvids)
                        if deleted:
                            app.logger.info('[bv_poster_cleanup] deleted %d orphan poster(s)', deleted)
                    except Exception as exc:
                        app.logger.warning('[bv_poster_cleanup] error during sweep: %s', exc)
        except Exception as exc:
            app.logger.warning('[bv_poster_cleanup] lock error: %s', exc)
        # Sleep until the same time tomorrow (approximately 24 h).
        time.sleep(86400)


threading.Thread(target=_bv_poster_cleanup_loop, daemon=True, name='bv-poster-cleanup').start()
