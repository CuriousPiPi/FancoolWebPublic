...
def setup(fetch_all_fn, exec_write_fn, engine_obj, logger_obj, recent_likes_limit=None):
    """Inject shared dependencies.  Call once during app startup."""
    global _fetch_all, _exec_write, _engine, _logger, _recent_likes_limit
    _fetch_all = fetch_all_fn
    _exec_write = exec_write_fn
    _engine = engine_obj
    _logger = logger_obj
    _recent_likes_limit = recent_likes_limit
...
def api_recent_likes():
    ...
    try:
        user_id = get_or_create_user_identifier()
        limit = int(_recent_likes_limit) if isinstance(_recent_likes_limit, int) and _recent_likes_limit > 0 else 100
        items = get_user_rate_events_full(user_id, limit=limit)
        fp = compute_like_fingerprint(user_id)
