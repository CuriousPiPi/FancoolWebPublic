...
from app import like_rank_cache
from app import lighting_like_cache
from app.curves.lock_utils import startup_lock
from app.audio_services import spectrum_reader
from app import model_meta_cache
...
_rankings_warmup_hooks: list[Callable[[dict, str], None]] = []
_rankings_warmup_hooks_lock = threading.Lock()
...
def _rankings_cache_path() -> str:
    ...
    return os.path.join(_shared_cache_dir(), f'rankings_v3_{safe_key}{_score_cache_suffix()}.json')
...
def _canonical_facts_path() -> str:
    return os.path.join(_shared_cache_dir(), f'canonical_facts_v2{_score_cache_suffix()}.json')
...
def _build_canonical_facts() -> dict:
    ...
    sql_radar_add = """
        ...
        WHERE event_time > (NOW() - INTERVAL 30 DAY)
          AND event_type = 'add'
          AND model_id IN (SELECT DISTINCT model_id FROM mids_cids_in_data_view)
        GROUP BY model_id
    """
    ...
    try:
        ...
        _logger.warning('[canonical_facts] condition query fetch failed: %s', e)
        cq_rows = []
    try:
        radar_add_rows = _fetch_all(sql_radar_add)
    except Exception as e:
        _logger.warning('[canonical_facts] radar-add fetch failed: %s', e)
        radar_add_rows = []
    ...
    # Lighting like counts (model-level, rate_dimension_id=2, condition_id=0).
    # These contribute to heat_score with the same ×10 multiplier as condition
    # likes, but they must NOT pollute condition_heat / condition_likes.
    llc_snapshot: dict[int, int] = {}
    try:
        lighting_like_cache.ensure_cache(_fetch_all)
    except Exception as e:
        _logger.warning('[canonical_facts] lighting like cache refresh failed: %s', e)
    try:
        llc_snapshot = lighting_like_cache.get_all_lighting_like_counts()
    except Exception as e:
        _logger.warning('[canonical_facts] lighting like cache snapshot read failed: %s', e)
    ...
    # canonical_facts/model_lookup must cover every currently visible model, even
    # when it has no recent heat activity yet.  Downstream boards/search/recent
    # updates decide whether to display a model; the candidate set itself must
    # not be filtered by heat/query/like behavior.
    all_mids: set[int] = set()
    visible_mids_loaded = False
    try:
        for raw_mid in model_meta_cache.get_all_model_ids(force_refresh=False):
            try:
                mid = int(raw_mid)
            except Exception:
                continue
            if mid > 0:
                all_mids.add(mid)
        visible_mids_loaded = True
    except Exception as e:
        _logger.warning('[canonical_facts] visible model fetch failed: %s', e)

    if not visible_mids_loaded:
        all_mids = set(cond_data.keys()) | set(llc_snapshot.keys()) | set(model_add_counts.keys())

    model_lookup: dict[int, dict] = {}
    for mid in all_mids:
        cid_map = cond_data.get(mid, {})
        ...
        lighting_like_count = llc_snapshot.get(mid, 0)
        # heat_score: condition queries + (condition likes × 10) + (lighting likes × 10) + radar-add × 2
        # condition_heat and condition_likes use only real condition rows (cid_map).
        heat_score  = total_query + total_like * 10 + lighting_like_count * 10 + model_add * 2

        condition_heat: dict[int, int] = {}
        condition_likes: dict[int, int] = {}
        # Per-radar-slot query counts are kept separately so API layers can consume
        # raw query facts without reverse-deriving from heat or likes.
        condition_queries: dict[int, int] = {}
        for cid in RADAR_CIDS:
            entry = cid_map.get(cid)
            if entry:
                condition_queries[cid] = entry['query']
                condition_heat[cid]  = entry['query'] + entry['like'] * 10
                condition_likes[cid] = entry['like']
            else:
                condition_queries[cid] = 0
        ...
        model_lookup[mid] = {
            ...
            'lighting_like_count': lighting_like_count,
            'radar_add_count':    model_add,
...
def _normalize_canonical_facts_payload(model_lookup: dict) -> dict:
    ...
    for k, v in model_lookup.items():
        ...
        if isinstance(v, dict):
            ...
            cq = v_local.get('condition_queries')
            if isinstance(cq, dict):
                v_local['condition_queries'] = {int(ck): cv for ck, cv in cq.items()}
...
def refresh_model_score_cache(model_id: int) -> dict | None:
    """Recompute a model's score entry and refresh the in-process cache."""
...
def score_price_model_sort_key(score, reference_price, model_id):
    """Unified score ordering: score DESC, price ASC(None last), model_id DESC."""
    score_val = score if score is not None else float('-inf')
    price = reference_price
    price_missing = 1 if price is None else 0
...
def composite_score_sort_key(composite_score, model_id, reference_price=None):
    """Unified composite-score ordering: score DESC, price ASC(None last), model_id DESC."""
    return score_price_model_sort_key(composite_score, reference_price, model_id)
...
def _build_rankings_v2() -> dict:
    ...
    model_meta_map = model_meta_cache.get_many_model_meta(facts_lookup.keys())
    items = []
    for mid, facts in facts_lookup.items():
        model_meta = model_meta_map.get(mid)
        if not isinstance(model_meta, dict):
            continue
        raw_tier_list = model_meta.get('tier_list')
        try:
            tier_list = int(raw_tier_list) if raw_tier_list not in (None, '') else 1
        except (TypeError, ValueError):
            tier_list = 1
        ...
        ranking_item = {
            ...
            'brand_name_en':   model_meta.get('brand_name_en', ''),
            ...
            'speed_switch_type_name_zh': model_meta.get('speed_switch_type_name_zh'),
            'speed_switch_type_name_en': model_meta.get('speed_switch_type_name_en'),
            'stop_pwm':       model_meta.get('stop_pwm'),
            'chain_type_name_zh': model_meta.get('chain_type_name_zh'),
            'chain_type_name_en': model_meta.get('chain_type_name_en'),
            'color_flags':     model_meta.get('color_flags'),
            'reverse_opt':     model_meta.get('reverse_opt'),
            'caution':         int(model_meta.get('caution') or 0),
            'tier_list':       tier_list,
            'query_count':     total_query,
            'like_count':      total_like,
            ...
            'purchase_links': list(model_meta.get('purchase_links') or []),
        }
        if not _is_displayable_ranking_item(ranking_item):
            continue
    ...
    return {
        ...
        'lighting_board': _build_lighting_board(items, limit),
...
def _performance_sort_key_for_item(row: dict):
    return composite_score_sort_key(
...
def _build_lighting_board(items: list[dict], limit: int) -> list[dict]:
    """Build the lighting board: only RGB models (rgb_flags != 0), sorted by lighting_like_count DESC."""
    rgb_items = []
    for row in items:
        try:
            rgb_flags = int(row.get('rgb_flags') or 0)
        except (TypeError, ValueError):
            rgb_flags = 0
        if rgb_flags == 0:
            continue
        if int(row.get('lighting_like_count') or 0) <= 0:
            continue
        rgb_items.append(row)
    ...
    for i, row in enumerate(
        sorted(
            rgb_items,
            key=lambda x: (
                -int(x.get('lighting_like_count') or 0),
        ...
        lighting_board.append({**row, 'rank': i})
    return lighting_board
...
def _filter_non_displayable_rankings_result(
    ...
    return {
        ...
        'lighting_board': _build_lighting_board(filtered_items, limit),
...
def get_recent_updates(limit: int = 100) -> List[dict]:
    ...
    if not RADAR_CIDS:
        return []
    cid_placeholders = ','.join(f':c{i}' for i in range(len(RADAR_CIDS)))
    params = {'l': limit, **{f'c{i}': cid for i, cid in enumerate(RADAR_CIDS)}}

    sql = f"""
      SELECT
        model_id,
        DATE_FORMAT(MAX(u.update_date), '%Y-%m-%d') AS update_date
      FROM update_notice_d30_view u
      WHERE u.condition_id IN ({cid_placeholders})
      GROUP BY model_id
    ...
    for r in rows:
        try:
            model_ids.append(int(r['model_id']))
    ...
    for r in rows:
        ...
        query_count    = facts.get('query_count',    0) if facts else 0
        like_count     = facts.get('like_count',     0) if facts else 0
        lighting_like_count = facts.get('lighting_like_count', 0) if facts else 0
        radar_add_count = facts.get('radar_add_count', 0) if facts else 0
        ...
        items.append({
            'model_id':        mid,
            'brand_name_zh':   model_meta.get('brand_name_zh') or '',
            'model_name':      model_meta.get('model_name') or '',
            'size':            model_meta.get('size'),
            'thickness':       model_meta.get('thickness'),
            'max_speed':       model_meta.get('max_speed'),
            'reference_price': model_meta.get('reference_price'),
            ...
            'lighting_like_count': lighting_like_count,
            'radar_add_count': radar_add_count,
            'heat_score':      heat_score,
            'composite_score': composite_score,
            'condition_scores': condition_scores,
            'condition_heat':   condition_heat,
...
def _do_denom_refresh():
    ...
    try:
        ...
        pair_rows = _fetch_all(
            f"SELECT DISTINCT model_id, condition_id FROM mids_cids_in_data_view "
...
def _fetch_current_visible_model_ids() -> set[int]:
    visible: set[int] = set()
    for raw_mid in model_meta_cache.get_all_model_ids():
        try:
            mid = int(raw_mid)
        except Exception:
            continue
...
def _warmup_rankings_async() -> None:
    ...
    def _run():
        try:
            ...
            try:
                _invalidate_rankings_v2_cache(purge_disk=True)
                rankings_result = get_rankings_v2()
                _run_rankings_warmup_hooks(rankings_result, trigger_source='warmup')
                if _app_debug:
...
def register_rankings_warmup_hook(hook: Callable[[dict, str], None]) -> None:
    """Register a callback that runs after rankings warm-up completes."""
    if not callable(hook):
        return
    with _rankings_warmup_hooks_lock:
        if hook in _rankings_warmup_hooks:
            return
        _rankings_warmup_hooks.append(hook)


def _run_rankings_warmup_hooks(rankings_result: dict, trigger_source: str) -> None:
    with _rankings_warmup_hooks_lock:
        hooks = list(_rankings_warmup_hooks)
    for hook in hooks:
        try:
            hook(rankings_result, trigger_source)
        except Exception as e:
            _logger.warning('[warmup] rankings hook failed: %s', e)
