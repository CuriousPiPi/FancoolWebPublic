# -*- coding: utf-8 -*-
"""
like_rank_cache: Full-table shared cache for active_like_d30_view (condition likes).

Loads the condition-like subset of active_like_d30_view once per TTL period and exposes
...
def _fetch_from_db(fetch_all_fn) -> Dict[Tuple[int, int], int]:
    """Query active_like_d30_view (condition-like slice) and return a (mid, cid) -> count dict."""
    rows = fetch_all_fn(
        "SELECT model_id, condition_id, "
        "COALESCE(active_like_d30, 0) AS like_count "
        "FROM active_like_d30_view "
        "WHERE rate_dimension_id = 1 AND condition_id > 0"
    )
